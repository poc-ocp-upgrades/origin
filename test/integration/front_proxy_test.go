...
func TestFrontProxy(t *testing.T) {
	...
	masterConfig.AuthConfig.RequestHeader = &config.RequestHeaderAuthenticationOptions{ClientCA: frontProxyClientCA, ClientCommonNames: []string{proxyCertCommonName}, UsernameHeaders: []string{proxyUserHeader}, GroupHeaders: []string{proxyGroupHeader}}
	...
	if _, err := clusterAdminAuthorizationClient.ClusterRoles().Create(&authorizationapi.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: listProjectsRoleName}, Rules: []authorizationapi.PolicyRule{authorizationapi.NewRule("list").Groups(legacy.GroupName).Resources("projects").RuleOrDie()}}); err != nil {
		t.Fatal(err)
	}
	for _, username := range []string{"david", "jordan"} {
		...
		if _, err := clusterAdminAuthorizationClient.ClusterRoleBindings().Create(&authorizationapi.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: username + "-clusterrolebinding"}, Subjects: []kapi.ObjectReference{{Kind: authorizationapi.UserKind, Name: username}}, RoleRef: kapi.ObjectReference{Name: listProjectsRoleName}}); err != nil {
			t.Fatal(err)
		}
	}
	...
	}{{name: "empty user", isUnauthorized: true}, {name: "david can only see his project", user: &user.DefaultInfo{Name: "david"}, expectedProjects: sets.NewString("david-project")}, {name: "david can see all projects when given cluster admin group", user: &user.DefaultInfo{Name: "david", Groups: []string{bootstrappolicy.ClusterAdminGroup}}, expectedProjects: sets.NewString("david-project", "jordan-project", "default", "kube-public", "kube-system", "openshift", "openshift-infra", "openshift-node")}} {
...
func newFrontProxyHandler(rawURL, clientCA, userHeader, groupHeader string, proxyCert *tls.Certificate) (*frontProxyHandler, error) {
	...
	return &frontProxyHandler{proxier: proxier, userHeader: userHeader, groupHeader: groupHeader}, nil
}
func mutualAuthRoundTripper(ca string, cert *tls.Certificate) (http.RoundTripper, error) {
	...
	tlsConfig := &tls.Config{RootCAs: bundle, ClientCAs: bundle, ClientAuth: tls.RequireAndVerifyClientCert, Certificates: []tls.Certificate{*cert}}
...
func createClientCert(commonName, certDir, caPrefix string) (*tls.Certificate, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	signerCertOptions := &admin.SignerCertOptions{CertFile: admin.DefaultCertFilename(certDir, caPrefix), KeyFile: admin.DefaultKeyFilename(certDir, caPrefix), SerialFile: admin.DefaultSerialFilename(certDir, caPrefix)}
	clientCertOptions := &admin.CreateClientCertOptions{SignerCertOptions: signerCertOptions, CertFile: admin.DefaultCertFilename(certDir, commonName), KeyFile: admin.DefaultKeyFilename(certDir, commonName), ExpireDays: crypto.DefaultCertificateLifetimeInDays, User: commonName, Overwrite: true}
...
func createServerCert(hostnames []string, commonName, certDir, caPrefix string) (*tls.Certificate, error) {
	...
	defer _logClusterCodePath()
	signerCertOptions := &admin.SignerCertOptions{CertFile: admin.DefaultCertFilename(certDir, caPrefix), KeyFile: admin.DefaultKeyFilename(certDir, caPrefix), SerialFile: admin.DefaultSerialFilename(certDir, caPrefix)}
	serverCertOptions := &admin.CreateServerCertOptions{SignerCertOptions: signerCertOptions, CertFile: admin.DefaultCertFilename(certDir, commonName), KeyFile: admin.DefaultKeyFilename(certDir, commonName), ExpireDays: crypto.DefaultCertificateLifetimeInDays, Hostnames: hostnames, Overwrite: true}
