...
func TestOAuthServiceAccountClientEvent(t *testing.T) {
	...
	}{"test-good-url": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationURIPrefix + "one", annotation: "/oauthcallback", numEvents: 0}, "test-bad-url": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationURIPrefix + "one", annotation: "foo:foo", expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: "system:serviceaccount:" + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>", numEvents: 1, expectBadRequest: true}, "test-bad-url-parse": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationURIPrefix + "one", annotation: "::", expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: "[parse ::: missing protocol scheme, system:serviceaccount:" + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>]", numEvents: 1, expectBadRequest: true}, "test-bad-redirect-annotation-kind": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationReferencePrefix + "1", annotation: `{"kind":"foo","apiVersion":"oauth.openshift.io/v1","metadata":{"creationTimestamp":null},"reference":{"group":"foo","kind":"Route","name":"route1"}}`, expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: `[no kind "foo" is registered for version "oauth.openshift.io/v1" in scheme "github.com/openshift/origin/pkg/serviceaccounts/oauthclient/oauthclientregistry.go:54", system:serviceaccount:` + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>]", numEvents: 1, expectBadRequest: true}, "test-bad-redirect-type-parse": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationReferencePrefix + "1", annotation: `{asdf":"adsf"}`, expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: `[couldn't get version/kind; json parse error: invalid character 'a' looking for beginning of object key string, system:serviceaccount:` + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>]", numEvents: 1, expectBadRequest: true}, "test-bad-redirect-route-not-found": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationReferencePrefix + "1", annotation: buildRedirectObjectReferenceString(t, "Route", "route1", "route.openshift.io"), expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: `[routes.route.openshift.io "route1" not found, system:serviceaccount:` + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>]", numEvents: 1, expectBadRequest: true}, "test-bad-redirect-route-wrong-group": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationReferencePrefix + "1", annotation: buildRedirectObjectReferenceString(t, "Route", "route1", "foo"), expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: `system:serviceaccount:` + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>", numEvents: 1, expectBadRequest: true}, "test-bad-redirect-reference-kind": {annotationPrefix: saoauth.OAuthRedirectModelAnnotationReferencePrefix + "1", annotation: buildRedirectObjectReferenceString(t, "foo", "route1", "route.openshift.io"), expectedEventReason: "NoSAOAuthRedirectURIs", expectedEventMsg: `system:serviceaccount:` + projectName + ":" + saName + " has no redirectURIs; set serviceaccounts.openshift.io/oauth-redirecturi.<some-value>=<redirect> or create a dynamic URI using serviceaccounts.openshift.io/oauth-redirectreference.<some-value>=<reference>", numEvents: 1, expectBadRequest: true}}
...
func setupTestOAuthServer() (*testServer, error) {
	...
	return &testServer{clusterAdminKubeClient: clusterAdminKubeClientset, clusterAdminClientConfig: clusterAdminClientConfig, clusterAdminOAuthClient: clusterAdminOAuthClient, authCodes: authorizationCodes, authErrors: authorizationErrors, oauthServer: oauthServer, masterConfig: masterConfig}, nil
}
...
func doOAuthFlow(t *testing.T, clusterAdminClientConfig *restclient.Config, oauthClientConfig *osincli.ClientConfig, authorizationCodes chan string, authorizationErrors chan string, expectBadRequest bool, expectOperations []string) {
	...
	directHTTPClient := &http.Client{Transport: testTransport, CheckRedirect: func(redirectReq *http.Request, via []*http.Request) error {
