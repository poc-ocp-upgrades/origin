...
func TestOAuthExpiration(t *testing.T) {
	...
	{
		zero := int32(0)
		client, err := oauthClient.OAuthClients().Create(&oauthapi.OAuthClient{ObjectMeta: metav1.ObjectMeta{Name: "nonexpiring"}, RespondWithChallenges: true, RedirectURIs: []string{"http://localhost"}, AccessTokenMaxAgeSeconds: &zero, GrantMethod: oauthapi.GrantHandlerAuto})
		if err != nil {
			t.Fatal(err)
		}
	...
	{
		ten := int32(10)
		client, err := oauthClient.OAuthClients().Create(&oauthapi.OAuthClient{ObjectMeta: metav1.ObjectMeta{Name: "shortexpiring"}, RespondWithChallenges: true, RedirectURIs: []string{"http://localhost"}, AccessTokenMaxAgeSeconds: &ten, GrantMethod: oauthapi.GrantHandlerAuto})
		if err != nil {
			t.Fatal(err)
		}
...
func testExpiringOAuthFlows(t *testing.T, clusterAdminClientConfig *restclient.Config, oauthClient *oauthapi.OAuthClient, anonConfig *restclient.Config, expectedExpires int) string {
	...
	}
	{
		...
		conf := &oauth2.Config{ClientID: oauthClient.Name, ClientSecret: oauthClient.Secret, RedirectURL: oauthClient.RedirectURIs[0], Endpoint: oauth2.Endpoint{AuthURL: anonConfig.Host + "/oauth/authorize", TokenURL: anonConfig.Host + "/oauth/token"}}
