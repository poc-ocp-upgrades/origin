...
func TestImageStreamTagsAdmission(t *testing.T) {
	...
	for i, name := range []string{imagetest.BaseImageWith1LayerDigest, imagetest.BaseImageWith2LayersDigest, imagetest.MiscImageDigest} {
		...
		image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: name}, DockerImageReference: imageReference}
		tag := fmt.Sprintf("tag%d", i+1)
		_, err := client.Image().ImageStreamMappings(testutil.Namespace()).Create(&imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: "src"}, Tag: tag, Image: *image})
		if err != nil {
			t.Fatal(err)
		}
	}
	...
	ist := &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag1"}, Tag: &imageapi.TagReference{Name: "1", From: &kapi.ObjectReference{Kind: "ImageStreamImage", Name: "src@" + imagetest.BaseImageWith1LayerDigest}}}
	...
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag2"}, Tag: &imageapi.TagReference{Name: "2", From: &kapi.ObjectReference{Kind: "ImageStreamImage", Name: "src@" + imagetest.BaseImageWith2LayersDigest}}}
	_, err = client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
	if err == nil {
		t.Fatalf("expected error")
	}
	...
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag1again"}, Tag: &imageapi.TagReference{Name: "tag1again", From: &kapi.ObjectReference{Kind: "ImageStreamImage", Name: "src@" + imagetest.BaseImageWith1LayerDigest}}}
	_, err = client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	t.Log("trying to create ImageStreamTag in a new image stream")
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "new:misc"}, Tag: &imageapi.TagReference{Name: "misc", From: &kapi.ObjectReference{Kind: "ImageStreamImage", Name: "src@" + imagetest.MiscImageDigest}}}
	_, err = client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag2"}, Tag: &imageapi.TagReference{Name: "2", From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "src:tag2"}}}
	err = wait.ExponentialBackoff(quotaExceededBackoff, func() (bool, error) {
		_, err := client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
		if err != nil && !quotautil.IsErrorQuotaExceeded(err) {
			return false, err
	...
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag3"}, Tag: &imageapi.TagReference{Name: "3", From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "src:tag3"}}}
	_, err = client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
	if err == nil {
	...
	ist = &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "dest:tag2again"}, Tag: &imageapi.TagReference{Name: "tag2again", From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "src:tag2"}}}
	_, err = client.Image().ImageStreamTags(testutil.Namespace()).Update(ist)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImageStreamAdmitSpecUpdate(t *testing.T) {
	...
	limit := corev1.ResourceList{imagev1.ResourceImageStreamTags: resource.MustParse("0"), imagev1.ResourceImageStreamImages: resource.MustParse("0")}
	lrClient := kClient.CoreV1().LimitRanges(testutil.Namespace())
	createLimitRangeOfType(t, lrClient, limitRangeName, imagev1.LimitTypeImageStream, limit)
	t.Logf("trying to create a new image stream with a tag exceeding limit %v", limit)
	_, err := client.Image().ImageStreams(testutil.Namespace()).Create(newImageStreamWithSpecTags("is", map[string]kapi.ObjectReference{"tag1": {Kind: "ImageStreamTag", Name: "src:tag1"}}))
	if err == nil {
	...
	is.Spec.Tags["tag2"] = imageapi.TagReference{Name: "tag2", From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "src:tag2"}}
	...
	is.Spec.Tags["1again"] = imageapi.TagReference{Name: "1again", From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "src:tag1"}}
	_, err = client.Image().ImageStreams(testutil.Namespace()).Update(is)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestImageStreamAdmitStatusUpdate(t *testing.T) {
	...
	is.Status.Tags["tag1"] = imageapi.TagEventList{Items: []imageapi.TagEvent{{DockerImageReference: images[0].DockerImageReference, Image: images[0].Name}}}
	...
	is.Status.Tags["tag2"] = imageapi.TagEventList{Items: []imageapi.TagEvent{{DockerImageReference: images[1].DockerImageReference, Image: images[1].Name}}}
	_, err = client.Image().ImageStreams(testutil.Namespace()).UpdateStatus(is)
	if err == nil {
		t.Fatalf("unexpected non-error")
	}
	...
	is.Status.Tags["1again"] = imageapi.TagEventList{Items: []imageapi.TagEvent{{DockerImageReference: images[0].DockerImageReference, Image: images[0].Name}}}
	_, err = client.Image().ImageStreams(testutil.Namespace()).UpdateStatus(is)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func newImageStreamWithSpecTags(name string, tags map[string]kapi.ObjectReference) *imageapi.ImageStream {
	...
	return &imageapi.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: imageapi.ImageStreamSpec{Tags: specTags}}
}
