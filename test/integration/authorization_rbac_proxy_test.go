...
func TestLegacyLocalRoleBindingEndpoint(t *testing.T) {
	...
	roleBindingToCreate := &authorizationv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: testBindingName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "testuser"}}, RoleRef: corev1.ObjectReference{Kind: "Role", Name: "edit", Namespace: namespace}}
	...
	roleBindingToEdit := &authorizationv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: testBindingName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "testuser"}, {Kind: rbacv1.UserKind, Name: "testuser2"}}, RoleRef: corev1.ObjectReference{Kind: "Role", Name: "edit", Namespace: namespace}}
	...
	localClusterRoleBindingToCreate := &authorizationv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "test-crb", Namespace: namespace}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "testuser"}}, RoleRef: corev1.ObjectReference{Kind: "ClusterRole", Name: "edit"}}
...
func TestLegacyClusterRoleBindingEndpoint(t *testing.T) {
	...
	clusterRoleBindingToCreate := &authorizationv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: testBindingName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "testuser"}}, RoleRef: corev1.ObjectReference{Kind: "ClusterRole", Name: "edit"}}
	...
	clusterRoleBindingToEdit := &authorizationv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: testBindingName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "testuser"}, {Kind: rbacv1.UserKind, Name: "testuser2"}}, RoleRef: corev1.ObjectReference{Kind: "ClusterRole", Name: "edit"}}
...
func TestLegacyClusterRoleEndpoint(t *testing.T) {
	...
	clusterRoleToCreate := &authorizationv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: testRole}, Rules: []authorizationv1.PolicyRule{{Verbs: []string{"get"}, APIGroups: []string{""}, Resources: []string{"services"}}}}
	...
	clusterRoleUpdate := &authorizationv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: testRole}, Rules: []authorizationv1.PolicyRule{{Verbs: []string{"get", "list"}, APIGroups: []string{""}, Resources: []string{"services"}}}}
...
func TestLegacyLocalRoleEndpoint(t *testing.T) {
	...
	roleToCreate := &authorizationv1.Role{ObjectMeta: metav1.ObjectMeta{Name: testRole, Namespace: namespace}, Rules: []authorizationv1.PolicyRule{{Verbs: []string{"get"}, APIGroups: []string{""}, Resources: []string{"services"}}}}
	...
	roleUpdate := &authorizationv1.Role{ObjectMeta: metav1.ObjectMeta{Name: testRole, Namespace: namespace}, Rules: []authorizationv1.PolicyRule{{Verbs: []string{"get", "list"}, APIGroups: []string{""}, Resources: []string{"services"}}}}
...
func TestLegacyEndpointConfirmNoEscalation(t *testing.T) {
	...
	userName := "test-user"
	userSubjects := []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: userName}}
	...
	escalatingRules := []authorizationv1.PolicyRule{{Verbs: []string{"hug"}, APIGroups: []string{"bear"}, Resources: []string{"pandas"}}}
	nonEscalatingRules := []authorizationv1.PolicyRule{{Verbs: []string{"create"}, APIGroups: []string{kauthorizationv1.GroupName}, Resources: []string{"selfsubjectaccessreviews"}}}
	...
	clusterRoleObj, err := clusterAdminAuthorizationClient.ClusterRoles().Create(&authorizationv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: clusterRoleName}, Rules: []authorizationv1.PolicyRule{{Verbs: []string{"get", "create", "update"}, APIGroups: []string{authorizationv1.GroupName, rbacv1.GroupName}, Resources: []string{"clusterroles", "clusterrolebindings"}}}})
	if err != nil {
		t.Fatal(err)
	}
	if _, err := clusterAdminAuthorizationClient.ClusterRoleBindings().Create(&authorizationv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: clusterRoleName}, Subjects: userSubjects, RoleRef: corev1.ObjectReference{Name: clusterRoleName}}); err != nil {
		t.Fatal(err)
	}
		...
		for _, verb := range rule.Verbs {
			for _, group := range rule.APIGroups {
				for _, resource := range rule.Resources {
					if err := testutil.WaitForClusterPolicyUpdate(userInternalClient.AuthorizationV1(), verb, schema.GroupResource{Group: group, Resource: resource}, true); err != nil {
						t.Fatal(err)
					}
				}
			}
	...
	tests := []struct {
		...
		resource	string
		run		func() error
	}{{name: "role create", resource: "roles", run: func() error {
		_, err := userAuthorizationClient.Roles(namespace).Create(&authorizationv1.Role{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Rules: escalatingRules})
		return err
	}}, {name: "role update", resource: "roles", run: func() error {
		role, err := userAuthorizationClient.Roles(namespace).Create(&authorizationv1.Role{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Rules: nonEscalatingRules})
		if err != nil {
		...
		return err
	}}, {name: "role binding create", resource: "rolebindings", run: func() error {
		_, err := userAuthorizationClient.RoleBindings(namespace).Create(&authorizationv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Subjects: userSubjects, RoleRef: corev1.ObjectReference{Name: bootstrappolicy.ClusterAdminRoleName}})
		return err
	}}, {name: "role binding update", resource: "rolebindings", run: func() error {
		roleBinding, err := clusterAdminAuthorizationClient.RoleBindings(namespace).Create(&authorizationv1.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "some-other-user"}}, RoleRef: corev1.ObjectReference{Name: bootstrappolicy.ClusterAdminRoleName}})
		if err != nil {
			return fmt.Errorf("failed to create role binding: %v", err)
		}
		roleBinding.Subjects = userSubjects
		roleBinding.UserNames = nil
		_, err = userAuthorizationClient.RoleBindings(namespace).Update(roleBinding)
		return err
	}}, {name: "cluster role create", resource: "clusterroles", run: func() error {
		_, err := userAuthorizationClient.ClusterRoles().Create(&authorizationv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Rules: escalatingRules})
		return err
	}}, {name: "cluster role update", resource: "clusterroles", run: func() error {
		clusterRole, err := userAuthorizationClient.ClusterRoles().Create(&authorizationv1.ClusterRole{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Rules: nonEscalatingRules})
		if err != nil {
		...
		return err
	}}, {name: "cluster role binding create", resource: "clusterrolebindings", run: func() error {
		_, err := userAuthorizationClient.ClusterRoleBindings().Create(&authorizationv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Subjects: userSubjects, RoleRef: corev1.ObjectReference{Name: bootstrappolicy.ClusterAdminRoleName}})
		return err
	}}, {name: "cluster role binding update", resource: "clusterrolebindings", run: func() error {
		clusterRoleBinding, err := clusterAdminAuthorizationClient.ClusterRoleBindings().Create(&authorizationv1.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: resourceName}, Subjects: []corev1.ObjectReference{{Kind: rbacv1.UserKind, Name: "some-other-user"}}, RoleRef: corev1.ObjectReference{Name: bootstrappolicy.ClusterAdminRoleName}})
		if err != nil {
			return fmt.Errorf("failed to create cluster role binding: %v", err)
		}
		clusterRoleBinding.Subjects = userSubjects
		clusterRoleBinding.UserNames = nil
