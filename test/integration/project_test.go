...
func TestProjectIsNamespace(t *testing.T) {
	...
	namespace := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: "integration-test"}}
	...
	project = &projectapi.Project{ObjectMeta: metav1.ObjectMeta{Name: "new-project", Annotations: map[string]string{oapi.OpenShiftDisplayName: "Hello World", "openshift.io/node-selector": "env=test"}}}
...
func TestProjectWatch(t *testing.T) {
	...
	addBob := &policy.RoleModificationOptions{RoleBindingNamespace: "ns-02", RoleName: bootstrappolicy.EditRoleName, RoleKind: "ClusterRole", RbacClient: rbacv1client.NewForConfigOrDie(joeConfig), Users: []string{"bob"}, PrintFlags: genericclioptions.NewPrintFlags(""), ToPrinter: func(string) (printers.ResourcePrinter, error) {
...
func TestProjectWatchWithSelectionPredicate(t *testing.T) {
	...
	bobProjectClient := projectclient.NewForConfigOrDie(bobConfig).Project()
	w, err := bobProjectClient.Projects().Watch(metav1.ListOptions{FieldSelector: "metadata.name=ns-01"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if _, _, err := testserver.CreateNewProject(clusterAdminClientConfig, "ns-01", "bob"); err != nil {
	...
	waitForNoEvent(w, t)
	beginningWatch, err := bobProjectClient.Projects().Watch(metav1.ListOptions{ResourceVersion: "0", FieldSelector: "metadata.name=ns-01"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	waitForAdd("ns-01", beginningWatch, t)
	fromNowWatch, err := bobProjectClient.Projects().Watch(metav1.ListOptions{FieldSelector: "metadata.name=ns-01"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestScopedProjectAccess(t *testing.T) {
	...
	_, oneTwoBobConfig, err := testutil.GetScopedClientForUser(clusterAdminClientConfig, "bob", []string{scope.UserListScopedProjects, scope.ClusterRoleIndicator + "view:one", scope.ClusterRoleIndicator + "view:two"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	oneTwoBobClient := projectclient.NewForConfigOrDie(oneTwoBobConfig).Project()
	_, twoThreeBobConfig, err := testutil.GetScopedClientForUser(clusterAdminClientConfig, "bob", []string{scope.UserListScopedProjects, scope.ClusterRoleIndicator + "view:two", scope.ClusterRoleIndicator + "view:three"})
	twoThreeBobClient := projectclient.NewForConfigOrDie(twoThreeBobConfig).Project()
	_, allBobConfig, err := testutil.GetScopedClientForUser(clusterAdminClientConfig, "bob", []string{scope.UserListScopedProjects, scope.ClusterRoleIndicator + "view:*"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestInvalidRoleRefs(t *testing.T) {
	...
	modifyRole := &policy.RoleModificationOptions{RoleName: roleName, RoleKind: "ClusterRole", RbacClient: clusterAdminRbacClient, Users: []string{"someuser"}, PrintFlags: genericclioptions.NewPrintFlags(""), ToPrinter: func(string) (printers.ResourcePrinter, error) {
		return printers.NewDiscardingPrinter(), nil
	}}
