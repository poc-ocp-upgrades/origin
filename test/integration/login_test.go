...
func TestLogin(t *testing.T) {
	...
	newProjectOptions := &newproject.NewProjectOptions{ProjectClient: projectv1typedclient.NewForConfigOrDie(clusterAdminClientConfig), RbacClient: rbacClient, SARClient: authorizationInterface.SubjectAccessReviews(), ProjectName: project, AdminRole: bootstrappolicy.AdminRoleName, AdminUser: username, UseNodeSelector: false, IOStreams: genericclioptions.NewTestIOStreamsDiscard()}
...
func newLoginOptions(server string, username string, password string, insecure bool) *login.LoginOptions {
	...
	loginOptions := &login.LoginOptions{Server: server, StartingKubeConfig: &startingConfig, Username: username, Password: password, InsecureTLS: insecure, IOStreams: genericclioptions.NewTestIOStreamsDiscard()}
	return loginOptions
}
