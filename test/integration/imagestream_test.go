...
func TestImageStreamMappingCreate(t *testing.T) {
	...
	mapping := &imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: stream.Name}, Tag: "newer", Image: imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: "image1"}, DockerImageReference: "some/other/name"}}
	...
	image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: "image2"}, DockerImageMetadata: imageapi.DockerImage{Config: &imageapi.DockerConfig{Env: []string{"A=B"}}}}
	if _, err := clusterAdminImageClient.Images().Create(image); err == nil {
		t.Error("unexpected non-error")
	}
	image.DockerImageReference = "some/other/name"
	...
	mapping = &imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: stream.Name}, Tag: "newest", Image: *image}
	...
	mapping = &imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: stream.Name}, Tag: "anothertag", Image: *image}
	if _, err := clusterAdminImageClient.ImageStreamMappings(testutil.Namespace()).Create(mapping); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	updated, err = clusterAdminImageClient.ImageStreams(testutil.Namespace()).Get(stream.Name, metav1.GetOptions{})
...
func TestImageStreamWithoutDockerImageConfig(t *testing.T) {
	...
	imageConfig := imageapi.DockerConfig{Hostname: "example.com", Env: []string{"A=B"}}
	imageConfigBytes, err := json.Marshal(imageConfig)
	if err != nil {
		t.Fatalf("error marshaling image config: %s", err)
	}
	image := imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: imagetest.BaseImageWith1LayerDigest}, DockerImageMetadata: imageapi.DockerImage{Config: &imageapi.DockerConfig{Hostname: "example.com", Env: []string{"A=B"}}}, DockerImageConfig: string(imageConfigBytes), DockerImageReference: "some/namespace/name"}
	mapping := &imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: stream.Name}, Tag: "newer", Image: image}
	if _, err := clusterAdminImageClient.ImageStreamMappings(testutil.Namespace()).Create(mapping); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestImageStreamTagLifecycleHook(t *testing.T) {
	...
	err = exec.Execute(&appsv1.LifecycleHook{TagImages: []appsv1.TagImageHook{{ContainerName: "test", To: corev1.ObjectReference{Kind: "ImageStreamTag", Name: stream.Name + ":test"}}}}, &corev1.ReplicationController{ObjectMeta: metav1.ObjectMeta{Name: "rc-1", Namespace: testutil.Namespace()}, Spec: corev1.ReplicationControllerSpec{Template: &corev1.PodTemplateSpec{Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "test", Image: "someimage:other"}}}}}}, "test", "test")
	if err != nil {
		t.Fatal(err)
	}
	...
	exec = stratsupport.NewHookExecutor(coreClient, imageClientset.ImageV1(), os.Stdout)
	err = exec.Execute(&appsv1.LifecycleHook{TagImages: []appsv1.TagImageHook{{ContainerName: "test", To: corev1.ObjectReference{Kind: "ImageStreamTag", Name: "test2:test"}}}}, &corev1.ReplicationController{ObjectMeta: metav1.ObjectMeta{Name: "rc-1", Namespace: testutil.Namespace()}, Spec: corev1.ReplicationControllerSpec{Template: &corev1.PodTemplateSpec{Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "test", Image: "someimage:other"}}}}}}, "test", "test")
	if err != nil {
		t.Fatal(err)
	}
...
func TestRegistryWhitelistingValidation(t *testing.T) {
	...
	stream := mockImageStream()
	stream.Spec = imageapi.ImageStreamSpec{Tags: map[string]imageapi.TagReference{"latest": {Name: "latest", From: &kapi.ObjectReference{Kind: "DockerImage", Name: "my.test.registry/repo/sitory:latest"}}}}
	...
	stream.Spec.Tags["fail"] = imageapi.TagReference{Name: "fail", From: &kapi.ObjectReference{Kind: "DockerImage", Name: "this.will.fail/repo:tag"}}
	...
	stream.Spec.Tags["pass"] = imageapi.TagReference{Name: "pass", From: &kapi.ObjectReference{Kind: "DockerImage", Name: "127.0.0.1:5000/repo:tag"}}
	_, err = clusterAdminImageClient.ImageStreams(testutil.Namespace()).Update(stream)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	istag := &imageapi.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: stream.Name + ":new"}, Tag: &imageapi.TagReference{Name: "new", From: &kapi.ObjectReference{Kind: "DockerImage", Name: "my.insecure.registry/repo:new"}}}
	...
	istag.Tag.From = &kapi.ObjectReference{Kind: "DockerImage", Name: "example.com/repo:tag"}
	istag.ObjectMeta = metav1.ObjectMeta{Name: istag.Name, ResourceVersion: istag.ResourceVersion}
	...
	istag.Tag.From = &kapi.ObjectReference{Kind: "DockerImage", Name: "myupstream/repo:latest"}
	_, err = clusterAdminImageClient.ImageStreamTags(testutil.Namespace()).Update(istag)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
