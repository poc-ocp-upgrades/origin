...
func TestBootstrapPolicySelfSubjectAccessReviews(t *testing.T) {
	...
	subjectAccessReviewTest{description: "can I get a subjectaccessreview on myself even if I have no rights to do it generally", localInterface: valerieAuthorizationClient.LocalSubjectAccessReviews("openshift"), localReview: askCanICreatePolicyBindings, kubeAuthInterface: valerieKubeClient.AuthorizationV1(), response: authorizationapi.SubjectAccessReviewResponse{Allowed: false, Reason: ``, Namespace: "openshift"}}.run(t)
	askCanClusterAdminsCreateProject := &authorizationapi.LocalSubjectAccessReview{Groups: sets.NewString("system:cluster-admins"), Action: authorizationapi.Action{Verb: "create", Resource: "projects"}}
...
func TestSelfSubjectAccessReviewsNonExistingNamespace(t *testing.T) {
	...
	askCanICreatePodsInNonExistingNamespace := &authorizationapi.LocalSubjectAccessReview{Action: authorizationapi.Action{Namespace: "foo", Verb: "create", Resource: "pods"}}
	subjectAccessReviewTest{description: "ensure SAR for non-existing namespace does not leak namespace info", localInterface: authorizationclient.NewForConfigOrDie(valerieClientConfig).Authorization().LocalSubjectAccessReviews("foo"), localReview: askCanICreatePodsInNonExistingNamespace, kubeAuthInterface: valerieKubeClient.AuthorizationV1(), response: authorizationapi.SubjectAccessReviewResponse{Allowed: false, Reason: ``, Namespace: "foo"}}.run(t)
}
