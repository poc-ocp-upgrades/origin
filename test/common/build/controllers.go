...
func RunBuildControllerTest(t testingT, buildClient buildv1clienttyped.BuildsGetter, kClientset kubernetes.Interface, ns string) {
	...
	podWatch, err := kClientset.CoreV1().Pods(ns).Watch(metav1.ListOptions{FieldSelector: fields.OneTermEqualSelector("metadata.name", buildutil.GetBuildPodName(b)).String()})
	if err != nil {
		t.Fatal(err)
	}
	defer podWatch.Stop()
		...
		for _, state := range test.States {
			if err := retry.RetryOnConflict(retry.DefaultRetry, func() error {
				...
				if pod.Status.Phase == corev1.PodSucceeded {
					pod.Status.ContainerStatuses = []corev1.ContainerStatus{{Name: "container", State: corev1.ContainerState{Terminated: &corev1.ContainerStateTerminated{ExitCode: 0}}}}
				}
...
WaitLoop2:
	...
	if _, err := clusterAdminImageClient.ImageStreamMappings(ns).Create(&imagev1.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Namespace: ns, Name: imageStream.Name}, Tag: tag, Image: imagev1.Image{ObjectMeta: metav1.ObjectMeta{Name: "ref-2-random"}, DockerImageReference: registryHostname + "/openshift/test-image-trigger:ref-2-random"}}); err != nil {
...
func mockImageStream2(registryHostname, tag string) *imagev1.ImageStream {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &imagev1.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: "test-image-trigger-repo"}, Spec: imagev1.ImageStreamSpec{DockerImageRepository: registryHostname + "/openshift/test-image-trigger", Tags: []imagev1.TagReference{{Name: tag, From: &corev1.ObjectReference{Kind: "DockerImage", Name: registryHostname + "/openshift/test-image-trigger:" + tag}}}}}
}
