...
func tryEchoUDP(svc *kapiv1.Service) error {
	...
	o.Eventually(func() ([]byte, error) {
		return tryEchoUDPOnce(ip, udpPort, expectedBuff, readTimeout)
	}, 2*time.Minute, readTimeout).Should(o.Equal(expectedBuff))
	return nil
}
...
func checkSingleIdle(oc *exutil.CLI, idlingFile string, resources map[string][]string, resourceName string, kind string) {
	...
	o.Expect(err).ToNot(o.HaveOccurred())
	o.Expect(unidleTargets).To(o.Equal([]unidlingapi.RecordedScaleReference{{Replicas: 2, CrossGroupObjectReference: unidlingapi.CrossGroupObjectReference{Name: resources[resourceName][0], Kind: kind}}}))
}
	...
	g.Describe("unidling", func() {
		...
		g.It("should work with TCP (while idling) [local]", func() {
			...
			o.Consistently(func() error {
				return tryEchoTCP(svc)
			}, 10*time.Second, 500*time.Millisecond).ShouldNot(o.HaveOccurred())
			g.By("Waiting until we have endpoints")
			err = exutil.WaitForEndpointsAvailable(oc, serviceName)
			o.Expect(err).ToNot(o.HaveOccurred())
			endpoints, err := oc.KubeClient().CoreV1().Endpoints(oc.Namespace()).Get(serviceName, metav1.GetOptions{})
