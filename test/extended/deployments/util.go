...
func deploymentInfo(oc *exutil.CLI, name string) (*appsv1.DeploymentConfig, []*corev1.ReplicationController, []corev1.Pod, error) {
	...
	rcs, err := oc.KubeClient().CoreV1().ReplicationControllers(oc.Namespace()).List(metav1.ListOptions{LabelSelector: appsutil.ConfigSelector(name).String()})
	if err != nil {
		return nil, nil, nil, err
	}
...
func failureTrapForDetachedRCs(oc *exutil.CLI, dcName string, failed bool) {
	...
	dc, err := kclient.CoreV1().ReplicationControllers(oc.Namespace()).List(metav1.ListOptions{LabelSelector: labels.NewSelector().Add(*requirement).String()})
	if err != nil {
...
func HasValidDCControllerRef(dc metav1.Object, controllee metav1.Object) bool {
	...
	return ref != nil && ref.UID == dc.GetUID() && ref.APIVersion == deploymentConfigControllerRefKind.GroupVersion().String() && ref.Kind == deploymentConfigControllerRefKind.Kind && ref.Name == dc.GetName()
}
...
func (d *deployerPodInvariantChecker) UpdatePod(pod *corev1.Pod) {
	...
	o.Expect(oldPhaseIsTerminated && pod.Status.Phase != oldPhase).To(o.BeFalse(), spew.Sprintf("%v: detected deployer pod '%s/%s' transition from terminated phase: %q -> %q;\n"+"old: %#+v\nnew: %#+v\ndiff: %s", time.Now(), pod.Namespace, pod.Name, oldPhase, pod.Status.Phase, oldPod, pod, diff.ObjectReflectDiff(oldPod, pod)))
