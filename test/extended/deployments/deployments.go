...
var _ = g.Describe("[Feature:DeploymentConfig] deploymentconfigs", func() {
	...
	g.JustBeforeEach(func() {
		...
		dic.Start(ctx)
		dicMap[namespace] = dicEntry{dic: dic, ctx: ctx, cancel: cancel}
	})
	g.AfterEach(func() {
		namespace := oc.Namespace()
			...
			for i := 0; i < iterations; i++ {
				...
				case n < 0.7:
					...
					if out, err := oc.Run("rollout").Args("cancel", "dc/deployment-simple").Output(); err != nil {
						if !strings.Contains(out, "the object has been modified") && !strings.Contains(out, "there have been no replication controllers") && !strings.Contains(out, "there is a meaningful conflict") {
							o.Expect(err).NotTo(o.HaveOccurred())
						}
	...
	g.Describe("paused [Conformance]", func() {
		...
		g.It("should disable actions on deployments", func() {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			_, err = waitForDCModification(oc, dc.Namespace, dcName, deploymentChangeTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
	...
	g.Describe("with revision history limits [Conformance]", func() {
		...
		g.It("should never persist more old deployments than acceptable after being observed by the controller", func() {
			...
			iterations := 10
			for i := 0; i < iterations; i++ {
				o.Expect(waitForLatestCondition(oc, "history-limit", deploymentTimeout, deploymentReachedCompletion)).NotTo(o.HaveOccurred(), "the current deployment needs to have finished before attempting to trigger a new deployment through configuration change")
				e2e.Logf("%02d: triggering a new deployment with config change", i)
			...
			o.Expect(waitForSyncedConfig(oc, "history-limit", deploymentRunTimeout)).NotTo(o.HaveOccurred(), "the controller needs to have synced with the updated deployment configuration before checking that the revision history limits are being adhered to")
			var pollErr error
	...
	g.Describe("with minimum ready seconds set [Conformance]", func() {
		...
		g.It("should not transition the deployment to Complete before satisfied", func() {
			...
			o.Expect(dc.Name).To(o.Equal(dcName))
			rcName := func(i int) string {
				return fmt.Sprintf("%s-%d", dc.Name, i)
			}
			namespace := oc.Namespace()
			...
			rc1, err = waitForRCModification(oc, namespace, rc1.Name, deploymentRunTimeout, rc1.GetResourceVersion(), func(rc *corev1.ReplicationController) (bool, error) {
			...
			rc1, err = waitForRCModification(oc, namespace, rc1.Name, deploymentRunTimeout+time.Duration(dc.Spec.MinReadySeconds)*time.Second, rc1.GetResourceVersion(), func(rc *corev1.ReplicationController) (bool, error) {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(time.Since(startTime)).To(o.BeNumerically(">=", time.Duration(dc.Spec.MinReadySeconds)*time.Second), "Deployment shall not finish before MinReadySeconds elapse.")
	...
	g.Describe("", func() {
		...
		g.It("should adhere to Three Laws of Controllers [Conformance]", func() {
			namespace := oc.Namespace()
			rcName := func(i int) string {
				return fmt.Sprintf("%s-%d", dcName, i)
			}
			...
			g.By("releasing RCs that no longer match its selector", func() {
				...
				o.Expect(err).NotTo(o.HaveOccurred())
				rc1, err = waitForRCModification(oc, namespace, rcName(1), deploymentChangeTimeout, rc1.GetResourceVersion(), rCConditionFromMeta(controllerRefChangeCondition(metav1.GetControllerOf(rc1))))
				o.Expect(err).NotTo(o.HaveOccurred())
				controllerRef := metav1.GetControllerOf(rc1)
				o.Expect(controllerRef).To(o.BeNil())
				dc, err = waitForDCModification(oc, namespace, dcName, deploymentChangeTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
	...
	g.Describe("keep the deployer pod invariant valid [Conformance]", func() {
		...
		g.It("should deal with cancellation of running deployment", func() {
			...
			dc, err = waitForDCModification(oc, namespace, dcName, deploymentRunTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
			...
			rc, err := waitForRCModification(oc, namespace, appsutil.LatestDeploymentNameForConfigAndVersion(dc.Name, dc.Status.LatestVersion), deploymentRunTimeout, "", func(currentRC *corev1.ReplicationController) (bool, error) {
			...
			rc, err = oc.KubeClient().CoreV1().ReplicationControllers(namespace).Patch(appsutil.LatestDeploymentNameForConfigAndVersion(dc.Name, dc.Status.LatestVersion), types.StrategicMergePatchType, []byte(fmt.Sprintf(`{"metadata":{"annotations":{%q: %q, %q: %q}}}`, deploymentCancelledAnnotation, "true", appsv1.DeploymentStatusReasonAnnotation, "cancelled by the user")))
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			dc, err = oc.AppsClient().AppsV1().DeploymentConfigs(dc.Namespace).Patch(dc.Name, types.StrategicMergePatchType, []byte(`{"spec":{"template":{"metadata":{"annotations":{"foo": "bar"}}}}}`))
			o.Expect(err).NotTo(o.HaveOccurred())
			dc, err = waitForDCModification(oc, namespace, dcName, deploymentRunTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
		...
		g.It("should deal with config change in case the deployment is still running", func() {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("waiting for RC to be created")
			dc, err = waitForDCModification(oc, namespace, dc.Name, deploymentRunTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
				cond := appsutil.GetDeploymentCondition(config.Status, appsv1.DeploymentProgressing)
				if cond != nil && cond.Reason == appsutil.NewReplicationControllerReason {
					return true, nil
				}
			...
			o.Expect(dc.Status.LatestVersion).To(o.BeEquivalentTo(1))
			g.By("waiting for deployer pod to be running")
			_, err = waitForRCModification(oc, namespace, appsutil.LatestDeploymentNameForConfigAndVersion(dc.Name, dc.Status.LatestVersion), deploymentRunTimeout, "", func(currentRC *corev1.ReplicationController) (bool, error) {
				if appsutil.DeploymentStatusFor(currentRC) == appsv1.DeploymentStatusRunning {
					return true, nil
				}
				return false, nil
		...
		g.It("should deal with cancellation after deployer pod succeeded", func() {
			...
			_, err = waitForPodModification(oc, namespace, appsutil.DeployerPodNameForDeployment(rcName), deploymentRunTimeout, "", func(pod *corev1.Pod) (bool, error) {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("canceling the deployment")
			rc, err := oc.KubeClient().CoreV1().ReplicationControllers(namespace).Patch(rcName, types.StrategicMergePatchType, []byte(fmt.Sprintf(`{"metadata":{"annotations":{%q: %q, %q: %q}}}`, deploymentCancelledAnnotation, "true", deploymentStatusReasonAnnotation, "cancelled by the user")))
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			_, err = waitForRCModification(oc, namespace, appsutil.LatestDeploymentNameForConfigAndVersion(dc.Name, dc.Status.LatestVersion), deploymentRunTimeout, rc.ResourceVersion, func(currentRC *corev1.ReplicationController) (bool, error) {
	...
	g.Describe("won't deploy RC with unresolved images [Conformance]", func() {
		...
		g.It("when patched with empty image", func() {
			...
			_, err = waitForRCModification(oc, namespace, rcName(1), deploymentRunTimeout, rcList.ResourceVersion, func(currentRC *corev1.ReplicationController) (bool, error) {
				switch appsutil.DeploymentStatusFor(currentRC) {
			...
			for i := 0; i < 50; i++ {
				dc, err = oc.AppsClient().AppsV1().DeploymentConfigs(namespace).Patch(dc.Name, types.StrategicMergePatchType, []byte(`{"spec":{"template":{"spec":{"containers":[{"name":"test","image":""}]}}}}`))
				o.Expect(err).NotTo(o.HaveOccurred())
			}
			g.By("waiting to see if it won't deploy RC with invalid revision or the same one multiple times")
			dc, err = waitForDCModification(oc, namespace, dc.Name, deploymentChangeTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			dcTmp, err := waitForDCModification(oc, namespace, dc.Name, deploymentChangeTimeout, dc.GetResourceVersion(), func(config *appsv1.DeploymentConfig) (bool, error) {
				if config.Status.ObservedGeneration >= dc.Generation {
					return true, nil
				}
				return false, nil
			...
			dc = dcTmp
			rcs, err := oc.KubeClient().CoreV1().ReplicationControllers(namespace).List(metav1.ListOptions{LabelSelector: appsutil.ConfigSelector(dc.Name).String()})
