...
var _ = g.Describe("[Conformance][Area:Networking][Feature:Router]", func() {
	...
	g.Describe("The HAProxy router", func() {
		g.It("should run even if it has no access to update status", func() {
			...
			err := oc.AsAdmin().Run("new-app").Args("-f", configPath, `-p=IMAGE=`+routerImage, `-p=SCOPE=["--name=test-unprivileged", "--namespace=$(POD_NAMESPACE)", "--loglevel=4", "--labels=select=first", "--update-status=false"]`).Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
