...
var _ = Describe("[Area:Networking] multicast", func() {
	InPluginContext([]string{network.SingleTenantPluginName}, func() {
	...
	InPluginContext([]string{network.MultiTenantPluginName, network.NetworkPolicyPluginName}, func() {
		oc := testexutil.NewCLI("multicast", testexutil.KubeConfigPath())
		f := oc.KubeFramework()
...
func launchTestMulticastPod(f *e2e.Framework, nodeName string, podName string) (string, error) {
	...
	pod := &kapiv1.Pod{TypeMeta: metav1.TypeMeta{Kind: "Pod"}, ObjectMeta: metav1.ObjectMeta{Name: podName}, Spec: kapiv1.PodSpec{Containers: []kapiv1.Container{{Name: contName, Image: "openshift/test-multicast", Command: []string{"sleep", "1000"}}}, NodeName: nodeName, RestartPolicy: kapiv1.RestartPolicyNever}}
