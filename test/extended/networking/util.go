...
func waitForPodCondition(c kclientset.Interface, ns, podName, desc string, timeout time.Duration, condition podCondition) error {
	...
	for start := time.Now(); time.Since(start) < timeout; time.Sleep(poll) {
		...
		if err != nil {
			e2e.Logf("Get pod %-[1]*[2]s in namespace '%[3]s' failed, ignoring for %[4]v. Error: %[5]v", podPrintWidth, podName, ns, poll, err)
			continue
		}
		...
		e2e.Logf("Waiting for pod %-[1]*[2]s in namespace '%[3]s' status to be '%[4]s'"+"(found phase: %[5]q, readiness: %[6]t) (%[7]v elapsed)", podPrintWidth, podName, ns, desc, pod.Status.Phase, podReady(pod), time.Since(start))
	}
...
func launchWebserverService(f *e2e.Framework, serviceName string, nodeName string) (serviceAddr string) {
	...
	expectNoError(err)
	servicePort := 8080
	service := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: serviceName}, Spec: corev1.ServiceSpec{Type: corev1.ServiceTypeClusterIP, Ports: []corev1.ServicePort{{Protocol: corev1.ProtocolTCP, Port: int32(servicePort)}}, Selector: map[string]string{"name": "web"}}}
...
func checkConnectivityToHost(f *e2e.Framework, nodeName string, podName string, host string, timeout time.Duration) error {
	...
	debugPodName := e2e.CreateExecPodOrFail(f.ClientSet, f.Namespace.Name, fmt.Sprintf("debugpod-sourceip-%s", nodeName), func(pod *corev1.Pod) {
		...
		privileged := true
		pod.Spec.Volumes = []corev1.Volume{{Name: "ovs-socket", VolumeSource: corev1.VolumeSource{HostPath: &corev1.HostPathVolumeSource{Path: "/var/run/openvswitch/br0.mgmt"}}}}
		pod.Spec.Containers[0].VolumeMounts = []corev1.VolumeMount{{Name: "ovs-socket", MountPath: "/var/run/openvswitch/br0.mgmt"}}
...
func InPluginContext(plugins []string, body func()) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	Context(fmt.Sprintf("when using one of the plugins '%s'", strings.Join(plugins, ", ")), func() {
		BeforeEach(func() {
			found := false
