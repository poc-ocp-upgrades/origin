...
var _ = g.Describe("[cli][Slow] can use rsync to upload files to pods", func() {
	...
	g.Describe("using a watch", func() {
		g.It("should watch for changes and rsync them", func() {
			...
			cmd, stdout, stderr, err := oc.Run("rsync").Args(fmt.Sprintf("%s/", tempDir), fmt.Sprintf("%s:/tmp/rsync", podName), "--loglevel=5", "--delete", "--watch").Background()
			failed := true
	...
	g.Describe("copy by strategy", func() {
		testRsyncFn := func(strategy string) func() {
			return func() {
				...
				err := oc.Run("rsync").Args(sourcePath1, fmt.Sprintf("%s:/tmp", podName), fmt.Sprintf("--strategy=%s", strategy)).Execute()
				o.Expect(err).NotTo(o.HaveOccurred())
				g.By("Verifying that files are copied to the container")
				...
				err = oc.Run("rsync").Args(sourcePath2+"/", fmt.Sprintf("%s:/tmp/image-streams", podName), fmt.Sprintf("--strategy=%s", strategy), "--delete").Execute()
				o.Expect(err).NotTo(o.HaveOccurred())
				...
				err = oc.Run("rsync").Args(fmt.Sprintf("%s:/tmp/image-streams/", podName), tempDir, fmt.Sprintf("--strategy=%s", strategy)).Execute()
				...
				o.Expect(err).NotTo(o.HaveOccurred())
				err = oc.Run("rsync").Args(fmt.Sprintf("%s:/tmp/image-streams/", podName), tempDir, "--delete", fmt.Sprintf("--strategy=%s", strategy)).Execute()
				...
				result, err = oc.Run("rsync").Args(sourcePath1, fmt.Sprintf("%s:/", podName), fmt.Sprintf("--strategy=%s", strategy)).Output()
	...
	g.Describe("rsync specific flags", func() {
		g.It("should honor the --exclude flag", func() {
			...
			err := oc.Run("rsync").Args(sourcePath1, fmt.Sprintf("%s:/tmp", podName), "--exclude=image-streams-rhel7.json").Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("Verifying that files are copied to the container")
			result, err := oc.Run("rsh").Args(podName, "ls", "/tmp/image-streams").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("should honor multiple --exclude flags", func() {
			...
			err := oc.Run("rsync").Args(sourcePath2, fmt.Sprintf("%s:/tmp", podName), "--exclude=application-template-pullspecbuild.json", "--exclude=application-template-dockerbuild.json").Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("Verifying that files are copied to the container")
		...
		g.It("should honor the --include flag", func() {
			...
			err := oc.Run("rsync").Args(sourcePath1, fmt.Sprintf("%s:/tmp", podName), "--exclude=*.json", "--include=image-streams-rhel7.json").Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("Verifying that files are copied to the container")
			result, err := oc.Run("rsh").Args(podName, "ls", "/tmp/image-streams").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("should honor multiple --include flags", func() {
			...
			err := oc.Run("rsync").Args(sourcePath2, fmt.Sprintf("%s:/tmp", podName), "--exclude=*.json", "--include=application-template-stibuild.json", "--include=application-template-dockerbuild.json").Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("Verifying that files are copied to the container")
			result, err := oc.Run("rsh").Args(podName, "ls", "/tmp/sample-app").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("should honor the --progress flag", func() {
			...
			result, err := oc.Run("rsync").Args(sourcePath1, fmt.Sprintf("%s:/tmp", podName), "--progress").Output()
			o.Expect(err).NotTo(o.HaveOccurred())
		...
		g.It("should honor the --no-perms flag", func() {
			...
			err = oc.Run("rsync").Args(fmt.Sprintf("%s:/var/lib/mysql", podName), tempDir).Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			err = oc.Run("rsync").Args(fmt.Sprintf("%s/", localmariadbDir), fmt.Sprintf("%s:/var/lib/mysql", podName)).Execute()
			o.Expect(err).To(o.HaveOccurred())
			...
			err = oc.Run("rsync").Args(fmt.Sprintf("%s/", localmariadbDir), fmt.Sprintf("%s:/var/lib/mysql", podName), "--no-perms").Execute()
			o.Expect(err).NotTo(o.HaveOccurred())
		})
	})
})
