...
func listMachines(dc dynamic.Interface, labelSelector string) ([]objx.Map, error) {
	...
	defer _logClusterCodePath()
	mc := machineClient(dc)
	obj, err := mc.List(metav1.ListOptions{LabelSelector: labelSelector})
	if err != nil {
		return nil, err
	}
...
var _ = g.Describe("[Feature:Machines][Disruptive] Managed cluster should", func() {
	...
	g.It("recover from deleted worker machines", func() {
		...
		workerNodes, err := c.CoreV1().Nodes().List(metav1.ListOptions{LabelSelector: nodeLabelSelectorWorker})
		o.Expect(err).NotTo(o.HaveOccurred())
