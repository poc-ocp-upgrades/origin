	...
	g.Context("", func() {
		...
		g.Describe("creating from a template", func() {
			g.It(fmt.Sprintf("should instantiate the template"), func() {
				...
				podNames, err := exutil.WaitForPods(oc.KubeClient().CoreV1().Pods(oc.Namespace()), exutil.ParseLabelsOrDie("name=mongodb"), exutil.CheckPodIsRunning, 1, 4*time.Minute)
				o.Expect(err).ShouldNot(o.HaveOccurred())
				...
				findCmd := "printjson(db.foo.find({}, {_id: 0}).toArray())"
