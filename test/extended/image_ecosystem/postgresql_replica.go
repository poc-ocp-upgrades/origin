...
var (
	...
	postgreSQLImages		= []string{"postgresql:9.6"}
)
...
func PostgreSQLReplicationTestFactory(oc *exutil.CLI, image string, cleanup func()) func() {
	...
	return func() {
		...
		assertReplicationIsWorking := func(masterDeployment, slaveDeployment string, slaveCount int) (exutil.Database, []exutil.Database, exutil.Database) {
			...
			check(err)
			err = exutil.WaitForQueryOutputContains(oc, master, 10*time.Second, false, fmt.Sprintf("SELECT * FROM %s;", table), "col1 | val1\ncol2 | val2")
			check(err)
			for _, slave := range slaves {
				err = exutil.WaitForQueryOutputContains(oc, slave, 90*time.Second, false, fmt.Sprintf("SELECT * FROM %s;", table), "col1 | val1\ncol2 | val2")
				check(err)
			}
			return master, slaves, helper
		}
