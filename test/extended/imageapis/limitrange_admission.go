...
var _ = g.Describe("[Feature:ImageQuota][registry][Serial][Suite:openshift/registry/serial] Image limit range", func() {
	...
	g.It(fmt.Sprintf("should deny a push of built image exceeding %s limit", imageapi.LimitTypeImage), func() {
		_, err := createLimitRangeOfType(oc, imageapi.LimitTypeImage, kapi.ResourceList{kapi.ResourceStorage: resource.MustParse("10Ki")})
	...
	g.It(fmt.Sprintf("should deny a push of built image exceeding limit on %s resource", imageapi.ResourceImageStreamImages), func() {
		limits := kapi.ResourceList{imageapi.ResourceImageStreamTags: resource.MustParse("0"), imageapi.ResourceImageStreamImages: resource.MustParse("0")}
	...
	g.It(fmt.Sprintf("should deny a docker image reference exceeding limit on %s resource", imageapi.ResourceImageStreamTags), func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		is.Spec.Tags["foo"] = imageapi.TagReference{Name: "foo", From: &kapi.ObjectReference{Kind: "DockerImage", Name: tag2Image["tag2"].DockerImageReference}, ImportPolicy: imageapi.TagImportPolicy{Insecure: true}}
		...
		is, err = oc.ImageClient().Image().ImageStreams(oc.Namespace()).Get("stream", metav1.GetOptions{})
		o.Expect(err).NotTo(o.HaveOccurred())
		is.Spec.Tags["duplicate"] = imageapi.TagReference{Name: "duplicate", From: &kapi.ObjectReference{Kind: "DockerImage", Name: tag2Image["tag1"].DockerImageReference}, ImportPolicy: imageapi.TagImportPolicy{Insecure: true}}
		_, err = oc.ImageClient().Image().ImageStreams(oc.Namespace()).Update(is)
		o.Expect(err).NotTo(o.HaveOccurred())
	})
	g.It(fmt.Sprintf("should deny an import of a repository exceeding limit on %s resource", imageapi.ResourceImageStreamTags), func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		limit := kapi.ResourceList{imageapi.ResourceImageStreamTags: *resource.NewQuantity(int64(maxBulkImport)+1, resource.DecimalSI), imageapi.ResourceImageStreamImages: *resource.NewQuantity(int64(maxBulkImport)+1, resource.DecimalSI)}
		_, err = createLimitRangeOfType(oc, imageapi.LimitTypeImageStream, limit)
		o.Expect(err).NotTo(o.HaveOccurred())
...
func createLimitRangeOfType(oc *exutil.CLI, limitType kapi.LimitType, maxLimits kapi.ResourceList) (*kapi.LimitRange, error) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	lr := &kapi.LimitRange{ObjectMeta: metav1.ObjectMeta{Name: limitRangeName}, Spec: kapi.LimitRangeSpec{Limits: []kapi.LimitRangeItem{{Type: limitType, Max: maxLimits}}}}
