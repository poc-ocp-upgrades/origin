...
var _ = g.Describe("[Feature:ImageQuota][registry] Image resource quota", func() {
	...
	g.It(fmt.Sprintf("should deny a push of built image exceeding %s quota", imagev1.ResourceImageStreams), func() {
		...
		quota := corev1.ResourceList{imagev1.ResourceImageStreams: resource.MustParse("0")}
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		used, err = exutil.WaitForResourceQuotaSync(oc.KubeClient().CoreV1().ResourceQuotas(oc.Namespace()), quotaName, corev1.ResourceList{imagev1.ResourceImageStreams: resource.MustParse("1")}, true, waitTimeout)
		o.Expect(err).NotTo(o.HaveOccurred())
...
func waitForResourceQuotaSync(oc *exutil.CLI, name string, expectedResources corev1.ResourceList) (corev1.ResourceList, error) {
	...
	used, err := exutil.WaitForResourceQuotaSync(oc.KubeClient().CoreV1().ResourceQuotas(oc.Namespace()), quotaName, expectedResources, false, waitTimeout)
	if err != nil {
		return nil, err
	}
	return used, nil
...
func createImageStreamMapping(oc *exutil.CLI, namespace, name, tag string) error {
	...
	if kerrors.IsNotFound(err) {
		_, err = oc.AdminImageClient().ImageV1().ImageStreams(namespace).Create(&imagev1.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace}})
		if err != nil {
			return err
		}
	} else if err != nil {
	...
	_, err = oc.AdminImageClient().ImageV1().ImageStreamMappings(namespace).Create(&imagev1.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: name, Namespace: namespace}, Image: imagev1.Image{ObjectMeta: metav1.ObjectMeta{Name: "sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855"}}, Tag: tag})
	return err
}
