...
var _ = g.Describe("[Slow]jenkins repos e2e openshift using slow samples pipeline build", func() {
	...
	g.Context("Sync plugin tests", func() {
		g.It("using the persistent template", func() {
			...
			_, err := oc.AdminKubeClient().CoreV1().LimitRanges(oc.Namespace()).Create(&corev1.LimitRange{ObjectMeta: metav1.ObjectMeta{Name: "limitrange"}, Spec: corev1.LimitRangeSpec{Limits: []corev1.LimitRangeItem{{Type: corev1.LimitTypeContainer, Default: corev1.ResourceList{corev1.ResourceMemory: resource.MustParse("512Mi")}}}}})
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			g.By("Blue-green pipeline should build and complete successfully", func() {
				...
				buildAndSwitch := func(newColour string) {
					...
					go func() {
						...
						jenkinsBuildURI = strings.Trim(url.Path, "/")
