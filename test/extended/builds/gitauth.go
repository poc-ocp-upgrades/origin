...
var _ = g.Describe("[Feature:Builds][Slow] can use private repositories as build input", func() {
	...
	g.Context("", func() {
		...
		testGitAuth := func(routeName, gitServerYaml, urlTemplate string, secretFunc func() string) {
			...
			g.By(fmt.Sprintf("creating a new BuildConfig by calling oc new-app -f %q -p SOURCE_SECRET=%s -p SOURCE_URL=%s", testBuildFixture, sourceSecretName, sourceURL))
		...
		g.Describe("Build using a username and password", func() {
			g.It("should create a new build using the internal gitserver", func() {
				...
				testGitAuth("gitserver", gitServerFixture, sourceURLTemplate, func() string {
					g.By(fmt.Sprintf("creating a new secret for the gitserver by calling oc secrets new-basicauth %s --username=%s --password=%s", sourceSecretName, gitUserName, gitPassword))
					...
					for _, s := range sa.Secrets {
						if strings.Contains(s.Name, "token") {
							...
							o.Expect(err).NotTo(o.HaveOccurred())
							err = oc.Run("create").Args("secret", "generic", sourceSecretName, "--type", "kubernetes.io/basic-auth", "--from-literal", fmt.Sprintf("username=%s", gitUserName), "--from-literal", fmt.Sprintf("password=%s", gitPassword), "--from-literal", fmt.Sprintf("ca.crt=%s", string(secret.Data["service-ca.crt"]))).Execute()
							o.Expect(err).NotTo(o.HaveOccurred())
