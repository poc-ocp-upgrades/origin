...
`
	...
	g.Context("", func() {
		...
		g.It("should succeed [Conformance]", func() {
			...
			build, err := oc.AdminBuildClient().BuildV1().Builds(oc.Namespace()).Create(&buildv1.Build{ObjectMeta: metav1.ObjectMeta{Name: "optimized"}, Spec: buildv1.BuildSpec{CommonSpec: buildv1.CommonSpec{Source: buildv1.BuildSource{Dockerfile: &testDockerfile}, Strategy: buildv1.BuildStrategy{DockerStrategy: &buildv1.DockerBuildStrategy{ImageOptimizationPolicy: &skipLayers}}}}})
