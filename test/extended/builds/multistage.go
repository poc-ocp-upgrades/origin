...
`
	...
	g.Context("", func() {
		...
		g.It("should succeed [Conformance]", func() {
			...
			build, err := oc.BuildClient().BuildV1().Builds(oc.Namespace()).Create(&buildv1.Build{ObjectMeta: metav1.ObjectMeta{Name: "multi-stage"}, Spec: buildv1.BuildSpec{CommonSpec: buildv1.CommonSpec{Source: buildv1.BuildSource{Dockerfile: &testDockerfile, Images: []buildv1.ImageSource{{From: corev1.ObjectReference{Kind: "DockerImage", Name: "centos:7"}, As: []string{"scratch"}}}}, Strategy: buildv1.BuildStrategy{DockerStrategy: &buildv1.DockerBuildStrategy{}}, Output: buildv1.BuildOutput{To: &corev1.ObjectReference{Kind: "DockerImage", Name: fmt.Sprintf("%s/%s/multi-stage:v1", registry, oc.Namespace())}}}}})
			o.Expect(err).NotTo(o.HaveOccurred())
			...
			pod = c.Create(&corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: "test"}, Spec: corev1.PodSpec{RestartPolicy: corev1.RestartPolicyNever, Containers: []corev1.Container{{Name: "run", Image: fmt.Sprintf("%s/%s/multi-stage:v1", registry, oc.Namespace()), Command: []string{"/test/curl", "-k", "https://kubernetes.default.svc"}}}}})
