...
var _ = g.Describe("[Feature:Prometheus][Feature:Builds] Prometheus", func() {
	...
	g.Describe("when installed on the cluster", func() {
		g.It("should start and expose a secured proxy and verify build metrics", func() {
			...
			terminalTests := map[string][]metricTest{buildCountQuery: {metricTest{labels: map[string]string{"phase": string(buildv1.BuildPhaseComplete)}, greaterThanEqual: true}}}
