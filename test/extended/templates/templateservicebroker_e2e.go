...
var _ = g.Describe("[Conformance][templates] templateservicebroker end-to-end test", func() {
	...
	g.JustBeforeEach(func() {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		privatetemplate, err = cli.TemplateClient().TemplateV1().Templates(cli.Namespace()).Create(&templatev1.Template{ObjectMeta: metav1.ObjectMeta{Name: "private"}})
		o.Expect(err).NotTo(o.HaveOccurred())
		clusterrolebinding, err = cli.AdminAuthorizationClient().Authorization().ClusterRoleBindings().Create(&authorizationapi.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: cli.Namespace() + "templateservicebroker-client"}, RoleRef: kapi.ObjectReference{Name: bootstrappolicy.TemplateServiceBrokerClientRoleName}, Subjects: []kapi.ObjectReference{{Kind: authorizationapi.GroupKind, Name: bootstrappolicy.UnauthenticatedGroup}}})
		o.Expect(err).NotTo(o.HaveOccurred())
	})
	g.AfterEach(func() {
	...
	provision := func() {
		...
		_, err := brokercli.Provision(context.Background(), cliUser, instanceID, &api.ProvisionRequest{ServiceID: string(privatetemplate.UID), PlanID: plan.ID, Context: api.KubernetesContext{Platform: api.ContextPlatformKubernetes, Namespace: cli.Namespace()}})
		o.Expect(err).To(o.HaveOccurred())
		o.Expect(err.Error()).To(o.ContainSubstring("not found"))
		ctx, cancel := context.WithTimeout(context.Background(), 20*time.Minute)
		defer cancel()
		_, err = brokercli.Provision(ctx, cliUser, instanceID, &api.ProvisionRequest{ServiceID: service.ID, PlanID: plan.ID, Context: api.KubernetesContext{Platform: api.ContextPlatformKubernetes, Namespace: cli.Namespace()}, Parameters: map[string]string{"MYSQL_USER": "test"}})
		if err != nil {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		o.Expect(brokerTemplateInstance.Spec).To(o.Equal(templatev1.BrokerTemplateInstanceSpec{TemplateInstance: corev1.ObjectReference{Kind: "TemplateInstance", Namespace: cli.Namespace(), Name: templateInstance.Name, UID: templateInstance.UID}, Secret: corev1.ObjectReference{Kind: "Secret", Namespace: cli.Namespace(), Name: secret.Name, UID: secret.UID}}))
		blockOwnerDeletion := true
		o.Expect(templateInstance.Annotations).To(o.Equal(map[string]string{api.OpenServiceBrokerInstanceExternalID: templateInstance.Name}))
		o.Expect(templateInstance.OwnerReferences).To(o.ContainElement(metav1.OwnerReference{APIVersion: templatev1.SchemeGroupVersion.String(), Kind: "BrokerTemplateInstance", Name: brokerTemplateInstance.Name, UID: brokerTemplateInstance.UID, BlockOwnerDeletion: &blockOwnerDeletion}))
		o.Expect(templateInstance.Spec).To(o.Equal(templatev1.TemplateInstanceSpec{Template: *template, Secret: &corev1.LocalObjectReference{Name: secret.Name}, Requester: &templatev1.TemplateInstanceRequester{Username: cli.Username(), Groups: []string{"system:authenticated"}}}))
		...
		o.Expect(secret.OwnerReferences).To(o.ContainElement(metav1.OwnerReference{APIVersion: templatev1.SchemeGroupVersion.String(), Kind: "BrokerTemplateInstance", Name: brokerTemplateInstance.Name, UID: brokerTemplateInstance.UID, BlockOwnerDeletion: &blockOwnerDeletion}))
		o.Expect(secret.Type).To(o.Equal(corev1.SecretTypeOpaque))
		o.Expect(secret.Data).To(o.Equal(map[string][]byte{"MYSQL_USER": []byte("test")}))
	...
	bind := func() {
		...
		bind, err := brokercli.Bind(context.Background(), cliUser, instanceID, bindingID, &api.BindRequest{ServiceID: service.ID, PlanID: plan.ID})
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	deprovision := func() {
		...
		err = wait.Poll(time.Second*1, 5*time.Minute, func() (bool, error) {
			for gvk := range legacyscheme.Scheme.AllKnownTypes() {
				...
				case kapi.Kind("Event"), kapi.Kind("ServiceAccount"), kapi.Kind("Secret"), kapi.Kind("RoleBinding"), rbacapi.Kind("RoleBinding"), legacy.Kind("RoleBinding"), authorization.Kind("RoleBinding"), schema.GroupKind{Group: "events.k8s.io", Kind: "Event"}:
					continue
				}
