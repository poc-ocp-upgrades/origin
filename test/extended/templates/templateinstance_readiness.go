...
var _ = g.Describe("[Conformance][templates] templateinstance readiness test", func() {
	...
	waitSettle := func() (bool, error) {
		...
		case buildv1.BuildPhaseComplete:
			...
			if (progressing != nil && progressing.Status == corev1.ConditionTrue && progressing.Reason == appsutil.NewRcAvailableReason && available != nil && available.Status == corev1.ConditionTrue) || (progressing != nil && progressing.Status == corev1.ConditionFalse) {
				return true, nil
			}
		}
	...
	g.Context("", func() {
		...
		g.It("should report ready soon after all annotated objects are ready", func() {
			var err error
			templateinstance = &templatev1.TemplateInstance{ObjectMeta: metav1.ObjectMeta{Name: "templateinstance"}, Spec: templatev1.TemplateInstanceSpec{Template: *template}}
		...
		g.It("should report failed soon after an annotated objects has failed", func() {
			var err error
			secret, err := cli.KubeClient().CoreV1().Secrets(cli.Namespace()).Create(&v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret"}, Data: map[string][]byte{"SOURCE_REPOSITORY_URL": []byte("https://bad")}})
			o.Expect(err).NotTo(o.HaveOccurred())
			templateinstance = &templatev1.TemplateInstance{ObjectMeta: metav1.ObjectMeta{Name: "templateinstance"}, Spec: templatev1.TemplateInstanceSpec{Template: *template, Secret: &corev1.LocalObjectReference{Name: secret.Name}}}
			g.By("instantiating the templateinstance")
			templateinstance, err = cli.TemplateClient().TemplateV1().TemplateInstances(cli.Namespace()).Create(templateinstance)
			o.Expect(err).NotTo(o.HaveOccurred())
			g.By("waiting for build and dc to settle")
