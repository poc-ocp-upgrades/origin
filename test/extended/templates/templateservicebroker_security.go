...
var _ = g.Describe("[Conformance][templates] templateservicebroker security test", func() {
	...
	provision := func(username string) error {
		...
		defer cancel()
		_, err := brokercli.Provision(ctx, &user.DefaultInfo{Name: username, Groups: []string{"system:authenticated"}}, instanceID, &api.ProvisionRequest{ServiceID: service.ID, PlanID: plan.ID, Context: api.KubernetesContext{Platform: api.ContextPlatformKubernetes, Namespace: cli.Namespace()}})
		if err != nil {
	...
	bind := func(username string) error {
		...
		_, err := brokercli.Bind(context.Background(), &user.DefaultInfo{Name: username, Groups: []string{"system:authenticated"}}, instanceID, bindingID, &api.BindRequest{ServiceID: service.ID, PlanID: plan.ID})
		return err
	}
	...
	g.Context("", func() {
		...
		g.It("should pass security tests", func() {
			...
			o.Expect(err).NotTo(o.HaveOccurred())
			o.Expect(brokerTemplateInstance.Spec.BindingIDs).To(o.HaveLen(0))
			g.By("having view permissions to the namespace, bind should fail with 403")
