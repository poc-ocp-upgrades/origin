...
func createUser(cli *exutil.CLI, name, role string) *userapi.User {
	...
	user, err := cli.AdminUserClient().User().Users().Create(&userapi.User{ObjectMeta: metav1.ObjectMeta{Name: name}})
	o.Expect(err).NotTo(o.HaveOccurred())
	if role != "" {
		_, err = cli.AdminAuthorizationClient().Authorization().RoleBindings(cli.Namespace()).Create(&authorizationapi.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("%s-%s-binding", name, role)}, RoleRef: kapi.ObjectReference{Name: role}, Subjects: []kapi.ObjectReference{{Kind: authorizationapi.UserKind, Name: name}}})
		o.Expect(err).NotTo(o.HaveOccurred())
	}
	return user
}
func createGroup(cli *exutil.CLI, name, role string) *userapi.Group {
	...
	defer _logClusterCodePath()
	name = cli.Namespace() + "-" + name
	group, err := cli.AdminUserClient().User().Groups().Create(&userapi.Group{ObjectMeta: metav1.ObjectMeta{Name: name}})
	o.Expect(err).NotTo(o.HaveOccurred())
	if role != "" {
		_, err = cli.AdminAuthorizationClient().Authorization().RoleBindings(cli.Namespace()).Create(&authorizationapi.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("%s-%s-binding", name, role)}, RoleRef: kapi.ObjectReference{Name: role}, Subjects: []kapi.ObjectReference{{Kind: authorizationapi.GroupKind, Name: name}}})
		o.Expect(err).NotTo(o.HaveOccurred())
	}
	return group
}
...
func TSBClient(oc *exutil.CLI) (osbclient.Client, error) {
	...
	return osbclient.NewClient(&http.Client{Transport: &http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}}}, "https://"+svc.Spec.ClusterIP+templateapi.ServiceBrokerRoot), nil
}
