...
var _ = g.Describe("[Conformance][templates] templateinstance impersonation tests", func() {
	...
	var (
		...
		adminuser		*userapi.User
		impersonateuser		*userapi.User
		impersonatebygroupuser	*userapi.User
		impersonategroup	*userapi.Group
		edituser1		*userapi.User
		edituser2		*userapi.User
		viewuser		*userapi.User
		dummytemplateinstance	*templateapi.TemplateInstance
		dummycondition		= templateapi.TemplateInstanceCondition{Type: templateapi.TemplateInstanceConditionType("dummy"), Status: kapi.ConditionTrue}
		tests			[]struct {
			user				*userapi.User
	...
	g.BeforeEach(func() {
		...
		role, err := cli.AdminAuthorizationClient().Authorization().Roles(cli.Namespace()).Create(&authorizationapi.Role{ObjectMeta: metav1.ObjectMeta{Name: "impersonater"}, Rules: []authorizationapi.PolicyRule{{Verbs: sets.NewString("assign"), APIGroups: []string{templateapi.GroupName}, Resources: sets.NewString("templateinstances")}}})
		o.Expect(err).NotTo(o.HaveOccurred())
		_, err = cli.AdminAuthorizationClient().Authorization().RoleBindings(cli.Namespace()).Create(&authorizationapi.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "impersonater-binding"}, RoleRef: kapi.ObjectReference{Name: role.Name, Namespace: cli.Namespace()}, Subjects: []kapi.ObjectReference{{Kind: authorizationapi.UserKind, Name: impersonateuser.Name}, {Kind: authorizationapi.GroupKind, Name: impersonategroup.Name}}})
		o.Expect(err).NotTo(o.HaveOccurred())
		err = wait.PollImmediate(time.Second, 30*time.Second, func() (done bool, err error) {
			...
			sar, err := cli.AuthorizationClient().Authorization().LocalSubjectAccessReviews(cli.Namespace()).Create(&authorizationapi.LocalSubjectAccessReview{Action: authorizationapi.Action{Verb: "assign", Group: templateapi.GroupName, Resource: "templateinstances"}})
			if err != nil {
				return false, err
			}
			if !sar.Allowed {
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		dummytemplateinstance = &templateapi.TemplateInstance{ObjectMeta: metav1.ObjectMeta{Name: "test"}, Spec: templateapi.TemplateInstanceSpec{Template: templateapi.Template{ObjectMeta: metav1.ObjectMeta{Name: "template", Namespace: "dummy"}}, Requester: &templateapi.TemplateInstanceRequester{Username: edituser1.Name}}}
		tests = []struct {
			user				*userapi.User
			expectCreateSuccess		bool
			expectDeleteSuccess		bool
			hasUpdatePermission		bool
			hasUpdateStatusPermission	bool
		}{{user: nil, expectCreateSuccess: true, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: true}, {user: adminuser, expectCreateSuccess: false, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: false}, {user: impersonateuser, expectCreateSuccess: true, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: false}, {user: impersonatebygroupuser, expectCreateSuccess: true, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: false}, {user: edituser1, expectCreateSuccess: true, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: false}, {user: edituser2, expectCreateSuccess: false, expectDeleteSuccess: true, hasUpdatePermission: true, hasUpdateStatusPermission: false}, {user: viewuser, expectCreateSuccess: false, expectDeleteSuccess: false, hasUpdatePermission: false, hasUpdateStatusPermission: false}}
	})
	g.AfterEach(func() {
