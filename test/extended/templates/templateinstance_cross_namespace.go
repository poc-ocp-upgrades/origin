...
var _ = g.Describe("[Conformance][templates] templateinstance cross-namespace test", func() {
	...
	g.It("should create and delete objects across namespaces", func() {
		...
		_, err = cli.KubeClient().CoreV1().Secrets(cli.Namespace()).Create(&kapiv1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret"}, Data: map[string][]byte{"NAMESPACE": []byte(cli2.Namespace())}})
		o.Expect(err).NotTo(o.HaveOccurred())
		templateinstance := &templateapi.TemplateInstance{ObjectMeta: metav1.ObjectMeta{Name: "templateinstance"}, Spec: templateapi.TemplateInstanceSpec{Template: templateapi.Template{ObjectMeta: metav1.ObjectMeta{Name: "template", Namespace: cli.Namespace()}, Parameters: []templateapi.Parameter{{Name: "NAMESPACE"}}}, Secret: &kapi.LocalObjectReference{Name: "secret"}}}
		err = addObjectsToTemplate(&templateinstance.Spec.Template, []runtime.Object{&kapi.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret1"}}, &kapi.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret2", Namespace: "${NAMESPACE}"}}}, legacyscheme.Scheme.PrioritizedVersionsAllGroups()...)
		o.Expect(err).NotTo(o.HaveOccurred())
