...
var _ = g.Describe("[registry][Serial][Suite:openshift/registry/serial] Image signature workflow", func() {
	...
	g.It("can push a signed image to openshift registry and verify it", func() {
		...
		out, err := pod.Exec("rm -f /dev/random; ln -sf /dev/urandom /dev/random && " + "GNUPGHOME=/var/lib/origin/gnupg gpg2 --batch --gen-key dummy_key.conf")
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		out, err = pod.Exec(strings.Join([]string{"KUBERNETES_MASTER=https://$KUBERNETES_SERVICE_HOST:$KUBERNETES_SERVICE_PORT", "GNUPGHOME=/var/lib/origin/gnupg", "skopeo", "--debug", "copy", "--sign-by", "joe@foo.bar", "--dest-creds=" + user + ":" + token, "--dest-tls-verify=false", "docker://docker.io/library/memcached:latest", "atomic:" + signedImage}, " "))
		...
		out, err = pod.Exec(strings.Join([]string{"GNUPGHOME=/var/lib/origin/gnupg", "oc", "adm", "verify-image-signature", "--insecure=true", "--loglevel=8", imageName, "--expected-identity=" + signedImage, "--save"}, " "))
		fmt.Fprintf(g.GinkgoWriter, "output: %s\n", out)
		o.Expect(err).NotTo(o.HaveOccurred())
