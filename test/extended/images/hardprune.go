...
var _ = g.Describe("[Feature:ImagePrune][registry][Serial][Suite:openshift/registry/serial][local] Image hard prune", func() {
	...
	g.JustBeforeEach(func() {
		...
		readOnly := false
		acceptSchema2 := true
		_, err := ConfigureRegistry(oc, RegistryConfiguration{ReadOnly: &readOnly, AcceptSchema2: &acceptSchema2})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		o.Expect(err).NotTo(o.HaveOccurred())
		err = oc.AsAdmin().SetNamespace(metav1.NamespaceDefault).Run("adm").Args("policy", "add-cluster-role-to-user", "system:image-pruner", fmt.Sprintf("system:serviceaccount:%s:registry", metav1.NamespaceDefault)).Execute()
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.AfterEach(func() {
		readOnly := false
		_, err := ConfigureRegistry(oc, RegistryConfiguration{ReadOnly: &readOnly, AcceptSchema2: originalAcceptSchema2})
		o.Expect(err).NotTo(o.HaveOccurred())
	})
	...
	testHardPrune := func(dryRun bool) {
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions := &RegistryStorageFiles{ManifestLinks: RepoLinks{oc.Namespace() + "/a": []string{baseImg1}}}
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions, &RegistryStorageFiles{ManifestLinks: RepoLinks{oc.Namespace() + "/c": []string{childImg1}}, Blobs: []string{childImg1, imgs[childImg1].DockerImageMetadata.ID, imgs[childImg1].DockerImageLayers[0].Name}}, dryRun)
		err = AssertDeletedStorageFiles(deleted, expectedDeletions)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions, &RegistryStorageFiles{Blobs: []string{baseImg1, imgs[baseImg1].DockerImageMetadata.ID, imgs[baseImg1].DockerImageLayers[0].Name, imgs[baseImg1].DockerImageLayers[1].Name}}, dryRun)
		err = AssertDeletedStorageFiles(deleted, expectedDeletions)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions, &RegistryStorageFiles{ManifestLinks: RepoLinks{oc.Namespace() + "/b": []string{childImg2}}, Blobs: []string{childImg2, imgs[childImg2].DockerImageMetadata.ID, imgs[childImg2].DockerImageLayers[0].Name}}, dryRun)
		err = AssertDeletedStorageFiles(deleted, expectedDeletions)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions, &RegistryStorageFiles{Repos: []string{oc.Namespace() + "/b"}, Blobs: []string{baseImg2, imgs[baseImg2].DockerImageMetadata.ID, imgs[baseImg2].DockerImageLayers[0].Name, imgs[baseImg2].DockerImageLayers[1].Name}}, dryRun)
		err = AssertDeletedStorageFiles(deleted, expectedDeletions)
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		deleted, err = RunHardPrune(oc, dryRun)
		o.Expect(err).NotTo(o.HaveOccurred())
		expectedDeletions = mergeOrSetExpectedDeletions(expectedDeletions, &RegistryStorageFiles{ManifestLinks: RepoLinks{oc.Namespace() + "/c": []string{childImg3}}, Blobs: []string{childImg3, imgs[childImg3].DockerImageMetadata.ID, imgs[childImg3].DockerImageLayers[0].Name}}, dryRun)
		err = AssertDeletedStorageFiles(deleted, expectedDeletions)
		o.Expect(err).NotTo(o.HaveOccurred())
		assertImageBlobsPresent := func(present bool, img *imageapi.Image) {
			for _, layer := range img.DockerImageLayers {
				o.Expect(pathExistsInRegistry(oc, strings.Split(blobToPath("", layer.Name), "/")...)).To(o.Equal(present))
			}
			o.Expect(pathExistsInRegistry(oc, strings.Split(blobToPath("", img.DockerImageMetadata.ID), "/")...)).To(o.Equal(present))
			o.Expect(pathExistsInRegistry(oc, strings.Split(blobToPath("", img.Name), "/")...)).To(o.Equal(present))
		}
...
func (ba byAgeDesc) Swap(i, j int) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	ba[i], ba[j] = ba[j], ba[i]
}
...
func GetRegistryPod(podsGetter kcoreclient.PodsGetter) (*kapiv1.Pod, error) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	podList, err := podsGetter.Pods(metav1.NamespaceDefault).List(metav1.ListOptions{LabelSelector: labels.SelectorFromSet(labels.Set{"deploymentconfig": "docker-registry"}).String()})
	if err != nil {
		return nil, err
	}
