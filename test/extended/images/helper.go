...
func blobToPath(root, dgst string) string {
	...
	defer _logClusterCodePath()
	d := godigest.Digest(dgst)
	return path.Join(root, fmt.Sprintf("blobs/%s/%s/%s/data", d.Algorithm(), d.Hex()[0:2], d.Hex()))
}

var (
	pushDeniedErrorMessages	[]string	= []string{`requested access to the resource is denied`, `failed to push image: denied`, `^denied$`, `Error uploading manifest.*to.*: denied`}
...
func BuildAndPushImageOfSizeWithDocker(oc *exutil.CLI, dClient *dockerclient.Client, name, tag string, size uint64, numberOfLayers int, outSink io.Writer, shouldSucceed bool, removeBuiltImage bool) (string, string, error) {
	...
	digest, err := pushImageWithDocker(oc, dClient, image, imageName, tag, outSink, shouldSucceed, removeBuiltImage)
	if err != nil {
		return "", "", err
	}
...
func BuildAndPushChildImage(oc *exutil.CLI, dClient *dockerclient.Client, parent string, name, tag string, numberOfNewLayers int, outSink io.Writer, removeBuiltImage bool) (string, string, error) {
	...
	digest, err := pushImageWithDocker(oc, dClient, image, imageName, tag, outSink, true, removeBuiltImage)
	if err != nil {
		return "", "", err
	}
	return digest, image.ID, nil
...
func buildImageOfSizeWithDocker(oc *exutil.CLI, dClient *dockerclient.Client, parent, name, tag string, size uint64, numberOfLayers int, outSink io.Writer) (string, *dockerclient.Image, error) {
	...
	err = dClient.BuildImage(dockerclient.BuildImageOptions{Name: taggedName, RmTmpContainer: true, ForceRmTmpContainer: true, ContextDir: tempDir, OutputStream: outSink})
	if err != nil {
...
func pushImageWithDocker(oc *exutil.CLI, dClient *dockerclient.Client, image *dockerclient.Image, name, tag string, outSink io.Writer, shouldSucceed bool, removeBuiltImage bool) (string, error) {
	...
	err = dClient.PushImage(dockerclient.PushImageOptions{Name: name, Tag: tag, Registry: registryURL, OutputStream: &buf}, dockerclient.AuthConfiguration{Username: "test", Password: token, Email: "test@test.com", ServerAddress: registryURL})
	out = buf.String()
...
func MirrorBlobInRegistry(oc *exutil.CLI, dgst godigest.Digest, repository string, timeout time.Duration) error {
	...
	c := http.Client{Transport: knet.SetTransportDefaults(&http.Transport{TLSClientConfig: &tls.Config{InsecureSkipVerify: true}})}
...
func pathExistsInRegistry(oc *exutil.CLI, pthComponents ...string) (bool, error) {
	...
	out, err := oc.SetNamespace(metav1.NamespaceDefault).AsAdmin().Run("rsh").Args("dc/docker-registry", "/bin/sh", "-c", cmd).Output()
	if err != nil {
...
func IsBlobStoredInRegistry(oc *exutil.CLI, dgst godigest.Digest, repository string) (bool, bool, error) {
	...
	if len(repository) > 0 {
		presentInRepository, err = pathExistsInRegistry(oc, "repositories", repository, "_layers", string(dgst.Algorithm()), dgst.Hex(), "link")
	}
...
func RunHardPrune(oc *exutil.CLI, dryRun bool) (*RegistryStorageFiles, error) {
	...
	deleted := &RegistryStorageFiles{Repos: []string{}, ManifestLinks: make(RepoLinks), LayerLinks: make(RepoLinks), Blobs: []string{}}
	err = wait.ExponentialBackoff(retry.DefaultBackoff, func() (bool, error) {
		pruneType := "delete"
		if dryRun {
			pruneType = "check"
		}
		out, err := oc.SetNamespace(metav1.NamespaceDefault).AsAdmin().Run("exec").Args("--stdin", pod.Name, "--", "/bin/sh", "-s").InputString(fmt.Sprintf(registryGCLauncherScript, dockerRegistryBinary, pruneType)).Output()
	...
	var reDeleteRepositoryLink = regexp.MustCompile(reCommon + `(manifest|layer)(?:\s+link)?:\s+` + `([^#@]+)[#@]` + `([^"]+)"`)
	var reDeleteBlob = regexp.MustCompile(reCommon + `blob:\s+` + `(?:[^"]+/)?` + `([^":/]+)` + `(?:/[^"/]{2}/|:)` + `([^":/]+)"`)
...
func (c *CleanUpContainer) Run() {
	...
	out, err := c.OC.SetNamespace(metav1.NamespaceDefault).AsAdmin().Run("rsh").Args("dc/docker-registry", "find", "/registry", "-mindepth", "1", "-delete").Output()
	if err != nil {
