...
var _ = g.Describe("[Feature:ImageLayers][registry] Image layer subresource", func() {
	...
	g.It("should identify a deleted image as missing", func() {
		...
		_, err := client.ImageStreams(oc.Namespace()).Create(&imagev1.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: "test"}})
		o.Expect(err).NotTo(o.HaveOccurred())
		_, err = client.ImageStreamMappings(oc.Namespace()).Create(&imagev1.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: "test"}, Image: imagev1.Image{ObjectMeta: metav1.ObjectMeta{Name: "an_image_to_be_deleted"}, DockerImageReference: "example.com/random/image:latest"}, Tag: "missing"})
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	g.It("should return layers from tagged images", func() {
		...
		isi, err := client.ImageStreamImports(oc.Namespace()).Create(&imagev1.ImageStreamImport{ObjectMeta: metav1.ObjectMeta{Name: "1"}, Spec: imagev1.ImageStreamImportSpec{Import: true, Images: []imagev1.ImageImportSpec{{From: corev1.ObjectReference{Kind: "DockerImage", Name: "busybox:latest"}, To: &corev1.LocalObjectReference{Name: "busybox"}}, {From: corev1.ObjectReference{Kind: "DockerImage", Name: "mysql:latest"}, To: &corev1.LocalObjectReference{Name: "mysql"}}}}})
		o.Expect(err).NotTo(o.HaveOccurred())
	...
	Retry:
		...
		_, err = client.ImageStreams(oc.Namespace()).Create(&imagev1.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: "output"}})
		o.Expect(err).NotTo(o.HaveOccurred())
...
`
		...
		_, err = buildClient.Builds(oc.Namespace()).Create(&buildv1.Build{ObjectMeta: metav1.ObjectMeta{Name: "output"}, Spec: buildv1.BuildSpec{CommonSpec: buildv1.CommonSpec{Source: buildv1.BuildSource{Dockerfile: &dockerfile}, Strategy: buildv1.BuildStrategy{DockerStrategy: &buildv1.DockerBuildStrategy{From: &corev1.ObjectReference{Kind: "ImageStreamTag", Name: "1:busybox"}}}, Output: buildv1.BuildOutput{To: &corev1.ObjectReference{Kind: "ImageStreamTag", Name: "output:latest"}}}}})
		o.Expect(err).NotTo(o.HaveOccurred())
		...
		_, err = client.ImageStreamTags(newNamespace).Create(&imagev1.ImageStreamTag{ObjectMeta: metav1.ObjectMeta{Name: "output:latest"}, Tag: &imagev1.TagReference{Name: "copied", From: &corev1.ObjectReference{Kind: "ImageStreamTag", Namespace: oc.Namespace(), Name: "output:latest"}}})
		o.Expect(err).NotTo(o.HaveOccurred())
