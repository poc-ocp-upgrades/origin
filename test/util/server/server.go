...
var (
	...
	startedMaster	bool
	openshiftGVs	= []schema.GroupVersion{{Group: "apps.openshift.io", Version: "v1"}, {Group: "authorization.openshift.io", Version: "v1"}, {Group: "build.openshift.io", Version: "v1"}, {Group: "image.openshift.io", Version: "v1"}, {Group: "oauth.openshift.io", Version: "v1"}, {Group: "project.openshift.io", Version: "v1"}, {Group: "quota.openshift.io", Version: "v1"}, {Group: "route.openshift.io", Version: "v1"}, {Group: "security.openshift.io", Version: "v1"}, {Group: "template.openshift.io", Version: "v1"}, {Group: "user.openshift.io", Version: "v1"}}
)
...
func DefaultMasterOptionsWithTweaks() (*configapi.MasterConfig, error) {
	...
	recommendedAllowedRegistriesForImport := configapi.AllowedRegistries{{DomainName: "docker.io"}, {DomainName: "*.docker.io"}, {DomainName: "*.redhat.com"}, {DomainName: "gcr.io"}, {DomainName: "quay.io"}, {DomainName: "registry.centos.org"}, {DomainName: "registry.redhat.io"}}
	masterConfig.ImagePolicyConfig.ScheduledImageImportMinimumIntervalSeconds = 1
	allowedRegistries := append(recommendedAllowedRegistriesForImport, configapi.RegistryLocation{DomainName: "127.0.0.1:*"})
...
func CreateMasterCerts(masterArgs *start.MasterArgs) error {
	...
	createMasterCerts := admin.CreateMasterCertsOptions{CertDir: masterArgs.ConfigDir.Value(), SignerName: admin.DefaultSignerName(), Hostnames: hostnames.List(), ExpireDays: crypto.DefaultCertificateLifetimeInDays, SignerExpireDays: crypto.DefaultCACertificateLifetimeInDays, APIServerURL: masterURL.String(), PublicAPIServerURL: publicMasterURL.String(), IOStreams: genericclioptions.IOStreams{Out: os.Stderr}}
...
func startKubernetesAPIServer(masterConfig *configapi.MasterConfig, clientConfig *restclient.Config, stopCh <-chan struct{}) error {
	...
	legacyConfigCodec := configapi.Codecs.LegacyCodec(legacyconfigv1.LegacySchemeGroupVersion, schema.GroupVersion{Group: "autoscaling.openshift.io", Version: "v1"}, schema.GroupVersion{Group: "image.openshift.io", Version: "v1"}, schema.GroupVersion{Group: "scheduling.openshift.io", Version: "v1"}, schema.GroupVersion{Group: "project.openshift.io", Version: "v1"}, schema.GroupVersion{Group: "apiserver.k8s.io", Version: "v1alpha1"}, schema.GroupVersion{Group: "audit.k8s.io", Version: "v1alpha1"}, schema.GroupVersion{Group: "admission.config.openshift.io", Version: "v1"})
	...
	if err := create.EnsureManifestsCreated(ctx, crdManifestsDir, clientConfig, create.CreateOptions{Verbose: true}); err != nil {
		return err
	}
	return nil
}
func startOpenShiftAPIServer(masterConfig *configapi.MasterConfig, clientConfig *restclient.Config, stopCh <-chan struct{}) error {
	...
	service := &corev1.Service{ObjectMeta: metav1.ObjectMeta{Name: "openshift"}, Spec: corev1.ServiceSpec{Type: corev1.ServiceTypeClusterIP, Ports: []corev1.ServicePort{{Name: "https", Protocol: corev1.ProtocolTCP, Port: 443, TargetPort: targetPort}}}}
	...
	endpoint := &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "openshift"}, Subsets: []corev1.EndpointSubset{{Addresses: []corev1.EndpointAddress{{IP: kubeAddr.Hostname()}}, Ports: []corev1.EndpointPort{{Name: "https", Protocol: corev1.ProtocolTCP, Port: int32(targetPort.IntValue())}}}}}
	if _, err := kubeClient.CoreV1().Endpoints("default").Create(endpoint); err != nil {
		return err
	}
	for _, apiService := range openshiftGVs {
		obj := &apiregistrationv1.APIService{ObjectMeta: metav1.ObjectMeta{Name: apiService.Version + "." + apiService.Group}, Spec: apiregistrationv1.APIServiceSpec{Group: apiService.Group, Version: apiService.Version, Service: &apiregistrationv1.ServiceReference{Namespace: "default", Name: "openshift"}, GroupPriorityMinimum: 20000, VersionPriority: 15, InsecureSkipTLSVerify: true}}
...
func WaitForPodCreationServiceAccounts(clientset kubernetes.Interface, namespace string) error {
	...
	testPod.Spec.Containers = []corev1.Container{{Name: "container", Image: "openshift/origin-pod:latest"}}
...
func CreateNewProject(clientConfig *restclient.Config, projectName, adminUser string) (kubernetes.Interface, *restclient.Config, error) {
	...
	newProjectOptions := &newproject.NewProjectOptions{ProjectClient: projectClient, RbacClient: kubeExternalClient.RbacV1(), SARClient: authorizationClient.SubjectAccessReviews(), ProjectName: projectName, AdminRole: bootstrappolicy.AdminRoleName, AdminUser: adminUser, UseNodeSelector: false, IOStreams: genericclioptions.NewTestIOStreamsDiscard()}
