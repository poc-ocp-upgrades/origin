...
func New(c *OsdnNodeConfig) (*OsdnNode, error) {
	...
	plugin := &OsdnNode{policy: policy, kClient: c.KClient, networkClient: c.NetworkClient, recorder: c.Recorder, oc: oc, podManager: newPodManager(c.KClient, policy, c.MTU, c.CNIBinDir, oc, c.EnableHostports), localIP: c.SelfIP, hostName: c.Hostname, useConnTrack: useConnTrack, iptablesSyncPeriod: c.IPTablesSyncPeriod, mtu: c.MTU, egressPolicies: make(map[uint32][]networkapi.EgressNetworkPolicy), egressDNS: common.NewEgressDNS(), kubeInformers: c.KubeInformers, networkInformers: c.NetworkInformers, egressIP: newEgressIPWatcher(oc, c.SelfIP, c.MasqueradeBit), cniDirPath: c.CNIConfDir, runtimeEndpoint: c.RuntimeEndpoint, runtimeRequestTimeout: 2 * time.Minute, runtimeService: nil}
	RegisterMetrics()
	return plugin, nil
}
...
func (node *OsdnNode) Start() error {
	...
	if err := node.podManager.Start(cniserver.CNIServerRunDir, node.localSubnetCIDR, node.networkInfo.ClusterNetworks, node.networkInfo.ServiceNetwork.String(), networkChanged); err != nil {
		return err
	}
...
func (node *OsdnNode) reattachPods(existingPods map[string]podNetworkInfo) error {
	...
	for sandboxID, podInfo := range existingPods {
		...
		req := &cniserver.PodRequest{Command: cniserver.CNI_ADD, PodNamespace: sandbox.Metadata.Namespace, PodName: sandbox.Metadata.Name, SandboxID: sandboxID, HostVeth: podInfo.vethName, AssignedIP: podInfo.ip, Result: make(chan *cniserver.PodResult)}
...
func (node *OsdnNode) UpdatePod(pod corev1.Pod) error {
	...
	req := &cniserver.PodRequest{Command: cniserver.CNI_UPDATE, PodNamespace: pod.Namespace, PodName: pod.Name, SandboxID: sandboxID, Result: make(chan *cniserver.PodResult)}
...
func (node *OsdnNode) GetLocalPods(namespace string) ([]corev1.Pod, error) {
	...
	opts := metav1.ListOptions{LabelSelector: labels.Everything().String(), FieldSelector: fieldSelector.String()}
