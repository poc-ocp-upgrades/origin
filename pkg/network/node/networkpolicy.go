...
func (np *networkPolicyPlugin) initNamespaces() error {
	...
	for _, ns := range namespaces.Items {
		...
		if vnid, err := np.vnids.WaitAndGetVNID(ns.Name); err == nil {
			np.namespaces[vnid] = &npNamespace{name: ns.Name, vnid: vnid, inUse: inUseVNIDs.Has(int(vnid)), policies: make(map[ktypes.UID]*npPolicy)}
		}
	}
...
func (np *networkPolicyPlugin) AddNetNamespace(netns *networkv1.NetNamespace) {
	...
	np.namespaces[netns.NetID] = &npNamespace{name: netns.NetName, vnid: netns.NetID, inUse: false, policies: make(map[ktypes.UID]*npPolicy)}
}
...
func (np *networkPolicyPlugin) refreshNetworkPolicies(refreshFor refreshForType) {
	...
	for _, npns := range np.namespaces {
		changed := false
		for _, npp := range npns.policies {
			if ((refreshFor == refreshForNamespaces) && npp.watchesNamespaces) || ((refreshFor == refreshForPods) && npp.watchesPods) {
