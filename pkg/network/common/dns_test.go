...
func TestAddDNS(t *testing.T) {
	...
	tests := []dnsTest{{testCase: "Test valid domain name with resolver returning only A record", domainName: "example.com", dnsResolverOutput: "example.com. 600 IN A 10.11.12.13", ips: []net.IP{ip}, ttl: 600, expectFailure: false}, {testCase: "Test valid domain name with resolver returning both CNAME and A records", domainName: "example.com", dnsResolverOutput: "example.com. 200 IN CNAME foo.example.com.\nfoo.example.com. 600 IN A 10.11.12.13", ips: []net.IP{ip}, ttl: 200, expectFailure: false}, {testCase: "Test invalid domain name", domainName: "sads@#$.com", dnsResolverOutput: "", expectFailure: true}}
...
func TestUpdateDNS(t *testing.T) {
	...
	tests := []dnsTest{{testCase: "Test dns update of valid domain", domainName: "example.com", addResolverOutput: "example.com. 600 IN A 10.11.12.13", addIPs: []net.IP{addIP}, addTTL: 600, updateResolverOutput: "example.com. 500 IN A 10.11.12.14", updateIPs: []net.IP{updateIP}, updateTTL: 500, expectFailure: false}, {testCase: "Test dns update of invalid domain", domainName: "sads@#$.com", addResolverOutput: "", updateResolverOutput: "", expectFailure: true}}
	for _, test := range tests {
