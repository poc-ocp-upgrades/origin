...
func TestValidateSecurityContextConstraints(t *testing.T) {
	...
	invalidRangeMinGreaterThanMax.FSGroup.Ranges = []securityapi.IDRange{{Min: 2, Max: 1}}
	invalidRangeNegativeMin := validSCC()
	invalidRangeNegativeMin.FSGroup.Ranges = []securityapi.IDRange{{Min: -1, Max: 10}}
	invalidRangeNegativeMax := validSCC()
	invalidRangeNegativeMax.FSGroup.Ranges = []securityapi.IDRange{{Min: 1, Max: -10}}
	...
	flexvolumeWhenFlexVolumesAllowed.AllowedFlexVolumes = []securityapi.AllowedFlexVolume{{Driver: "example/driver1"}}
	flexvolumeWhenAllVolumesAllowed := validSCC()
	flexvolumeWhenAllVolumesAllowed.Volumes = []securityapi.FSType{securityapi.FSTypeAll}
	flexvolumeWhenAllVolumesAllowed.AllowedFlexVolumes = []securityapi.AllowedFlexVolume{{Driver: "example/driver2"}}
...
func TestValidatePodSecurityPolicySelfSubjectReview(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	okCases := map[string]securityapi.PodSecurityPolicySelfSubjectReview{"good case": {Spec: securityapi.PodSecurityPolicySelfSubjectReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}}}}
	...
	koCases := map[string]securityapi.PodSecurityPolicySelfSubjectReview{"[spec.template.spec.containers[0].name: Required value, spec.template.spec.containers[0].image: Required value, spec.template.spec.containers[0].imagePullPolicy: Required value]": {Spec: securityapi.PodSecurityPolicySelfSubjectReviewSpec{Template: kapi.PodTemplateSpec{Spec: invalidPodSpec()}}}}
...
func TestValidatePodSecurityPolicySubjectReview(t *testing.T) {
	...
	koCases := map[string]securityapi.PodSecurityPolicySubjectReview{"[spec.template.spec.containers[0].name: Required value, spec.template.spec.containers[0].image: Required value, spec.template.spec.containers[0].imagePullPolicy: Required value, spec.user: Required value: at least one of user, groups or serviceAccountName must be specified]": {Spec: securityapi.PodSecurityPolicySubjectReviewSpec{Template: kapi.PodTemplateSpec{Spec: invalidPodSpec()}}}}
	for k, v := range koCases {
		errs := ValidatePodSecurityPolicySubjectReview(&v)
		if len(errs) == 0 {
			t.Errorf("%s expected error %v", k, errs)
...
func TestValidatePodSecurityPolicyReview(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	okCases := map[string]securityapi.PodSecurityPolicyReview{"good case 1": {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}}}, "good case 2": {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}, ServiceAccountNames: []string{"good-service.account"}}}}
	for k, v := range okCases {
	...
	koCases := map[string]securityapi.PodSecurityPolicyReview{"[spec.template.spec.containers[0].name: Required value, spec.template.spec.containers[0].image: Required value, spec.template.spec.containers[0].imagePullPolicy: Required value]": {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: invalidPodSpec()}}}, `spec.serviceAccountNames[0]: Invalid value: "my bad sa": a DNS-1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')`: {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}, ServiceAccountNames: []string{"my bad sa"}}}, `spec.serviceAccountNames[1]: Invalid value: "my bad sa": a DNS-1123 subdomain must consist of lower case alphanumeric characters, '-' or '.', and must start and end with an alphanumeric character (e.g. 'example.com', regex used for validation is '[a-z0-9]([-a-z0-9]*[a-z0-9])?(\.[a-z0-9]([-a-z0-9]*[a-z0-9])?)*')`: {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}, ServiceAccountNames: []string{"good-service.account", "my bad sa"}}}, `spec.serviceAccountNames[2]: Invalid value: ""`: {Spec: securityapi.PodSecurityPolicyReviewSpec{Template: kapi.PodTemplateSpec{Spec: validPodSpec()}, ServiceAccountNames: []string{"good-service.account1", "good-service.account2", ""}}}}
	for k, v := range koCases {
		errs := ValidatePodSecurityPolicyReview(&v)
		if len(errs) == 0 {
			t.Errorf("%s expected error %v", k, errs)
