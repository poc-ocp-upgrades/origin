...
func TestSCCAllowsVolumeType(t *testing.T) {
	...
	tests := map[string]struct {
		...
		allows	bool
	}{"nil scc": {scc: nil, fsType: securityapi.FSTypeHostPath, allows: false}, "empty volumes": {scc: &securityapi.SecurityContextConstraints{}, fsType: securityapi.FSTypeHostPath, allows: false}, "non-matching": {scc: &securityapi.SecurityContextConstraints{Volumes: []securityapi.FSType{securityapi.FSTypeAWSElasticBlockStore}}, fsType: securityapi.FSTypeHostPath, allows: false}, "match on FSTypeAll": {scc: &securityapi.SecurityContextConstraints{Volumes: []securityapi.FSType{securityapi.FSTypeAll}}, fsType: securityapi.FSTypeHostPath, allows: true}, "match on direct match": {scc: &securityapi.SecurityContextConstraints{Volumes: []securityapi.FSType{securityapi.FSTypeHostPath}}, fsType: securityapi.FSTypeHostPath, allows: true}}
	for k, v := range tests {
...
func TestEqualStringSlices(t *testing.T) {
	...
	tests := map[string]struct {
		...
		arg2		[]string
		expectedResult	bool
	}{"nil equals to nil": {arg1: nil, arg2: nil, expectedResult: true}, "equal by size": {arg1: []string{"1", "1"}, arg2: []string{"1", "1"}, expectedResult: true}, "not equal by size": {arg1: []string{"1"}, arg2: []string{"1", "1"}, expectedResult: false}, "not equal by elements": {arg1: []string{"1", "1"}, arg2: []string{"1", "2"}, expectedResult: false}}
	for k, v := range tests {
