...
func TestMustRunAsOptions(t *testing.T) {
	...
	tests := map[string]struct {
		...
		pass	bool
	}{"invalid opts": {opts: &securityapi.SELinuxContextStrategyOptions{}, pass: false}, "valid opts": {opts: &securityapi.SELinuxContextStrategyOptions{SELinuxOptions: &api.SELinuxOptions{}}, pass: true}}
...
func TestMustRunAsValidate(t *testing.T) {
	...
	newValidOpts := func() *api.SELinuxOptions {
		return &api.SELinuxOptions{User: "user", Role: "role", Level: "s0:c0,c6", Type: "type"}
	}
	...
	tests := map[string]struct {
		...
		expectedMsg	string
	}{"invalid role": {podSeLinux: role, sccSeLinux: validOpts, expectedMsg: "role: Invalid value"}, "invalid user": {podSeLinux: user, sccSeLinux: validOpts, expectedMsg: "user: Invalid value"}, "levels are not equal": {podSeLinux: newValidOptsWithLevel("s0"), sccSeLinux: newValidOptsWithLevel("s0:c1,c2"), expectedMsg: "level: Invalid value"}, "levels differ by sensitivity": {podSeLinux: newValidOptsWithLevel("s0:c6"), sccSeLinux: newValidOptsWithLevel("s1:c6"), expectedMsg: "level: Invalid value"}, "levels differ by categories": {podSeLinux: newValidOptsWithLevel("s0:c0,c8"), sccSeLinux: newValidOptsWithLevel("s0:c1,c7"), expectedMsg: "level: Invalid value"}, "valid": {podSeLinux: validOpts, sccSeLinux: validOpts, expectedMsg: ""}, "valid with different order of categories": {podSeLinux: newValidOptsWithLevel("s0:c6,c0"), sccSeLinux: validOpts, expectedMsg: ""}}
	for name, tc := range tests {
		opts := &securityapi.SELinuxContextStrategyOptions{SELinuxOptions: tc.sccSeLinux}
		mustRunAs, err := NewMustRunAs(opts)
		if err != nil {
