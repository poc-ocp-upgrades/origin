...
func TestController(t *testing.T) {
	...
	c := &NamespaceSCCAllocationController{requiredUIDRange: uidr, mcsAllocator: DefaultMCSAllocation(uidr, mcsr, 5), namespaceClient: kubeclient.CoreV1().Namespaces(), nsLister: corev1listers.NewNamespaceLister(indexer), rangeAllocationClient: securityclient.SecurityV1()}
	err := c.Repair()
	if err != nil {
		t.Fatal(err)
	}
...
func TestControllerError(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		actions	int
	}{"not found": {err: func() error {
		return errors.NewNotFound(kapi.Resource("Namespace"), "test")
	}, errFn: func(err error) bool {
		return err == nil
	}, actions: 1}, "unknown": {err: func() error {
		return fmt.Errorf("unknown")
	}, errFn: func(err error) bool {
		return err.Error() == "unknown"
	}, actions: 1}, "conflict": {actions: 1, reactFn: func(a clientgotesting.Action) (bool, runtime.Object, error) {
