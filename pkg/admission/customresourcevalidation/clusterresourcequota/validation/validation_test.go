...
func TestValidationClusterQuota(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	invalidQuotaResourceSpec := corev1.ResourceQuotaSpec{Hard: corev1.ResourceList{corev1.ResourceStorage: resource.MustParse("10")}}
	validLabels := map[string]string{"a": "b"}
	errs := ValidateClusterResourceQuota(&quotav1.ClusterResourceQuota{ObjectMeta: metav1.ObjectMeta{Name: "good"}, Spec: quotav1.ClusterResourceQuotaSpec{Selector: quotav1.ClusterResourceQuotaSelector{LabelSelector: &metav1.LabelSelector{MatchLabels: validLabels}}, Quota: spec()}})
	if len(errs) != 0 {
...
func TestValidationQuota(t *testing.T) {
	...
	tests := map[string]struct {
		...
		T	field.ErrorType
		F	string
	}{"scope": {A: corev1.ResourceQuota{ObjectMeta: metav1.ObjectMeta{Namespace: "ns", Name: "good"}, Spec: scopeableSpec(corev1.ResourceQuotaScopeNotTerminating)}}}
	for k, v := range tests {
