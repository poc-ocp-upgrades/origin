...
func TestCustomResourceValidator(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.description, func(t *testing.T) {
			...
			handler, err := NewValidator(map[schema.GroupResource]bool{{Group: testGroup, Resource: testResource}: true}, map[schema.GroupVersionKind]ObjectValidator{{Group: testGroup, Version: testVersion, Kind: testKind}: testValidator{validateCreate: func(obj runtime.Object) field.ErrorList {
			...
			if tc.resource == (schema.GroupVersionResource{}) {
				tc.resource = schema.GroupVersionResource{Group: testGroup, Version: testVersion, Resource: testResource}
			}
			attributes := admission.NewAttributesRecord(tc.object, tc.oldObject, tc.kind, tc.namespace, tc.name, tc.resource, tc.subresource, tc.operation, false, tc.userInfo)
