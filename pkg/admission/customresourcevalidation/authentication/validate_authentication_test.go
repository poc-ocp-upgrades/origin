...
func TestFailValidateAuthenticationStatus(t *testing.T) {
	...
	errorCases := map[string]struct {
		...
		errorType	field.ErrorType
		errorField	string
	}{"wrong reference name": {status: configv1.AuthenticationStatus{IntegratedOAuthMetadata: configv1.ConfigMapNameReference{Name: "something_wrong"}}, errorType: field.ErrorTypeInvalid, errorField: "status.integratedOAuthMetadata.name"}}
	for tcName, tc := range errorCases {
...
func TestSucceedValidateAuthenticationStatus(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	successCases := map[string]configv1.AuthenticationStatus{"basic case": {IntegratedOAuthMetadata: configv1.ConfigMapNameReference{Name: "hey-there"}}, "empty reference": {IntegratedOAuthMetadata: configv1.ConfigMapNameReference{Name: ""}}, "empty status": {}}
	for tcName, s := range successCases {
