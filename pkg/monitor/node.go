...
func startNodeMonitoring(ctx context.Context, m Recorder, client kubernetes.Interface) {
	...
	nodeChangeFns := []func(node, oldNode *corev1.Node) []Condition{func(node, oldNode *corev1.Node) []Condition {
		...
		for i := range node.Status.Conditions {
			...
			if c.Status != previous.Status {
				conditions = append(conditions, Condition{Level: Warning, Locator: locateNode(node), Message: fmt.Sprintf("condition %s changed", c.Type)})
			}
		}
		if node.UID != oldNode.UID {
			conditions = append(conditions, Condition{Level: Error, Locator: locateNode(node), Message: fmt.Sprintf("node was deleted and recreated")})
		}
		return conditions
	}}
	...
	nodeInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
		...
		m.Record(Condition{Level: Warning, Locator: locateNode(pod), Message: "deleted"})
	...
	m.AddSampler(func(now time.Time) []*Condition {
		...
		for _, obj := range nodeInformer.GetStore().List() {
			...
			if !isReady {
				conditions = append(conditions, &Condition{Level: Warning, Locator: locateNode(node), Message: "node is not ready"})
			}
		}
		return conditions
	})
