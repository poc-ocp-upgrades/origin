...
func startClusterOperatorMonitoring(ctx context.Context, m Recorder, client configclientset.Interface) {
	...
	}}), &configv1.ClusterOperator{}, time.Hour, nil)
	coChangeFns := []func(co, oldCO *configv1.ClusterOperator) []Condition{func(co, oldCO *configv1.ClusterOperator) []Condition {
		...
		for i := range co.Status.Conditions {
			...
			if s.Status != previous.Status {
				...
				conditions = append(conditions, Condition{Level: level, Locator: locateClusterOperator(co), Message: msg})
			}
			if changes := findOperatorVersionChange(oldCO.Status.Versions, co.Status.Versions); len(changes) > 0 {
				conditions = append(conditions, Condition{Level: Info, Locator: locateClusterOperator(co), Message: fmt.Sprintf("versions: %v", strings.Join(changes, ","))})
			}
		}
		return conditions
	}}
	startTime := time.Now().Add(-time.Minute)
	coInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
		...
		m.Record(Condition{Level: Info, Locator: locateClusterOperator(co), Message: "created"})
	}, DeleteFunc: func(obj interface{}) {
		co, ok := obj.(*configv1.ClusterOperator)
		if !ok {
			return
		}
		m.Record(Condition{Level: Warning, Locator: locateClusterOperator(co), Message: "deleted"})
	...
	cvInformer := cache.NewSharedIndexInformer(NewErrorRecordingListWatcher(m, &cache.ListWatch{ListFunc: func(options metav1.ListOptions) (runtime.Object, error) {
	...
	}}), &configv1.ClusterVersion{}, time.Hour, nil)
	cvChangeFns := []func(cv, oldCV *configv1.ClusterVersion) []Condition{func(cv, oldCV *configv1.ClusterVersion) []Condition {
		var conditions []Condition
		if len(cv.Status.History) == 0 {
			return nil
		}
		if len(oldCV.Status.History) == 0 {
			conditions = append(conditions, Condition{Level: Warning, Locator: locateClusterVersion(cv), Message: fmt.Sprintf("cluster converging to %s", cv.Status.History[0].Version)})
			return conditions
		}
		cvNew, cvOld := cv.Status.History[0], oldCV.Status.History[0]
		switch {
		case cvNew.State == configv1.CompletedUpdate && cvOld.State != cvNew.State:
			conditions = append(conditions, Condition{Level: Warning, Locator: locateClusterVersion(cv), Message: fmt.Sprintf("cluster reached %s", cvNew.Version)})
		case cvNew.State == configv1.PartialUpdate && cvOld.State == cvNew.State && cvOld.Image != cvNew.Image:
			conditions = append(conditions, Condition{Level: Warning, Locator: locateClusterVersion(cv), Message: fmt.Sprintf("cluster upgrading to %s without completing %s", cvNew.Version, cvOld.Version)})
		}
		return conditions
		...
		for i := range cv.Status.Conditions {
			...
			if s.Status != previous.Status {
				...
				conditions = append(conditions, Condition{Level: level, Locator: locateClusterVersion(cv), Message: msg})
			}
		}
		return conditions
	}}
	cvInformer.AddEventHandler(cache.ResourceEventHandlerFuncs{AddFunc: func(obj interface{}) {
		...
		m.Record(Condition{Level: Info, Locator: locateClusterVersion(cv), Message: "created"})
	}, DeleteFunc: func(obj interface{}) {
		cv, ok := obj.(*configv1.ClusterVersion)
		if !ok {
			return
		}
		m.Record(Condition{Level: Warning, Locator: locateClusterVersion(cv), Message: "deleted"})
	}, UpdateFunc: func(old, obj interface{}) {
		cv, ok := obj.(*configv1.ClusterVersion)
		if !ok {
			return
		...
		for _, obj := range cvInformer.GetStore().List() {
			...
			if len(cv.Status.History) > 0 {
				if cv.Status.History[0].State != configv1.CompletedUpdate {
					conditions = append(conditions, &Condition{Level: Warning, Locator: locateClusterVersion(cv), Message: fmt.Sprintf("cluster is updating to %s", cv.Status.History[0].Version)})
				}
			}
		}
		return conditions
