...
func TestController_stabilizeAfterCreate(t *testing.T) {
	...
	i := &ingressLister{Items: []*extensionsv1beta1.Ingress{ingress}}
	r := &routeLister{}
	s := &secretLister{}
	svc := &serviceLister{Items: []*v1.Service{{ObjectMeta: metav1.ObjectMeta{Name: "ingress-endpoint-1", Namespace: "test"}, Spec: v1.ServiceSpec{Ports: []v1.ServicePort{{Name: "http", Port: 80, TargetPort: intstr.FromInt(8080)}}}}, {ObjectMeta: metav1.ObjectMeta{Name: "ingress-endpoint-2", Namespace: "test"}, Spec: v1.ServiceSpec{Ports: []v1.ServicePort{{Name: "80-tcp", Port: 80, TargetPort: intstr.FromInt(8080)}}}}}}
	var names []string
	kc := &fake.Clientset{}
	...
	c := &Controller{queue: workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "ingress-to-route-test"), client: kc.RouteV1(), ingressLister: i, routeLister: r, secretLister: s, serviceLister: svc, expectations: newExpectations()}
...
func TestController_sync(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			c := &Controller{queue: workqueue.NewNamedRateLimitingQueue(workqueue.DefaultControllerRateLimiter(), "ingress-to-route-test"), client: kc.RouteV1(), ingressLister: tt.fields.i, routeLister: tt.fields.r, secretLister: tt.fields.s, serviceLister: tt.fields.svc, expectations: tt.expects}
