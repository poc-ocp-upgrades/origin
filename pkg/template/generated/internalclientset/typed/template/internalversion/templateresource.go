...
func (c *templates) Get(name string, options v1.GetOptions) (result *template.Template, err error) {
	...
	err = c.client.Get().Namespace(c.ns).Resource("templates").Name(name).VersionedParams(&options, scheme.ParameterCodec).Do().Into(result)
	return
}
func (c *templates) List(opts v1.ListOptions) (result *template.TemplateList, err error) {
	...
	err = c.client.Get().Namespace(c.ns).Resource("templates").VersionedParams(&opts, scheme.ParameterCodec).Timeout(timeout).Do().Into(result)
	return
}
...
func (c *templates) Create(templateResource *template.Template) (result *template.Template, err error) {
	...
	defer _logClusterCodePath()
	result = &template.Template{}
	err = c.client.Post().Namespace(c.ns).Resource("templates").Body(templateResource).Do().Into(result)
	return
}
...
func (c *templates) Delete(name string, options *v1.DeleteOptions) error {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return c.client.Delete().Namespace(c.ns).Resource("templates").Name(name).Body(options).Do().Error()
}
...
func (c *templates) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *template.Template, err error) {
	...
	defer _logClusterCodePath()
	result = &template.Template{}
	err = c.client.Patch(pt).Namespace(c.ns).Resource("templates").SubResource(subresources...).Name(name).Body(data).Do().Into(result)
	return
}
