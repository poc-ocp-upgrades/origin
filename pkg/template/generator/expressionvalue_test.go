...
func TestExpressionValueGenerator(t *testing.T) {
	...
	}{{"test[A-Z0-9]{4}template", "testQ3HVtemplate"}, {"[\\d]{3}", "889"}, {"[\\w]{20}", "hiG4uRbcUDd5PEJLyHZ7"}, {"[\\a]{10}", "4U390O49B9"}, {"[\\A]{10}", ")^&-|_:[><"}, {"strongPassword[\\w]{3}[\\A]{3}", "strongPasswordhiG-|_"}, {"admin[0-9]{2}[A-Z]{2}", "admin78YB"}, {"admin[0-9]{2}test[A-Z]{2}", "admin78testYB"}}
...
func TestRemoveDuplicatedCharacters(t *testing.T) {
	...
	var tests = []struct {
		Expression	string
		ExpectedValue	string
	}{{"abcdefgh", "abcdefgh"}, {"abcabc", "abc"}, {"1111111", "1"}, {"1234567890", "1234567890"}, {"test@@", "tes@"}}
	for _, test := range tests {
