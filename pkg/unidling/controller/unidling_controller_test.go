...
func prepFakeClient(t *testing.T, nowTime time.Time, scales ...autoscalingv1.Scale) (*kexternalfake.Clientset, *appsfake.Clientset, *fakeResults) {
	...
	for i, scale := range scales {
		targets[i] = unidlingapi.RecordedScaleReference{CrossGroupObjectReference: unidlingapi.CrossGroupObjectReference{Name: scale.Name, Kind: scale.Kind}, Replicas: 2}
	}
	targetsAnnotation, err := json.Marshal(targets)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	endpointsObj := corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: nowTimeStr, unidlingapi.UnidleTargetAnnotation: string(targetsAnnotation)}}}
	...
	fakeDeployClient.PrependReactor("get", "deploymentconfigs", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		...
		for _, scale := range scales {
			if scale.Kind == "DeploymentConfig" && objName == scale.Name {
				return true, &appsv1.DeploymentConfig{ObjectMeta: metav1.ObjectMeta{Name: objName}, Spec: appsv1.DeploymentConfigSpec{Replicas: scale.Spec.Replicas}}, nil
			}
		}
	...
	fakeClient.PrependReactor("get", "replicationcontrollers", func(action clientgotesting.Action) (bool, runtime.Object, error) {
		...
		for _, scale := range scales {
			if scale.Kind == "ReplicationController" && objName == scale.Name {
				return true, &corev1.ReplicationController{ObjectMeta: metav1.ObjectMeta{Name: objName}, Spec: corev1.ReplicationControllerSpec{Replicas: &scale.Spec.Replicas}}, nil
			}
		}
		return true, nil, errors.NewNotFound(action.GetResource().GroupResource(), objName)
	})
	res := &fakeResults{resMap: make(map[unidlingapi.CrossGroupObjectReference]autoscalingv1.Scale)}
...
func TestControllerHandlesStaleEvents(t *testing.T) {
	...
	controller := &UnidlingController{endpointsNamespacer: fakeClient.CoreV1(), rcNamespacer: fakeClient.CoreV1(), dcNamespacer: fakeDeployClient.AppsV1()}
	retry, err := controller.handleRequest(types.NamespacedName{Namespace: "somens", Name: "somesvc"}, nowTime.Add(-10*time.Second))
	if err != nil {
...
func TestControllerIgnoresAlreadyScaledObjects(t *testing.T) {
	...
	t.Skip("failing because of missing client")
	nowTime := time.Now().Truncate(time.Second)
	baseScales := []autoscalingv1.Scale{{ObjectMeta: metav1.ObjectMeta{Name: "somerc"}, TypeMeta: metav1.TypeMeta{Kind: "ReplicationController"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}, {ObjectMeta: metav1.ObjectMeta{Name: "somedc"}, TypeMeta: metav1.TypeMeta{Kind: "DeploymentConfig"}, Spec: autoscalingv1.ScaleSpec{Replicas: 5}}}
	idledTime := nowTime.Add(-10 * time.Second)
	fakeClient, fakeDeployClient, res := prepFakeClient(t, idledTime, baseScales...)
	controller := &UnidlingController{endpointsNamespacer: fakeClient.CoreV1(), rcNamespacer: fakeClient.CoreV1(), dcNamespacer: fakeDeployClient.AppsV1()}
	retry, err := controller.handleRequest(types.NamespacedName{Namespace: "somens", Name: "somesvc"}, nowTime)
	if err != nil {
		t.Fatalf("Unable to unidle: unexpected error (retry: %v): %v", retry, err)
	}
	if len(res.resMap) != 1 {
...
func TestControllerPerformsCorrectlyOnFailures(t *testing.T) {
	...
	t.Skip("failing because of missing client")
	nowTime := time.Now().Truncate(time.Second)
	baseScalables := []unidlingapi.RecordedScaleReference{{CrossGroupObjectReference: unidlingapi.CrossGroupObjectReference{Kind: "ReplicationController", Name: "somerc"}, Replicas: 2}, {CrossGroupObjectReference: unidlingapi.CrossGroupObjectReference{Kind: "DeploymentConfig", Group: appsv1.GroupName, Name: "somedc"}, Replicas: 2}}
	baseScalablesBytes, err := json.Marshal(baseScalables)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	outScalables := []unidlingapi.RecordedScaleReference{{CrossGroupObjectReference: unidlingapi.CrossGroupObjectReference{Kind: "DeploymentConfig", Group: appsv1.GroupName, Name: "somedc"}, Replicas: 2}}
	outScalablesBytes, err := json.Marshal(outScalables)
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	tests := []failureTestInfo{{name: "retry on failed endpoints get", endpointsGet: nil, errorExpected: true, retryExpected: true}, {name: "not retry on failure to parse time", endpointsGet: &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: "cheddar"}}}, errorExpected: true, retryExpected: false}, {name: "not retry on failure to unmarshal target scalables", endpointsGet: &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: "pecorino romano"}}}, errorExpected: true, retryExpected: false}, {name: "remove a scalable from the list if it cannot be found (while getting)", endpointsGet: &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: string(baseScalablesBytes)}}}, scaleGets: []autoscalingv1.Scale{{TypeMeta: metav1.TypeMeta{Kind: "DeploymentConfig", APIVersion: "apps.openshift.io/v1"}, ObjectMeta: metav1.ObjectMeta{Name: "somedc"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}}, errorExpected: false, annotationsExpected: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: string(outScalablesBytes)}}, {name: "should remove a scalable from the list if it cannot be found (while updating)", endpointsGet: &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: string(baseScalablesBytes)}}}, scaleGets: []autoscalingv1.Scale{{TypeMeta: metav1.TypeMeta{Kind: "ReplicationController"}, ObjectMeta: metav1.ObjectMeta{Name: "somerc"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}, {TypeMeta: metav1.TypeMeta{Kind: "DeploymentConfig", APIVersion: "apps.openshift.io/v1"}, ObjectMeta: metav1.ObjectMeta{Name: "somedc"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}}, scaleUpdatesNotFound: []bool{false, true}, errorExpected: false, annotationsExpected: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: string(outScalablesBytes)}}, {name: "retry on failed endpoints update", endpointsGet: &corev1.Endpoints{ObjectMeta: metav1.ObjectMeta{Name: "somesvc", Annotations: map[string]string{unidlingapi.IdledAtAnnotation: nowTime.Format(time.RFC3339), unidlingapi.UnidleTargetAnnotation: string(baseScalablesBytes)}}}, scaleGets: []autoscalingv1.Scale{{TypeMeta: metav1.TypeMeta{Kind: "ReplicationController"}, ObjectMeta: metav1.ObjectMeta{Name: "somerc"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}, {TypeMeta: metav1.TypeMeta{Kind: "DeploymentConfig", APIVersion: "apps.openshift.io/v1"}, ObjectMeta: metav1.ObjectMeta{Name: "somedc"}, Spec: autoscalingv1.ScaleSpec{Replicas: 0}}}, preventEndpointsUpdate: true, errorExpected: true, retryExpected: true}}
