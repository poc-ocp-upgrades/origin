...
type eventSignaler struct{ recorder record.EventRecorder }

func (sig *eventSignaler) NeedPods(serviceName types.NamespacedName, port string) error {
	_logClusterCodePath()
	defer _logClusterCodePath()
	serviceRef := v1.ObjectReference{Kind: "Service", Namespace: serviceName.Namespace, Name: serviceName.Name}
...
func NewEventSignaler(eventRecorder record.EventRecorder) NeedPodsSignaler {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &eventSignaler{recorder: eventRecorder}
}
