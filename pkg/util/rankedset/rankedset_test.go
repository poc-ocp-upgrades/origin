...
func TestRankedSet(t *testing.T) {
	...
	for _, tc := range []struct {
		name	string
		f	func(*testing.T)
	}{{name: "insert", f: func(t *testing.T) {
		assertLen(s, 0, t)
		s.Insert(a)
		assertLen(s, 1, t)
		s.Insert(b)
		...
		assertItem(b, s.Max(), t)
	}}, {name: "insert replace", f: func(t *testing.T) {
...
func newTestSetItem(key string, rank int64, data string) *testSetItem {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &testSetItem{key: key, rank: rank, data: data}
}
