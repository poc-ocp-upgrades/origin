...
func TestRateLimitedFunction(t *testing.T) {
	...
	tests := []struct {
		...
		Interval	int
		Times		int
	}{{Name: "unrated", Interval: 0, Times: 5}, {Name: "3PO", Interval: 3, Times: 10}, {Name: "five-fer", Interval: 5, Times: 20}}
	...
	for _, tc := range tests {
		...
		case <-time.After(time.Duration(tc.Interval+2) * time.Second):
			...
			if tc.Interval > 0 && counter >= tc.Times/2 {
				t.Errorf("For coalesced calls, expected number of invocations to be atleast half. Expected: < %v  Got: %v", tc.Times/2, counter)
			}
		}
	}
}
