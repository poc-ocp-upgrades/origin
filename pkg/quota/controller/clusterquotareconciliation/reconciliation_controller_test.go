...
func TestSyncFunc(t *testing.T) {
	...
	testCases := []struct {
	...
	}{{name: "from nothing", startingQuota: defaultQuota, workItems: []workItem{{namespaceName: "one"}}, mapperFunc: func() clusterquotamapping.ClusterQuotaMapper {
		...
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "one", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("10")}}})
		return ret
	}, expectedRetries: []workItem{}}, {name: "cache not ready", startingQuota: defaultQuota, workItems: []workItem{{namespaceName: "one"}}, mapperFunc: func() clusterquotamapping.ClusterQuotaMapper {
		mapper := newFakeClusterQuotaMapper()
		mapper.quotaToNamespaces["foo"] = sets.NewString("one")
		...
		return nil
	}, expectedRetries: []workItem{{namespaceName: "one"}}, expectedError: "mapping not up to date"}, {name: "removed from nothing", startingQuota: defaultQuota, workItems: []workItem{{namespaceName: "one"}}, mapperFunc: func() clusterquotamapping.ClusterQuotaMapper {
		...
		return ret
	}, expectedRetries: []workItem{}}, {name: "removed from something", startingQuota: func() *quotav1.ClusterResourceQuota {
		ret := defaultQuota()
		ret.Status.Total.Hard = ret.Spec.Quota.Hard
		ret.Status.Total.Used = corev1.ResourceList{corev1.ResourcePods: resource.MustParse("10")}
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "one", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("10")}}})
		return ret
	}, workItems: []workItem{{namespaceName: "one"}}, mapperFunc: func() clusterquotamapping.ClusterQuotaMapper {
		return newFakeClusterQuotaMapper()
	}, calculationFunc: func(namespaceName string, scopes []corev1.ResourceQuotaScope, hardLimits corev1.ResourceList, registry utilquota.Registry, scopeSelector *corev1.ScopeSelector) (corev1.ResourceList, error) {
		if e, a := "one", namespaceName; e != a {
		...
		return ret
	}, expectedRetries: []workItem{}}, {name: "update one, remove two, ignore three, fail four, remove deleted", startingQuota: func() *quotav1.ClusterResourceQuota {
		ret := defaultQuota()
		ret.Status.Total.Hard = ret.Spec.Quota.Hard
		ret.Status.Total.Used = corev1.ResourceList{corev1.ResourcePods: resource.MustParse("30")}
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "one", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("5")}}})
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "two", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("10")}}})
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "three", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("15")}}})
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "deleted", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("0")}}})
		return ret
	}, workItems: []workItem{{namespaceName: "one", forceRecalculation: true}, {namespaceName: "two"}, {namespaceName: "three"}, {namespaceName: "four"}}, mapperFunc: func() clusterquotamapping.ClusterQuotaMapper {
		mapper := newFakeClusterQuotaMapper()
		...
		quotav1conversions.InsertResourceQuotasStatus(&ret.Status.Namespaces, quotav1.ResourceQuotaStatusByNamespace{Namespace: "three", Status: corev1.ResourceQuotaStatus{Hard: ret.Spec.Quota.Hard, Used: corev1.ResourceList{corev1.ResourcePods: resource.MustParse("15")}}})
		return ret
	}, expectedRetries: []workItem{{namespaceName: "four"}}, expectedError: "calculation error"}}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			client := fakequotaclient.NewSimpleClientset(tc.startingQuota())
			quotaUsageCalculationFunc = tc.calculationFunc
			controller := ClusterQuotaReconcilationController{clusterQuotaMapper: tc.mapperFunc(), clusterQuotaClient: client.QuotaV1().ClusterResourceQuotas()}
