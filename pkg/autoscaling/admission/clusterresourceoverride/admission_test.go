...
var (
	deserializedYamlConfig = &clusterresourceoverride.ClusterResourceOverrideConfig{LimitCPUToMemoryPercent: 100, CPURequestToLimitPercent: 10, MemoryRequestToLimitPercent: 25}
)
...
func TestConfigReader(t *testing.T) {
	...
	}{{name: "process nil config", config: nil, expectNil: true}, {name: "deserialize initialConfig yaml", config: bytes.NewReader(serializedConfig), expectedConfig: initialConfig}, {name: "completely broken config", config: bytes.NewReader([]byte("asdfasdfasdF")), expectErr: true}, {name: "deserialize yamlConfig", config: bytes.NewReader([]byte(yamlConfig)), expectedConfig: deserializedYamlConfig}, {name: "choke on out-of-bounds ratio", config: bytes.NewReader([]byte(invalidConfig)), expectInvalid: true, expectErr: true}, {name: "complain about no settings", config: bytes.NewReader([]byte(invalidConfig2)), expectInvalid: true, expectErr: true}}
...
func TestLimitRequestAdmission(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			c.(*clusterResourceOverridePlugin).limitRangesLister = fakeLimitRangeLister{namespaceLister: fakeLimitRangeNamespaceLister{limits: test.namespaceLimits}}
			...
			resources := test.pod.Spec.InitContainers[0].Resources
...
func testPod(memLimit string, memRequest string, cpuLimit string, cpuRequest string) *kapi.Pod {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &kapi.Pod{Spec: kapi.PodSpec{InitContainers: []kapi.Container{{Resources: kapi.ResourceRequirements{Limits: kapi.ResourceList{kapi.ResourceCPU: resource.MustParse(cpuLimit), kapi.ResourceMemory: resource.MustParse(memLimit)}, Requests: kapi.ResourceList{kapi.ResourceCPU: resource.MustParse(cpuRequest), kapi.ResourceMemory: resource.MustParse(memRequest)}}}}, Containers: []kapi.Container{{Resources: kapi.ResourceRequirements{Limits: kapi.ResourceList{kapi.ResourceCPU: resource.MustParse(cpuLimit), kapi.ResourceMemory: resource.MustParse(memLimit)}, Requests: kapi.ResourceList{kapi.ResourceCPU: resource.MustParse(cpuRequest), kapi.ResourceMemory: resource.MustParse(memRequest)}}}}}}
}
...
func fakeNamespace(pluginEnabled bool) *corev1.Namespace {
	...
	defer _logClusterCodePath()
	nsIndex++
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: fmt.Sprintf("fakeNS%d", nsIndex), Annotations: map[string]string{}}}
	if !pluginEnabled {
...
func fakeMinLimitRange(limitType corev1.LimitType, resourceType corev1.ResourceName, limits ...string) *corev1.LimitRange {
	...
	for i := range limits {
		...
		r.Spec.Limits = append(r.Spec.Limits, corev1.LimitRangeItem{Type: limitType, Min: rl})
	}
	return r
}
