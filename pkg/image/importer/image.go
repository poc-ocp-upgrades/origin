...
func schema1ToImage(manifest *schema1.SignedManifest, d godigest.Digest) (*imageapi.Image, error) {
	...
	image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: dockerImage.ID}, DockerImageMetadata: *dockerImage, DockerImageManifest: string(payload), DockerImageManifestMediaType: mediatype, DockerImageMetadataVersion: "1.0"}
	return image, nil
}
func schema2ToImage(manifest *schema2.DeserializedManifest, imageConfig []byte, d godigest.Digest) (*imageapi.Image, error) {
	...
	dockerImage.ID = payloadDigest.String()
	image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: dockerImage.ID}, DockerImageMetadata: *dockerImage, DockerImageManifest: string(payload), DockerImageConfig: string(imageConfig), DockerImageManifestMediaType: mediatype, DockerImageMetadataVersion: "1.0"}
	return image, nil
}
func schema0ToImage(dockerImage *dockerregistry.Image) (*imageapi.Image, error) {
	...
	image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: dockerImage.Image.ID}, DockerImageMetadata: baseImage, DockerImageMetadataVersion: "1.0"}
	return image, nil
}
