...
func TestNewRegistryWhitelister(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		whitelist	openshiftcontrolplanev1.AllowedRegistries
		expectedError	error
	}{{name: "chinese works", whitelist: mkAllowed(true, "先生，先生！")}, {name: "don't try it with multiple colons though", whitelist: mkAllowed(true, "0:1:2:3"), expectedError: fmt.Errorf(`failed to parse allowed registry "0:1:2:3": too many colons`)}} {
		t.Run(tc.name, func(t *testing.T) {
