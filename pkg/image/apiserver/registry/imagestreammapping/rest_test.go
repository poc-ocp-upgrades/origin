...
var testDefaultRegistry = func() (string, bool) {
	return testDefaultRegistryURL, true
}
...
func validNewMappingWithName() *imageapi.ImageStreamMapping {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "somerepo"}, Image: imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: testImageID, Annotations: map[string]string{imageapi.ManagedByOpenShiftAnnotation: "true"}}, DockerImageReference: "localhost:5000/default/somerepo@" + testImageID, DockerImageMetadata: imageapi.DockerImage{Config: &imageapi.DockerConfig{Cmd: []string{"ls", "/"}, Env: []string{"a=1"}, ExposedPorts: map[string]struct{}{"1234/tcp": {}}, Memory: 1234, CPUShares: 99, WorkingDir: "/workingDir"}}}, Tag: "latest"}
}
...
func TestCreateSuccessWithName(t *testing.T) {
	...
	_, err := client.Put(context.TODO(), etcdtest.AddPrefix("/imagestreams/default/somerepo"), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), initialRepo))
	if err != nil {
...
func TestAddExistingImageWithNewTag(t *testing.T) {
	...
	existingRepo := &imageapi.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: "somerepo", Namespace: "default"}, Spec: imageapi.ImageStreamSpec{DockerImageRepository: "localhost:5000/default/somerepo"}, Status: imageapi.ImageStreamStatus{Tags: map[string]imageapi.TagEventList{"existingTag": {Items: []imageapi.TagEvent{{DockerImageReference: "localhost:5000/somens/somerepo@" + imageID}}}}}}
	existingImage := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: imageID}, DockerImageReference: "localhost:5000/somens/somerepo@" + imageID, DockerImageMetadata: imageapi.DockerImage{Config: &imageapi.DockerConfig{Cmd: []string{"ls", "/"}, Env: []string{"a=1"}, ExposedPorts: map[string]struct{}{"1234/tcp": {}}, Memory: 1234, CPUShares: 99, WorkingDir: "/workingDir"}}}
	client, server, storage := setup(t)
	defer server.Terminate(t)
	_, err := client.Put(context.TODO(), etcdtest.AddPrefix("/imagestreams/default/somerepo"), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), existingRepo))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	_, err = client.Put(context.TODO(), etcdtest.AddPrefix("/images/"+imageID), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), existingImage))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	mapping := imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: "somerepo"}, Image: *existingImage, Tag: "latest"}
...
func TestAddExistingImageOverridingDockerImageReference(t *testing.T) {
	...
	defer _logClusterCodePath()
	imageID := "sha256:8d812da98d6dd61620343f1a5bf6585b34ad6ed16e5c5f7c7216a525d6aeb772"
	newRepo := &imageapi.ImageStream{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "newrepo"}, Spec: imageapi.ImageStreamSpec{DockerImageRepository: "localhost:5000/default/newrepo"}, Status: imageapi.ImageStreamStatus{DockerImageRepository: "localhost:5000/default/newrepo"}}
	existingImage := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: imageID, Annotations: map[string]string{imageapi.ManagedByOpenShiftAnnotation: "true"}}, DockerImageReference: "localhost:5000/someproject/somerepo@" + imageID, DockerImageMetadata: imageapi.DockerImage{Config: &imageapi.DockerConfig{Cmd: []string{"ls", "/"}, Env: []string{"a=1"}, ExposedPorts: map[string]struct{}{"1234/tcp": {}}, Memory: 1234, CPUShares: 99, WorkingDir: "/workingDir"}}}
	client, server, storage := setup(t)
	defer server.Terminate(t)
	_, err := client.Put(context.TODO(), etcdtest.AddPrefix("/imagestreams/default/newrepo"), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), newRepo))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	_, err = client.Put(context.TODO(), etcdtest.AddPrefix("/images/"+imageID), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), existingImage))
	...
	mapping := imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Name: "newrepo"}, Image: *existingImage, Tag: "latest"}
	ctx := apirequest.NewDefaultContext()
	_, err = storage.Create(ctx, &mapping, rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Unexpected error creating mapping: %#v", err)
...
func TestAddExistingImageAndTag(t *testing.T) {
	...
	_, err = client.Put(context.TODO(), etcdtest.AddPrefix("/images/default/existingImage"), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), existingImage))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	mapping := imageapi.ImageStreamMapping{Image: *existingImage, Tag: "existingTag"}
...
func TestTrackingTags(t *testing.T) {
	...
	client, server, storage := setup(t)
	defer server.Terminate(t)
	stream := &imageapi.ImageStream{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "stream"}, Spec: imageapi.ImageStreamSpec{Tags: map[string]imageapi.TagReference{"tracking": {From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "2.0"}}, "tracking2": {From: &kapi.ObjectReference{Kind: "ImageStreamTag", Name: "2.0"}}}}, Status: imageapi.ImageStreamStatus{Tags: map[string]imageapi.TagEventList{"tracking": {Items: []imageapi.TagEvent{{DockerImageReference: "foo/bar@sha256:1234", Image: "1234"}}}, "nontracking": {Items: []imageapi.TagEvent{{DockerImageReference: "bar/baz@sha256:9999", Image: "9999"}}}, "2.0": {Items: []imageapi.TagEvent{{DockerImageReference: "foo/bar@sha256:1234", Image: "1234"}}}}}}
	_, err := client.Put(context.TODO(), etcdtest.AddPrefix("/imagestreams/default/stream"), runtime.EncodeOrDie(legacyscheme.Codecs.LegacyCodec(imagev1.SchemeGroupVersion), stream))
	if err != nil {
		t.Fatalf("Unexpected error: %v", err)
	}
	image := &imageapi.Image{ObjectMeta: metav1.ObjectMeta{Name: "sha256:503c75e8121369581e5e5abe57b5a3f12db859052b217a8ea16eb86f4b5561a1"}, DockerImageReference: "foo/bar@sha256:503c75e8121369581e5e5abe57b5a3f12db859052b217a8ea16eb86f4b5561a1"}
	mapping := imageapi.ImageStreamMapping{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "stream"}, Image: *image, Tag: "2.0"}
	ctx := apirequest.WithUser(apirequest.NewDefaultContext(), &user.DefaultInfo{})
	_, err = storage.Create(ctx, &mapping, rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	if err != nil {
		t.Fatalf("Unexpected error creating mapping: %v", err)
...
func TestCreateRetryUnrecoverable(t *testing.T) {
	...
	restInstance := &REST{strategy: NewStrategy(registry), imageRegistry: &fakeImageRegistry{createImage: func(ctx context.Context, image *imageapi.Image) error {
		return nil
	}}, imageStreamRegistry: &fakeImageStreamRegistry{getImageStream: func(ctx context.Context, id string, options *metav1.GetOptions) (*imageapi.ImageStream, error) {
...
func TestCreateRetryConflictNoTagDiff(t *testing.T) {
	...
	restInstance := &REST{strategy: NewStrategy(registry), imageRegistry: &fakeImageRegistry{createImage: func(ctx context.Context, image *imageapi.Image) error {
		...
		stream.Status = imageapi.ImageStreamStatus{Tags: map[string]imageapi.TagEventList{"latest": {Items: []imageapi.TagEvent{{DockerImageReference: "localhost:5000/someproject/somerepo:original"}}}}}
		return stream, nil
	}, updateImageStreamStatus: func(ctx context.Context, repo *imageapi.ImageStream) (*imageapi.ImageStream, error) {
		if firstUpdate {
			firstUpdate = false
...
func TestCreateRetryConflictTagDiff(t *testing.T) {
	...
	firstGet := true
	firstUpdate := true
	restInstance := &REST{strategy: NewStrategy(registryhostname.TestingRegistryHostnameRetriever(nil, "", testDefaultRegistryURL)), imageRegistry: &fakeImageRegistry{createImage: func(ctx context.Context, image *imageapi.Image) error {
		return nil
	}}, imageStreamRegistry: &fakeImageStreamRegistry{getImageStream: func(ctx context.Context, id string, options *metav1.GetOptions) (*imageapi.ImageStream, error) {
		if firstGet {
			firstGet = false
		...
		stream := validImageStream()
		stream.Status = imageapi.ImageStreamStatus{Tags: map[string]imageapi.TagEventList{"latest": {Items: []imageapi.TagEvent{{DockerImageReference: "localhost:5000/someproject/somerepo:newer"}}}}}
		return stream, nil
	}, updateImageStreamStatus: func(ctx context.Context, repo *imageapi.ImageStream) (*imageapi.ImageStream, error) {
		if firstUpdate {
			firstUpdate = false
