...
func getMarkerScanners(logsCommandName, securityPolicyCommandFormat, setProbeCommandName string, forbiddenResources sets.String) []osgraph.MarkerScanner {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return []osgraph.MarkerScanner{func(g osgraph.Graph, f osgraph.Namer) []osgraph.Marker {
		return kubeanalysis.FindRestartingPods(g, f, logsCommandName, securityPolicyCommandFormat)
	}, kubeanalysis.FindDuelingReplicationControllers, func(g osgraph.Graph, f osgraph.Namer) []osgraph.Marker {
		if forbiddenResources.Has("secrets") {
			return []osgraph.Marker{}
		}
		return kubeanalysis.FindMissingSecrets(g, f)
	}, kubeanalysis.FindHPASpecsMissingCPUTargets, kubeanalysis.FindOverlappingHPAs, buildanalysis.FindUnpushableBuildConfigs, buildanalysis.FindCircularBuilds, buildanalysis.FindPendingTags, appsanalysis.FindDeploymentConfigTriggerErrors, appsanalysis.FindPersistentVolumeClaimWarnings, buildanalysis.FindMissingInputImageStreams, func(g osgraph.Graph, f osgraph.Namer) []osgraph.Marker {
	...
	}, routeanalysis.FindPortMappingIssues, routeanalysis.FindMissingTLSTerminationType, routeanalysis.FindPathBasedPassthroughRoutes, routeanalysis.FindRouteAdmissionFailures, routeanalysis.FindMissingRouter}
}
...
type formatter interface{ ResourceName(obj interface{}) string }
...
func (e exposedRoutes) Swap(i, j int) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	e[i], e[j] = e[j], e[i]
}
