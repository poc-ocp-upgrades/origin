...
func TestBuildTemplates(t *testing.T) {
	...
	tests := map[string]struct {
		...
		namespace	string
		parms		map[string]string
	}{"simple": {templateName: "first-stored-template", namespace: "default", parms: map[string]string{}}}
	for n, c := range tests {
		appCfg := AppConfig{}
		...
		appCfg.Builder = resource.NewFakeBuilder(func(version schema.GroupVersion) (resource.RESTClient, error) {
		...
		appCfg.SetOpenShiftClient(imageFake.ImageV1(), templateFake.TemplateV1(), routeFake.RouteV1(), c.namespace, nil)
...
func TestBuildPipelinesWithUnresolvedImage(t *testing.T) {
	...
	sourceRepo.SetInfo(&app.SourceRepositoryInfo{Dockerfile: dockerFile})
	refs := app.ComponentReferences{app.ComponentReference(&app.ComponentInput{Value: "mysql", Uses: sourceRepo, ExpectToBuild: true, ResolvedMatch: &app.ComponentMatch{Value: "mysql"}})}
	a := AppConfig{}
	a.Out = &bytes.Buffer{}
...
func TestBuildOutputCycleResilience(t *testing.T) {
	...
	malOutputBC := &buildv1.BuildConfig{ObjectMeta: metav1.ObjectMeta{Name: "buildCfgWithWeirdOutputObjectRef"}, Spec: buildv1.BuildConfigSpec{CommonSpec: buildv1.CommonSpec{Source: buildv1.BuildSource{Dockerfile: &dfn}, Strategy: buildv1.BuildStrategy{DockerStrategy: &buildv1.DockerBuildStrategy{From: &corev1.ObjectReference{Kind: "ImageStreamTag", Name: "mockimagestream:latest"}}}, Output: buildv1.BuildOutput{To: &corev1.ObjectReference{Kind: "NewTypeOfRef", Name: "Yet-to-be-implemented"}}}}}
