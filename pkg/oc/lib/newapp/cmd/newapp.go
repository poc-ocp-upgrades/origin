...
func (c *AppConfig) DockerRegistrySearcher() app.Searcher {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return app.DockerRegistrySearcher{Client: dockerregistry.NewClient(30*time.Second, true), AllowInsecure: c.InsecureRegistry}
}
...
func (c *AppConfig) SetOpenShiftClient(imageClient imagev1typedclient.ImageV1Interface, templateClient templatev1typedclient.TemplateV1Interface, routeClient routev1typedclient.RouteV1Interface, OriginNamespace string, dockerclient *docker.Client) {
	...
	c.ImageStreamSearcher = app.ImageStreamSearcher{Client: c.ImageClient, Namespaces: namespaces, AllowMissingTags: c.AllowMissingImageStreamTags}
	c.ImageStreamByAnnotationSearcher = app.NewImageStreamByAnnotationSearcher(c.ImageClient, c.ImageClient, namespaces)
	c.TemplateSearcher = app.TemplateSearcher{Client: c.TemplateClient, Namespaces: namespaces}
	c.TemplateFileSearcher = &app.TemplateFileSearcher{Builder: c.Builder, Namespace: OriginNamespace}
	c.DockerSearcher = app.DockerClientSearcher{Client: dockerclient, Insecure: c.InsecureRegistry, AllowMissingImages: c.AllowMissingImages, RegistrySearcher: app.ImageImportSearcher{Client: c.ImageClient.ImageStreamImports(OriginNamespace), AllowInsecure: c.InsecureRegistry, Fallback: c.DockerRegistrySearcher()}}
}
func (c *AppConfig) tryToAddEnvironmentArguments(s string) bool {
	...
	if rc {
		...
		c.EnvironmentClassificationErrors[s] = ArgumentClassificationError{Key: "is not an environment variable", Value: nil}
	}
	return rc
}
func (c *AppConfig) tryToAddSourceArguments(s string) bool {
	...
	c.SourceClassificationErrors[s] = ArgumentClassificationError{Key: "is not a Git repository", Value: fmt.Errorf(errStr)}
	return false
}
func (c *AppConfig) tryToAddComponentArguments(s string) bool {
	...
	c.ComponentClassificationErrors[s] = ArgumentClassificationError{Key: "is not an image reference, image~source reference, nor template loaded in an accessible project", Value: err}
	return false
}
func (c *AppConfig) tryToAddTemplateArguments(s string) bool {
	...
	if err != nil {
		c.TemplateClassificationErrors[s] = ArgumentClassificationError{Key: "is not a template stored in a local file", Value: err}
	}
	return false
}
...
func (c *AppConfig) buildPipelines(components app.ComponentReferences, environment app.Environment, buildEnvironment app.Environment) (app.PipelineGroup, error) {
	...
	if len(c.BuildArgs) > 0 {
		DockerStrategyOptions = &buildv1.DockerStrategyOptions{BuildArgs: buildArgs}
	}
	numDockerBuilds := 0
...
type fakeSecretAccessor struct{ token string }
...
func (c *AppConfig) installComponents(components app.ComponentReferences, env app.Environment) ([]runtime.Object, string, error) {
	...
	if token != nil && token.ServiceAccount {
		if _, err := c.KubeClient.CoreV1().ServiceAccounts(c.OriginNamespace).Get(serviceAccountName, metav1.GetOptions{}); err != nil {
			if kerrors.IsNotFound(err) {
				objects = append(objects, &corev1.ServiceAccount{TypeMeta: metav1.TypeMeta{APIVersion: corev1.SchemeGroupVersion.String(), Kind: "ServiceAccount"}, ObjectMeta: metav1.ObjectMeta{Name: serviceAccountName}}, &authv1.RoleBinding{TypeMeta: metav1.TypeMeta{APIVersion: authv1.SchemeGroupVersion.String(), Kind: "RoleBinding"}, ObjectMeta: metav1.ObjectMeta{Name: "installer-role-binding"}, Subjects: []corev1.ObjectReference{{Kind: "ServiceAccount", Name: serviceAccountName}}, RoleRef: corev1.ObjectReference{Name: "edit"}})
			}
		}
	}
	...
	for i := range objects {
		outil.AddObjectAnnotations(objects[i], map[string]string{GeneratedForJob: "true", GeneratedForJobFor: input.String()})
	}
...
func (c *AppConfig) RunQuery() (*QueryResult, error) {
	...
	return &QueryResult{Matches: matches, List: &kapi.List{Items: objects}}, nil
}
...
func (c *AppConfig) Run() (*AppResult, error) {
	...
	if len(installables) > 0 {
		return &AppResult{List: &kapi.List{Items: installables}, Name: name, Namespace: c.OriginNamespace, GeneratedJobs: true}, nil
	}
	pipelines, err := c.buildPipelines(components.ImageComponentRefs(), env, buildenv)
	if err != nil {
		return nil, err
	}
	acceptors := app.Acceptors{app.NewAcceptUnique(c.Typer), app.AcceptNew, app.NewAcceptNonExistentImageStream(c.Typer, c.ImageClient, c.OriginNamespace), app.NewAcceptNonExistentImageStreamTag(c.Typer, c.ImageClient, c.OriginNamespace)}
	objects := app.Objects{}
	...
	return &AppResult{List: &kapi.List{Items: objects}, Name: name, HasSource: len(repositories) != 0, Namespace: c.OriginNamespace}, nil
}
...
func (c *AppConfig) crossStreamInputToOutputTagReference(instream, outstream *imagev1.ImageStream, intag, outtag string, objects app.Objects) bool {
	...
	for {
		if instream.ObjectMeta.Namespace == outstream.ObjectMeta.Namespace && instream.ObjectMeta.Name == outstream.ObjectMeta.Name && intag == outtag {
			return true
		}
...
func (c *AppConfig) followRefToDockerImage(ref *corev1.ObjectReference, isContext *imagev1.ImageStream, objects app.Objects) (*corev1.ObjectReference, error) {
	...
	if target == nil {
		...
		return &corev1.ObjectReference{Kind: "DockerImage", Name: imageName}, nil
	}
...
func (c *AppConfig) HasArguments() bool {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return len(c.Components) > 0 || len(c.ImageStreams) > 0 || len(c.DockerImages) > 0 || len(c.Templates) > 0 || len(c.TemplateFiles) > 0
}
