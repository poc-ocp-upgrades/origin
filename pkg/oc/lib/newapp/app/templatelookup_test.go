...
func testTemplateClient(templates *templatev1.TemplateList) templatev1client.TemplateV1Interface {
	...
	fake.AddReactor("list", "templates", func(action clientgotesting.Action) (handled bool, ret runtime.Object, err error) {
		if len(action.GetNamespace()) > 0 {
			matchingTemplates := &templatev1.TemplateList{Items: []templatev1.Template{}}
...
func TestTemplateSearcher(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	testData := map[string][]string{"openshift": {"nodejs-mongodb-example", "rails-postgresql-example", "jenkins-ephemeral", "my-jenkins"}, "mynamespace": {"my-jenkins"}}
	tests := []struct {
		value		string
		expectedMatches	int
		expectedErr	bool
	}{{value: "jenkins", expectedMatches: 3}, {value: "my-jenkins", expectedMatches: 2}, {value: "jenkins-ephemeral", expectedMatches: 1}, {value: "openshift/my-jenkins", expectedMatches: 1}, {value: "openshift/jenkins", expectedMatches: 2}, {value: "foobar", expectedMatches: 0}, {value: "openshift/foobar", expectedMatches: 0}, {value: "mynamespace/jenkins-ephemeral", expectedMatches: 0}, {value: "mynamespace/jenkins-ephemeral", expectedMatches: 0}, {value: "foo/bar/zee", expectedErr: true}}
	templates := fakeTemplates(testData)
	client := testTemplateClient(templates)
	searcher := &TemplateSearcher{Client: client, Namespaces: []string{"openshift", "mynamespace"}}
...
func fakeTemplates(testData map[string][]string) *templatev1.TemplateList {
	_logClusterCodePath()
	defer _logClusterCodePath()
	templates := &templatev1.TemplateList{Items: []templatev1.Template{}}
	for namespace, templateNames := range testData {
		for _, templateName := range templateNames {
			template := &templatev1.Template{ObjectMeta: metav1.ObjectMeta{Name: templateName, Namespace: namespace}}
