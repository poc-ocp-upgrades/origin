...
func fakeDeploymentConfig(name string, containers ...containerDesc) *appsv1.DeploymentConfig {
	...
	for _, c := range containers {
		...
		for _, p := range c.ports {
			container.Ports = append(container.Ports, corev1.ContainerPort{Name: fmt.Sprintf("port-%d-%s", p.port, p.protocol), ContainerPort: int32(p.port), Protocol: corev1.Protocol(p.protocol)})
		}
		specContainers = append(specContainers, container)
	}
	return &appsv1.DeploymentConfig{ObjectMeta: metav1.ObjectMeta{Name: name}, Spec: appsv1.DeploymentConfigSpec{Replicas: 1, Selector: map[string]string{"name": "test"}, Template: &corev1.PodTemplateSpec{Spec: corev1.PodSpec{Containers: specContainers}}}}
}
...
func TestAcceptUnique(t *testing.T) {
	...
	tests := []struct {
		...
		objs	[]runtime.Object
		expect	int
	}{{name: "same name, different kind, different ns", objs: objs(is("aaa", "ns1"), is("aaa", "ns2"), dc("aaa", "ns1")), expect: 3}, {name: "dup name, empty ns", objs: objs(is("aaa", ""), is("aaa", "")), expect: 1}, {name: "different name, empty ns", objs: objs(is("aaa", ""), is("bbb", ""), dc("aaa", "")), expect: 3}}
...
func TestAddServices(t *testing.T) {
	...
	}{{name: "single port", input: Objects{fakeDeploymentConfig("singleport", containerDesc{"test", []portDesc{{100, "tcp"}}})}, expectedServices: Objects{expectedService("singleport", portDesc{100, "tcp"})}}, {name: "multiple containers", input: Objects{fakeDeploymentConfig("multicontainer", containerDesc{"test1", []portDesc{{100, "tcp"}}}, containerDesc{"test2", []portDesc{{200, "udp"}}})}, expectedServices: Objects{expectedService("multicontainer", portDesc{100, "tcp"}, portDesc{200, "udp"})}}, {name: "duplicate ports", input: Objects{fakeDeploymentConfig("dupports", containerDesc{"test1", []portDesc{{80, "tcp"}, {25, "tcp"}}}, containerDesc{"test2", []portDesc{{80, "tcp"}}})}, expectedServices: Objects{expectedService("dupports", portDesc{25, "tcp"}, portDesc{80, "tcp"})}}, {name: "multiple deployment configs", input: Objects{fakeDeploymentConfig("multidc1", containerDesc{"test1", []portDesc{{100, "tcp"}, {200, "udp"}}}, containerDesc{"test2", []portDesc{{300, "tcp"}}}), fakeDeploymentConfig("multidc2", containerDesc{"dc2_test1", []portDesc{{300, "tcp"}}}, containerDesc{"dc2_test2", []portDesc{{200, "udp"}, {300, "tcp"}}})}, expectedServices: Objects{expectedService("multidc1", portDesc{100, "tcp"}, portDesc{200, "udp"}, portDesc{300, "tcp"}), expectedService("multidc2", portDesc{200, "udp"}, portDesc{300, "tcp"})}}, {name: "first only", input: Objects{fakeDeploymentConfig("firstonly", containerDesc{"test1", []portDesc{{80, "tcp"}, {25, "tcp"}, {100, "udp"}}})}, expectedServices: Objects{expectedService("firstonly", portDesc{25, "tcp"})}, firstOnly: true}}
	for _, test := range tests {
		...
		if !reflect.DeepEqual(services, test.expectedServices) {
			t.Errorf("%s: did not get expected output: %s", test.name, diff.ObjectGoPrintDiff(test.expectedServices, services))
		}
	}
}
