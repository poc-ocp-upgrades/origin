...
func newTestLDAPInterface(client ldap.Client) *LDAPInterface {
	...
	userQuery := ldaputil.LDAPQueryOnAttribute{LDAPQuery: ldaputil.LDAPQuery{BaseDN: "ou=users,dc=example,dc=com", Scope: ldaputil.ScopeWholeSubtree, DerefAliases: ldaputil.DerefAliasesAlways, TimeLimit: 0, Filter: "objectClass=inetOrgPerson"}, QueryAttribute: "dn"}
	userNameAttributes := []string{"cn"}
	errorHandler := syncerror.NewCompoundHandler(syncerror.NewMemberLookupOutOfBoundsSuppressor(ioutil.Discard), syncerror.NewMemberLookupMemberNotFoundSuppressor(ioutil.Discard))
	return NewLDAPInterface(testclient.NewConfig(client), groupQuery, groupNameAttributes, groupMembershipAttributes, userQuery, userNameAttributes, errorHandler)
}
...
func TestGroupEntryFor(t *testing.T) {
	...
	}{{name: "cached get", cacheSeed: map[string]*ldap.Entry{"cn=testGroup,ou=groups,dc=example,dc=com": newTestGroup("testGroup", "cn=testUser,ou=users,dc=example,dc=com")}, expectedError: nil, expectedEntry: newTestGroup("testGroup", "cn=testUser,ou=users,dc=example,dc=com")}, {name: "search request failure", queryBaseDNOverride: "dc=foo", expectedError: ldaputil.NewQueryOutOfBoundsError("cn=testGroup,ou=groups,dc=example,dc=com", "dc=foo"), expectedEntry: nil}, {name: "query failure", client: testclient.NewMatchingSearchErrorClient(testclient.New(), "cn=testGroup,ou=groups,dc=example,dc=com", errors.New("generic search error")), expectedError: errors.New("generic search error"), expectedEntry: nil}, {name: "no errors", client: testclient.NewDNMappingClient(testclient.New(), map[string][]*ldap.Entry{"cn=testGroup,ou=groups,dc=example,dc=com": {newTestGroup("testGroup", "cn=testUser,ou=users,dc=example,dc=com")}}), expectedError: nil, expectedEntry: newTestGroup("testGroup", "cn=testUser,ou=users,dc=example,dc=com")}}
	for _, testCase := range testCases {
		ldapInterface := newTestLDAPInterface(testCase.client)
...
func TestUserEntryFor(t *testing.T) {
	...
	var testCases = []struct {
		...
		expectedError		error
		expectedEntry		*ldap.Entry
	}{{name: "cached get", cacheSeed: map[string]*ldap.Entry{"cn=testUser,ou=users,dc=example,dc=com": newTestUser("testUser")}, expectedError: nil, expectedEntry: newTestUser("testUser")}, {name: "search request failure", queryBaseDNOverride: "dc=foo", expectedError: ldaputil.NewQueryOutOfBoundsError("cn=testUser,ou=users,dc=example,dc=com", "dc=foo"), expectedEntry: nil}, {name: "query failure", client: testclient.NewMatchingSearchErrorClient(testclient.New(), "cn=testUser,ou=users,dc=example,dc=com", errors.New("generic search error")), expectedError: errors.New("generic search error"), expectedEntry: nil}, {name: "no errors", client: testclient.NewDNMappingClient(testclient.New(), map[string][]*ldap.Entry{"cn=testUser,ou=users,dc=example,dc=com": {newTestUser("testUser")}}), expectedError: nil, expectedEntry: newTestUser("testUser")}}
	for _, testCase := range testCases {
		ldapInterface := newTestLDAPInterface(testCase.client)
		if len(testCase.cacheSeed) > 0 {
