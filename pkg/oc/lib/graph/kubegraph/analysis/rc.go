	...
	for _, uncastRCNode := range g.NodesByKind(kubegraph.ReplicationControllerNodeKind) {
		...
		for _, uncastPodNode := range g.PredecessorNodesByEdgeKind(rcNode, appsgraph.ManagedByControllerEdgeKind) {
			...
			if len(uncastOwningRCs) > 1 {
				...
				markers = append(markers, osgraph.Marker{Node: rcNode, RelatedNodes: relatedNodes, Severity: osgraph.WarningSeverity, Key: DuelingReplicationControllerWarning, Message: fmt.Sprintf("%s is competing for %s with %s", f.ResourceName(rcNode), f.ResourceName(podNode), strings.Join(involvedRCNames, ", "))})
			}
		}
	}
	return markers
