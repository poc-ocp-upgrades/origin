...
func TestNewAppRunFailure(t *testing.T) {
	...
	tests := map[string]struct {
		...
		expectedErr	string
	}{"list_and_search": {config: &newcmd.AppConfig{AsList: true, AsSearch: true}, expectedErr: "--list and --search can't be used together"}, "list_with_arguments": {config: &newcmd.AppConfig{AsList: true, ComponentInputs: newcmd.ComponentInputs{Templates: []string{"test"}}}, expectedErr: "--list can't be used with arguments"}, "list_no_matches": {config: &newcmd.AppConfig{AsList: true}, expectedErr: "no matches found"}, "search_with_source_code": {config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Components: []string{"mysql"}, SourceRepositories: []string{"https://github.com/openshift/ruby-hello-world"}}}, expectedErr: "--search can't be used with source code"}, "search_with_env": {config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Components: []string{"mysql"}}, GenerationInputs: newcmd.GenerationInputs{Environment: []string{"FOO=BAR"}}}, expectedErr: "--search can't be used with --env"}, "search_with_build_env": {config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Components: []string{"mysql"}}, GenerationInputs: newcmd.GenerationInputs{BuildEnvironment: []string{"FOO=BAR"}}}, expectedErr: "--search can't be used with --build-env"}, "search_with_param": {config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Components: []string{"mysql"}}, GenerationInputs: newcmd.GenerationInputs{TemplateParameters: []string{"FOO=BAR"}}}, expectedErr: "--search can't be used with --param"}, "search_without_argument": {config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{DockerImages: []string{""}}}, expectedErr: "no matches found"}, "without_args": {config: &newcmd.AppConfig{}, expectedErr: "You must specify one or more images, image streams, templates, or source code locations to create an application."}}
	opts := &AppOptions{ObjectGeneratorOptions: &ObjectGeneratorOptions{BaseName: "oc", CommandName: NewAppRecommendedCommandName}}
...
func TestNewAppRunQueryActions(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	type testAction struct{ namespace, verb, resource string }
	tests := []struct {
		name				string
		config				*newcmd.AppConfig
	...
	}{{name: "list", config: &newcmd.AppConfig{AsList: true}, expectedActions: []testAction{{namespace: "openshift", verb: "list", resource: "imagestreams"}, {namespace: "openshift", verb: "list", resource: "templates"}}, expectedDockerVisited: true}, {name: "search dockerimage", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{DockerImages: []string{"repo/test"}}}, expectedActions: []testAction{}, expectedDockerVisited: true}, {name: "search template", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Templates: []string{"test"}}}, expectedActions: []testAction{{namespace: "openshift", verb: "list", resource: "templates"}}}, {name: "search imagestream", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{ImageStreams: []string{"testimage"}}}, expectedActions: []testAction{{namespace: "openshift", verb: "list", resource: "imagestreams"}}}, {name: "search templatefiles", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{TemplateFiles: []string{"testfile"}}}, expectedTemplateFilesVisited: true}, {name: "search all", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{TemplateFiles: []string{"testfile"}, DockerImages: []string{"repo/test"}, Templates: []string{"test"}, ImageStreams: []string{"testimage"}}}, expectedActions: []testAction{{namespace: "openshift", verb: "list", resource: "imagestreams"}, {namespace: "openshift", verb: "list", resource: "templates"}}, expectedDockerVisited: true, expectedTemplateFilesVisited: true}, {name: "search template failure", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{Templates: []string{"non-exist-template"}}}, expectedActions: []testAction{{namespace: "openshift", verb: "list", resource: "templates"}}, expectedErr: "no matches found"}, {name: "search imagestream failure", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{ImageStreams: []string{"#@@#%*"}}}, expectedErr: "no matches found"}, {name: "search dockerimage failure", config: &newcmd.AppConfig{AsSearch: true, ComponentInputs: newcmd.ComponentInputs{DockerImages: []string{"fakerepo/non-exist-image"}}}, expectedDockerVisited: true, expectedActions: []testAction{}, expectedErr: "no matches found"}}
	o := &AppOptions{ObjectGeneratorOptions: &ObjectGeneratorOptions{Action: configcmd.BulkAction{IOStreams: genericclioptions.NewTestIOStreamsDiscard()}, BaseName: "oc", CommandName: NewAppRecommendedCommandName}}
	for _, test := range tests {
		...
		o.Config.DockerSearcher = MockSearcher{OnSearch: func(precise bool, terms ...string) (app.ComponentMatches, []error) {
			dockerVisited = true
			match := &app.ComponentMatch{Name: "repo/test", DockerImage: &dockerv10.DockerImage{}}
		...
		o.Config.TemplateFileSearcher = MockSearcher{OnSearch: func(precise bool, terms ...string) (app.ComponentMatches, []error) {
			tfVisited = true
			match := &app.ComponentMatch{Template: &templatev1.Template{ObjectMeta: metav1.ObjectMeta{Name: "testfile", Namespace: "openshift"}}}
			return app.ComponentMatches{match}, []error{}
		}}
...
func fakeImagestreamList() *imagev1.ImageStreamList {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &imagev1.ImageStreamList{Items: []imagev1.ImageStream{{ObjectMeta: metav1.ObjectMeta{Name: "testimage", Namespace: "openshift"}}}}
}
