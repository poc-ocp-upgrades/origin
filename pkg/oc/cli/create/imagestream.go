...
func NewCmdCreateImageStream(name, fullName string, f genericclioptions.RESTClientGetter, streams genericclioptions.IOStreams) *cobra.Command {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	o := &CreateImageStreamOptions{CreateSubcommandOptions: NewCreateSubcommandOptions(streams)}
	cmd := &cobra.Command{Use: name + " NAME", Short: "Create a new empty image stream.", Long: imageStreamLong, Example: fmt.Sprintf(imageStreamExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *CreateImageStreamOptions) Run() error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	imageStream := &imagev1.ImageStream{TypeMeta: metav1.TypeMeta{APIVersion: imagev1.SchemeGroupVersion.String(), Kind: "ImageStream"}, ObjectMeta: metav1.ObjectMeta{Name: o.CreateSubcommandOptions.Name}, Spec: imagev1.ImageStreamSpec{LookupPolicy: imagev1.ImageLookupPolicy{Local: o.LookupLocal}}}
