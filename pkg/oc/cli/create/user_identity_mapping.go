...
func NewCmdCreateUserIdentityMapping(name, fullName string, f genericclioptions.RESTClientGetter, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name + " <IDENTITY_NAME> <USER_NAME>", Short: "Manually map an identity to a user.", Long: userIdentityMappingLong, Example: fmt.Sprintf(userIdentityMappingExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *CreateUserIdentityMappingOptions) Run() error {
	_logClusterCodePath()
	defer _logClusterCodePath()
	mapping := &userv1.UserIdentityMapping{TypeMeta: metav1.TypeMeta{APIVersion: userv1.SchemeGroupVersion.String(), Kind: "UserIdentityMapping"}, Identity: corev1.ObjectReference{Name: o.Identity}, User: corev1.ObjectReference{Name: o.User}}
	var err error
	if !o.CreateSubcommandOptions.DryRun {
