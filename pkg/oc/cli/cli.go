...
func NewOcCommand(name, fullName string, in io.Reader, out, errout io.Writer) *cobra.Command {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	cmds := &cobra.Command{Use: name, Short: "Command line tools for managing applications", Long: cliLong, Run: func(c *cobra.Command, args []string) {
	...
	groups := ktemplates.CommandGroups{{Message: "Basic Commands:", Commands: []*cobra.Command{loginCmd, requestproject.NewCmdRequestProject(fullName, f, ioStreams), newapp.NewCmdNewApplication(newapp.NewAppRecommendedCommandName, fullName, f, ioStreams), status.NewCmdStatus(status.StatusRecommendedName, fullName, fullName+" "+status.StatusRecommendedName, f, ioStreams), project.NewCmdProject(fullName, f, ioStreams), projects.NewCmdProjects(fullName, f, ioStreams), kubectlwrappers.NewCmdExplain(fullName, f, ioStreams)}}, {Message: "Build and Deploy Commands:", Commands: []*cobra.Command{rollout.NewCmdRollout(fullName, f, ioStreams), rollback.NewCmdRollback(fullName, f, ioStreams), newbuild.NewCmdNewBuild(newbuild.NewBuildRecommendedCommandName, fullName, f, ioStreams), startbuild.NewCmdStartBuild(fullName, f, ioStreams), cancelbuild.NewCmdCancelBuild(cancelbuild.CancelBuildRecommendedCommandName, fullName, f, ioStreams), importimage.NewCmdImportImage(fullName, f, ioStreams), tag.NewCmdTag(fullName, f, ioStreams)}}, {Message: "Application Management Commands:", Commands: []*cobra.Command{kubectlwrappers.NewCmdCreate(fullName, f, ioStreams), kubectlwrappers.NewCmdApply(fullName, f, ioStreams), kubectlwrappers.NewCmdGet(fullName, f, ioStreams), kubectlwrappers.NewCmdDescribe(fullName, f, ioStreams), kubectlwrappers.NewCmdEdit(fullName, f, ioStreams), set.NewCmdSet(fullName, f, ioStreams), kubectlwrappers.NewCmdLabel(fullName, f, ioStreams), kubectlwrappers.NewCmdAnnotate(fullName, f, ioStreams), expose.NewCmdExpose(fullName, f, ioStreams), kubectlwrappers.NewCmdDelete(fullName, f, ioStreams), kubectlwrappers.NewCmdScale(fullName, f, ioStreams), kubectlwrappers.NewCmdAutoscale(fullName, f, ioStreams), secretcmds, serviceaccounts.NewCmdServiceAccounts(serviceaccounts.ServiceAccountsRecommendedName, fullName+" "+serviceaccounts.ServiceAccountsRecommendedName, f, ioStreams)}}, {Message: "Troubleshooting and Debugging Commands:", Commands: []*cobra.Command{logs.NewCmdLogs(logs.LogsRecommendedCommandName, fullName, f, ioStreams), rsh.NewCmdRsh(rsh.RshRecommendedName, fullName, f, ioStreams), rsync.NewCmdRsync(rsync.RsyncRecommendedName, fullName, f, ioStreams), kubectlwrappers.NewCmdPortForward(fullName, f, ioStreams), debug.NewCmdDebug(fullName, f, ioStreams), kubectlwrappers.NewCmdExec(fullName, f, ioStreams), kubectlwrappers.NewCmdProxy(fullName, f, ioStreams), kubectlwrappers.NewCmdAttach(fullName, f, ioStreams), kubectlwrappers.NewCmdRun(fullName, f, ioStreams), kubectlwrappers.NewCmdCp(fullName, f, ioStreams), kubectlwrappers.NewCmdWait(fullName, f, ioStreams)}}, {Message: "Advanced Commands:", Commands: []*cobra.Command{admin.NewCommandAdmin("adm", fullName+" "+"adm", f, ioStreams), kubectlwrappers.NewCmdReplace(fullName, f, ioStreams), kubectlwrappers.NewCmdPatch(fullName, f, ioStreams), process.NewCmdProcess(fullName, f, ioStreams), extract.NewCmdExtract(fullName, f, ioStreams), observe.NewCmdObserve(fullName, f, ioStreams), policy.NewCmdPolicy(policy.PolicyRecommendedName, fullName+" "+policy.PolicyRecommendedName, f, ioStreams), kubectlwrappers.NewCmdAuth(fullName, f, ioStreams), kubectlwrappers.NewCmdConvert(fullName, f, ioStreams), image.NewCmdImage(fullName, f, ioStreams), registry.NewCmd(fullName, f, ioStreams), idle.NewCmdIdle(fullName, f, ioStreams), kubectlwrappers.NewCmdApiVersions(fullName, f, ioStreams), kubectlwrappers.NewCmdApiResources(fullName, f, ioStreams), kubectlwrappers.NewCmdClusterInfo(fullName, f, ioStreams), diff.NewCmdDiff(f, ioStreams)}}, {Message: "Settings Commands:", Commands: []*cobra.Command{logout.NewCmdLogout("logout", fullName+" logout", fullName+" login", f, ioStreams), kubectlwrappers.NewCmdConfig(fullName, "config", f, ioStreams), whoami.NewCmdWhoAmI(whoami.WhoAmIRecommendedCommandName, fullName+" "+whoami.WhoAmIRecommendedCommandName, f, ioStreams), kubectlwrappers.NewCmdCompletion(fullName, ioStreams)}}}
	groups.Add(cmds)
	...
	filters := []string{"options", "deploy", moved(fullName, "logs", cmds, buildlogs.NewCmdBuildLogs(fullName, f, ioStreams)), moved(fullName, "secrets link", secretcmds, secrets.NewCmdLinkSecret("add", fullName, f, ioStreams)), moved(fullName, "create secret", secretcmds, secrets.NewCmdCreateSecret(secrets.NewSecretRecommendedCommandName, fullName, f, ioStreams)), moved(fullName, "create secret", secretcmds, secrets.NewCmdCreateDockerConfigSecret(secrets.CreateDockerConfigSecretRecommendedName, fullName, f, ioStreams, ocSecretsNewFullName, ocEditFullName)), moved(fullName, "create secret", secretcmds, secrets.NewCmdCreateBasicAuthSecret(secrets.CreateBasicAuthSecretRecommendedCommandName, fullName, f, ioStreams, ocSecretsNewFullName, ocEditFullName)), moved(fullName, "create secret", secretcmds, secrets.NewCmdCreateSSHAuthSecret(secrets.CreateSSHAuthSecretRecommendedCommandName, fullName, f, ioStreams, ocSecretsNewFullName, ocEditFullName))}
	changeSharedFlagDefaults(cmds)
	templates.ActsAsRootCommand(cmds, filters, groups...).ExposeFlags(loginCmd, "certificate-authority", "insecure-skip-tls-verify", "token")
	...
	if cmds.Flag("namespace") != nil {
		...
		cmds.Flag("namespace").Annotations[cobra.BashCompCustom] = append(cmds.Flag("namespace").Annotations[cobra.BashCompCustom], "__oc_get_namespaces")
	}
	return cmds
}
...
func newExperimentalCommand(name, fullName string, f kcmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	experimental := &cobra.Command{Use: name, Short: "Experimental commands under active development", Long: "The commands grouped here are under development and may change without notice.", Run: func(c *cobra.Command, args []string) {
	...
	default:
		...
		if strings.HasPrefix(basename, "kubectl-") {
			...
			if targetCmd, _, err := cmd.Find(args); targetCmd != nil && err == nil {
				newParent := &cobra.Command{Use: targetCmd.Use, Short: targetCmd.Short, Long: targetCmd.Long, Example: targetCmd.Example, Run: targetCmd.Run}
