...
func NewRequestProjectOptions(baseName string, streams genericclioptions.IOStreams) *RequestProjectOptions {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &RequestProjectOptions{IOStreams: streams, Name: baseName}
}
func NewCmdRequestProject(baseName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "new-project NAME [--display-name=DISPLAYNAME] [--description=DESCRIPTION]", Short: "Request a new project", Long: requestProjectLong, Example: fmt.Sprintf(requestProjectExample, baseName), Run: func(cmd *cobra.Command, args []string) {
