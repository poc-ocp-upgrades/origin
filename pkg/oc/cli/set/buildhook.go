...
func NewBuildHookOptions(streams genericclioptions.IOStreams) *BuildHookOptions {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &BuildHookOptions{PrintFlags: genericclioptions.NewPrintFlags("hooks updated").WithTypeSetter(scheme.Scheme), IOStreams: streams}
}
func NewCmdBuildHook(fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "build-hook BUILDCONFIG --post-commit [--command] [--script] -- CMD", Short: "Update a build hook on a build config", Long: buildHookLong, Example: fmt.Sprintf(buildHookExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *BuildHookOptions) Run() error {
	...
	if !o.Local {
		b = b.LabelSelectorParam(o.Selector).ResourceNames("buildconfigs", o.Resources...).Latest()
		if o.All {
