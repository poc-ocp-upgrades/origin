...
func fakePodWithVolumeClaim() *corev1.Pod {
	_logClusterCodePath()
	defer _logClusterCodePath()
	fakePod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Namespace: "default", Name: "fakepod"}, Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "fake-container", VolumeMounts: []corev1.VolumeMount{{Name: "fake-mount", MountPath: "/var/www/html"}}}}, Volumes: []corev1.Volume{{Name: "fake-mount", VolumeSource: corev1.VolumeSource{PersistentVolumeClaim: &corev1.PersistentVolumeClaimVolumeSource{ClaimName: "fake-claim"}}}}}}
	return fakePod
}
...
func getFakeMapping() *meta.RESTMapping {
	_logClusterCodePath()
	defer _logClusterCodePath()
	fakeMapping := &meta.RESTMapping{Resource: schema.GroupVersionResource{Group: "test.group", Version: "v1", Resource: "fake-mount"}}
	return fakeMapping
}
...
func TestCreateClaim(t *testing.T) {
	...
	}{{"Create ClaimClass with a value", &AddVolumeOptions{Type: "persistentVolumeClaim", ClaimClass: "foobar", ClassChanged: true, ClaimName: "foo-vol", ClaimSize: "5G", MountPath: "/sandbox"}, "foobar"}, {"Create ClaimClass with an empty value", &AddVolumeOptions{Type: "persistentVolumeClaim", ClaimClass: "", ClassChanged: true, ClaimName: "foo-vol", ClaimSize: "5G", MountPath: "/sandbox"}, ""}, {"Create ClaimClass without a value", &AddVolumeOptions{Type: "persistentVolumeClaim", ClassChanged: false, ClaimName: "foo-vol", ClaimSize: "5G", MountPath: "/sandbox"}, ""}}
...
func TestValidateAddOptions(t *testing.T) {
	...
	tests := []struct {
		...
		addOpts		*AddVolumeOptions
		expectedError	error
	}{{"using existing pvc", &AddVolumeOptions{Type: "persistentVolumeClaim"}, errors.New("must provide --claim-name or --claim-size (to create a new claim) for --type=pvc")}, {"creating new pvc", &AddVolumeOptions{Type: "persistentVolumeClaim", ClaimName: "sandbox-pvc", ClaimSize: "5G"}, nil}, {"error creating pvc with storage class", &AddVolumeOptions{Type: "persistentVolumeClaim", ClaimName: "sandbox-pvc", ClaimClass: "slow"}, errors.New("must provide --claim-size to create new pvc with claim-class")}, {"creating pvc with storage class", &AddVolumeOptions{Type: "persistentVolumeClaim", ClaimName: "sandbox-pvc", ClaimClass: "slow", ClaimSize: "5G"}, nil}, {"creating secret with good default-mode", &AddVolumeOptions{Type: "secret", SecretName: "sandbox-pv", DefaultMode: "0644"}, nil}, {"creating secret with good default-mode, three number variant", &AddVolumeOptions{Type: "secret", SecretName: "sandbox-pv", DefaultMode: "777"}, nil}, {"creating secret with bad default-mode, bad bits", &AddVolumeOptions{Type: "secret", SecretName: "sandbox-pv", DefaultMode: "0888"}, errors.New("--default-mode must be between 0000 and 0777")}, {"creating secret with bad default-mode, too long", &AddVolumeOptions{Type: "secret", SecretName: "sandbox-pv", DefaultMode: "07777"}, errors.New("--default-mode must be between 0000 and 0777")}, {"creating configmap with good default-mode", &AddVolumeOptions{Type: "configmap", ConfigMapName: "sandbox-pv", DefaultMode: "0644"}, nil}, {"creating configmap with good default-mode, three number variant", &AddVolumeOptions{Type: "configmap", ConfigMapName: "sandbox-pv", DefaultMode: "777"}, nil}, {"creating configmap with bad default-mode, bad bits", &AddVolumeOptions{Type: "configmap", ConfigMapName: "sandbox-pv", DefaultMode: "0888"}, errors.New("--default-mode must be between 0000 and 0777")}, {"creating configmap with bad default-mode, too long", &AddVolumeOptions{Type: "configmap", ConfigMapName: "sandbox-pv", DefaultMode: "07777"}, errors.New("--default-mode must be between 0000 and 0777")}}
	for _, testCase := range tests {
