...
func NewCmdBuildSecret(fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "build-secret BUILDCONFIG SECRETNAME", Short: "Update a build secret on a build config", Long: buildSecretLong, Example: fmt.Sprintf(buildSecretExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *BuildSecretOptions) Run() error {
	...
	if !o.Local {
		b = b.ResourceNames("buildconfigs", o.Resources...).LabelSelectorParam(o.Selector).Latest()
		if o.All {
...
func (o *BuildSecretOptions) updateBuildConfig(bc *buildv1.BuildConfig, secret string) {
	...
	if o.Push {
		if o.Remove {
			...
			bc.Spec.Output.PushSecret = &corev1.LocalObjectReference{Name: secret}
		}
	}
	if o.Pull {
		switch {
		case bc.Spec.Strategy.DockerStrategy != nil:
			if o.Remove {
				...
				bc.Spec.Strategy.DockerStrategy.PullSecret = &corev1.LocalObjectReference{Name: secret}
			}
		case bc.Spec.Strategy.SourceStrategy != nil:
			if o.Remove {
				bc.Spec.Strategy.SourceStrategy.PullSecret = nil
			} else {
				bc.Spec.Strategy.SourceStrategy.PullSecret = &corev1.LocalObjectReference{Name: secret}
			}
		case bc.Spec.Strategy.CustomStrategy != nil:
			if o.Remove {
				bc.Spec.Strategy.CustomStrategy.PullSecret = nil
			} else {
				bc.Spec.Strategy.CustomStrategy.PullSecret = &corev1.LocalObjectReference{Name: secret}
			}
		}
	}
	if o.Source {
		if o.Remove {
			...
			bc.Spec.Source.SourceSecret = &corev1.LocalObjectReference{Name: secret}
		}
	}
}
