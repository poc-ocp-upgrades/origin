...
func NewMustGatherOptions(streams genericclioptions.IOStreams) *MustGatherOptions {
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &MustGatherOptions{SourceDir: "/must-gather/", IOStreams: streams}
}
...
func (o *MustGatherOptions) copyFilesFromPod(pod *corev1.Pod) error {
	_logClusterCodePath()
	defer _logClusterCodePath()
	rsyncOptions := &rsync.RsyncOptions{Namespace: pod.Namespace, Source: &rsync.PathSpec{PodName: pod.Name, Path: path.Clean(o.SourceDir) + "/"}, ContainerName: "copy", Destination: &rsync.PathSpec{PodName: "", Path: o.DestDir}, Client: o.Client, Config: o.Config, RshCmd: fmt.Sprintf("%s --namespace=%s", o.RsyncRshCmd, pod.Namespace), IOStreams: o.IOStreams}
