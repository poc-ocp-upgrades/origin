...
func (o *MustGatherOptions) Run(rsyncCmd *cobra.Command) error {
	...
	var err error
	ns, err := o.Client.CoreV1().Namespaces().Create(&corev1.Namespace{ObjectMeta: metav1.ObjectMeta{GenerateName: "openshift-must-gather-", Labels: map[string]string{"openshift.io/run-level": "0"}, Annotations: map[string]string{"oc.openshift.io/command": "oc adm must-gather"}}})
	if err != nil {
		return err
	}
...
func (o *MustGatherOptions) newPod(node string) *corev1.Pod {
	...
	defer _logClusterCodePath()
	zero := int64(0)
	ret := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{GenerateName: "must-gather-", Labels: map[string]string{"app": "must-gather"}}, Spec: corev1.PodSpec{NodeName: node, RestartPolicy: corev1.RestartPolicyNever, Volumes: []corev1.Volume{{Name: "must-gather-output", VolumeSource: corev1.VolumeSource{EmptyDir: &corev1.EmptyDirVolumeSource{}}}}, InitContainers: []corev1.Container{{Name: "gather", Image: o.Image, Command: []string{"/usr/bin/gather"}, VolumeMounts: []corev1.VolumeMount{{Name: "must-gather-output", MountPath: "/must-gather", ReadOnly: false}}}}, Containers: []corev1.Container{{Name: "copy", Image: o.Image, Command: []string{"/bin/bash", "-c", "trap : TERM INT; sleep infinity & wait"}, VolumeMounts: []corev1.VolumeMount{{Name: "must-gather-output", MountPath: "/must-gather", ReadOnly: false}}}}, TerminationGracePeriodSeconds: &zero, Tolerations: []corev1.Toleration{{Operator: "Exists"}}}}
	if len(o.Command) > 0 {
