...
func NewMigrateTTLReferenceOptions(streams genericclioptions.IOStreams) *MigrateTTLReferenceOptions {
	...
	return &MigrateTTLReferenceOptions{IOStreams: streams}
}
func NewCmdMigrateTTLs(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: fmt.Sprintf("%s --etcd-address=HOST --ttl-keys-prefix=PATH", name), Short: "Attach keys to etcd v3 leases to assist in etcd v2 migrations", Long: internalMigrateTTLLong, Example: fmt.Sprintf(internalMigrateTTLExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func generateClientConfig(o *MigrateTTLReferenceOptions) (*clientv3.Config, error) {
	...
	c := &clientv3.Config{Endpoints: []string{o.etcdAddress}, DialTimeout: 5 * time.Second}
...
func (o *MigrateTTLReferenceOptions) Run() error {
	...
	for _, kv := range objectsResp.Kvs {
		...
		txnResp, err := client.KV.Txn(ctx).If(clientv3.Compare(clientv3.ModRevision(string(kv.Key)), "=", kv.ModRevision)).Then(clientv3.OpPut(string(kv.Key), string(kv.Value), clientv3.WithLease(lease.ID))).Commit()
		if err != nil {
