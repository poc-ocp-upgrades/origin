...
func NewCmdPrune(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: fmt.Sprintf("%s [WHITELIST] [--whitelist=WHITELIST-FILE] [--blacklist=BLACKLIST-FILE] --sync-config=CONFIG-SOURCE", name), Short: "Remove old OpenShift groups referencing missing records on an external provider", Long: pruneLong, Example: fmt.Sprintf(pruneExamples, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *PruneOptions) Run() error {
	...
	pruner := &syncgroups.LDAPGroupPruner{Host: clientConfig.Host(), GroupClient: o.GroupClient.Groups(), DryRun: !o.Confirm, Out: o.Out, Err: o.ErrOut}
