...
func Test_contentStream_Read(t *testing.T) {
	...
	tests := []struct {
		...
		wantN	int64
		wantErr	bool
	}{{parts: [][]byte{[]byte("test"), []byte("other"), []byte("a")}, want: "testothera", wantN: 10}, {parts: [][]byte{[]byte("test"), []byte(strings.Repeat("a", 4096))}, want: "test" + strings.Repeat("a", 4096), wantN: 4100}, {parts: nil, want: "", wantN: 0}}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			buf := &bytes.Buffer{}
			s := &contentStream{parts: tt.parts}
