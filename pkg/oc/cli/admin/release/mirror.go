...
func NewMirrorOptions(streams genericclioptions.IOStreams) *MirrorOptions {
	...
	return &MirrorOptions{IOStreams: streams, ParallelOptions: imagemanifest.ParallelOptions{MaxPerRegistry: 6}}
}
...
func (o *MirrorOptions) Run() error {
	...
	if len(o.From) > 0 && !o.SkipRelease {
		...
		if len(o.ToRelease) > 0 {
			...
			mappings = append(mappings, mirror.Mapping{Source: srcRef, Type: dstRef.Type(), Destination: dstRef.Combined(), Name: "release"})
		}
	}
	for i := range is.Spec.Tags {
		...
		mappings = append(mappings, mirror.Mapping{Source: from, Type: dstMirrorRef.Type(), Destination: dstMirrorRef.Combined(), Name: tag.Name})
