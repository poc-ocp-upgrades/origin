...
func NewExtractOptions(streams genericclioptions.IOStreams) *ExtractOptions {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &ExtractOptions{IOStreams: streams, Directory: "."}
}
func NewExtract(f kcmdutil.Factory, parentName string, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "extract", Short: "Extract the contents of an update payload to disk", Long: templates.LongDesc(`
...
func (o *ExtractOptions) Run() error {
	...
	case len(o.File) > 0:
		...
		opts.OnlyFiles = true
		opts.Mappings = []extract.Mapping{{ImageRef: ref, From: "release-manifests/", To: dir}}
		found := false
