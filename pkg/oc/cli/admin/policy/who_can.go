...
func NewWhoCanOptions(streams genericclioptions.IOStreams) *WhoCanOptions {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &WhoCanOptions{PrintFlags: genericclioptions.NewPrintFlags("").WithTypeSetter(scheme.Scheme), IOStreams: streams}
}
func NewCmdWhoCan(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name + " VERB RESOURCE [NAME]", Short: "List who can perform the specified action on a resource", Long: "List who can perform the specified action on a resource", Run: func(cmd *cobra.Command, args []string) {
...
func (o *WhoCanOptions) run() error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	authorizationAttributes := authorizationv1.Action{Verb: o.verb, Group: o.resource.Group, Resource: o.resource.Resource, ResourceName: o.resourceName}
