...
func NewCmdRemoveGroupFromProject(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name + " GROUP [GROUP ...]", Short: "Remove group from the current project", Long: `Remove group from the current project`, Run: func(cmd *cobra.Command, args []string) {
...
func (o *RemoveFromProjectOptions) Run() error {
	...
	updatedBindings := &rbacv1.RoleBindingList{TypeMeta: metav1.TypeMeta{Kind: "List", APIVersion: "v1"}, ListMeta: metav1.ListMeta{}}
