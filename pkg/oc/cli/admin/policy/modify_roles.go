...
func NewCmdAddRoleToGroup(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name + " ROLE GROUP [GROUP ...]", Short: "Add a role to groups for the current project", Long: addRoleToGroupLongDesc, Run: func(cmd *cobra.Command, args []string) {
...
func NewCmdAddRoleToUser(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	o := NewRoleModificationOptions(streams)
	o.SANames = []string{}
	cmd := &cobra.Command{Use: name + " ROLE (USER | -z SERVICEACCOUNT) [USER ...]", Short: "Add a role to users or serviceaccounts for the current project", Long: addRoleToUserLongDesc, Example: fmt.Sprintf(addRoleToUserExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func NewCmdRemoveRoleFromUser(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	o := NewRoleModificationOptions(streams)
	o.SANames = []string{}
	cmd := &cobra.Command{Use: name + " ROLE USER [USER ...]", Short: "Remove a role from users for the current project", Long: `Remove a role from users for the current project`, Run: func(cmd *cobra.Command, args []string) {
		kcmdutil.CheckErr(o.CompleteUserWithSA(f, cmd, args))
		kcmdutil.CheckErr(o.checkRoleBindingNamespace(f))
		kcmdutil.CheckErr(o.RemoveRole())
	}}
...
func NewCmdAddClusterRoleToUser(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	o.RoleKind = "ClusterRole"
	o.SANames = []string{}
	cmd := &cobra.Command{Use: name + " <role> <user | -z serviceaccount> [user]...", Short: "Add a role to users for all projects in the cluster", Long: addClusterRoleToUserLongDesc, Run: func(cmd *cobra.Command, args []string) {
		kcmdutil.CheckErr(o.CompleteUserWithSA(f, cmd, args))
		kcmdutil.CheckErr(o.AddRole())
	}}
...
func NewCmdRemoveClusterRoleFromUser(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	o.RoleKind = "ClusterRole"
	o.SANames = []string{}
	cmd := &cobra.Command{Use: name + " <role> <user> [user]...", Short: "Remove a role from users for all projects in the cluster", Long: `Remove a role from users for all projects in the cluster`, Run: func(cmd *cobra.Command, args []string) {
		kcmdutil.CheckErr(o.CompleteUserWithSA(f, cmd, args))
		kcmdutil.CheckErr(o.RemoveRole())
	}}
	cmd.Flags().StringVar(&o.RoleBindingName, "rolebinding-name", o.RoleBindingName, "Name of the rolebinding to modify. If left empty it will operate on all rolebindings")
...
func (o *RoleModificationOptions) checkRoleBindingNamespace(f kcmdutil.Factory) error {
	...
	if len(o.RoleNamespace) > 0 {
		if o.RoleBindingNamespace != o.RoleNamespace {
			return fmt.Errorf("role binding in namespace %q can't reference role in different namespace %q", o.RoleBindingNamespace, o.RoleNamespace)
		}
		o.RoleKind = "Role"
	} else {
		o.RoleKind = "ClusterRole"
...
func (o *RoleModificationOptions) getRoleBinding() (*roleBindingAbstraction, bool, error) {
	...
	if roleBinding.RoleName() != o.RoleName {
		return nil, false, fmt.Errorf("rolebinding %s found for role %s, not %s", o.RoleBindingName, roleBinding.RoleName(), o.RoleName)
	}
	if roleBinding.RoleKind() != o.RoleKind {
		return nil, false, fmt.Errorf("rolebinding %s found for %q, not %q", o.RoleBindingName, roleBinding.RoleKind(), o.RoleKind)
	}
...
subjectCheck:
	for _, subjectToAdd := range subjectsToAdd {
		for _, newSubject := range newSubjects {
			if newSubject.Kind == subjectToAdd.Kind && newSubject.Name == subjectToAdd.Name && newSubject.Namespace == subjectToAdd.Namespace {
...
func (o *RoleModificationOptions) checkRolebindingAutoupdate(roleBinding *roleBindingAbstraction) {
	...
	if roleBinding.Annotation(rbacv1.AutoUpdateAnnotationKey) == "true" {
		if o.PrintErrf != nil {
			o.PrintErrf("Warning: Your changes may get lost whenever a master"+" is restarted, unless you prevent reconciliation of this"+" rolebinding using the following command: oc annotate"+" %s.rbac %s '%s=false' --overwrite", roleBinding.Type(), roleBinding.Name(), rbacv1.AutoUpdateAnnotationKey)
		}
	}
}
func (o *RoleModificationOptions) roleObjectToPrint() runtime.Object {
	...
	if len(o.RoleBindingNamespace) == 0 {
		...
		roleToPrint = &rbacv1.Role{TypeMeta: metav1.TypeMeta{APIVersion: rbacv1.SchemeGroupVersion.String(), Kind: o.RoleKind}, ObjectMeta: metav1.ObjectMeta{Name: o.RoleName}}
	}
	return roleToPrint
}
func (o *RoleModificationOptions) RemoveRole() error {
	...
	if len(o.RoleBindingName) > 0 {
		...
		if existingRoleBinding.RoleName() != o.RoleName {
			return fmt.Errorf("rolebinding %s contains role %s, instead of role %s", o.RoleBindingName, existingRoleBinding.RoleName(), o.RoleName)
		}
		if existingRoleBinding.RoleKind() != o.RoleKind {
			return fmt.Errorf("rolebinding %s contains role %s of kind %q, not %q", o.RoleBindingName, o.RoleName, existingRoleBinding.RoleKind(), o.RoleKind)
		}
	...
	if o.PrintFlags.OutputFormat != nil && len(*o.PrintFlags.OutputFormat) > 0 {
		...
		if len(o.RoleBindingNamespace) > 0 {
			updatedBindings := &unstructured.UnstructuredList{Object: map[string]interface{}{"kind": "List", "apiVersion": "v1", "metadata": map[string]interface{}{}}}
...
existingLoop:
	for _, existingSubject := range haystack {
		for _, toRemove := range needles {
			if existingSubject.Kind == toRemove.Kind && existingSubject.Name == toRemove.Name && existingSubject.Namespace == toRemove.Namespace {
				found++
