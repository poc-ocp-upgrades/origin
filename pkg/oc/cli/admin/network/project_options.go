...
func (p *ProjectOptions) GetProjects() ([]*projectv1.Project, error) {
	...
	r := p.Builder.WithScheme(scheme.Scheme, scheme.Scheme.PrioritizedVersionsAllGroups()...).ContinueOnError().NamespaceParam(p.DefaultNamespace).LabelSelectorParam(p.Selector).ResourceTypeOrNameArgs(true, nameArgs...).Flatten().Do()
	if r.Err() != nil {
		return nil, r.Err()
	}
	errList := []error{}
...
func (p *ProjectOptions) UpdatePodNetwork(nsName string, action networkapihelpers.PodNetworkAction, args string) error {
	...
	backoff := wait.Backoff{Steps: 15, Duration: 500 * time.Millisecond, Factor: 1.1}
