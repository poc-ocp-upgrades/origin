...
func NewLogsOptions(streams genericclioptions.IOStreams) *LogsOptions {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &LogsOptions{Path: "journal", IOStreams: streams, GrepCaseSensitive: true}
}
func NewCmdLogs(baseName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "node-logs [-l LABELS] [NODE...]", DisableFlagsInUseLine: true, Short: "Display and filter node logs", Long: logsLong, Example: fmt.Sprintf(logsExample, baseName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *LogsOptions) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	builder := f.NewBuilder().WithScheme(scheme.Scheme, scheme.Scheme.PrioritizedVersionsAllGroups()...).SingleResourceType()
...
func (o LogsOptions) RunLogs() error {
	...
	err := result.Visit(func(info *resource.Info, err error) error {
		...
		path := client.Get().Namespace(info.Namespace).Name(info.Name).Resource(mapping.Resource.Resource).SubResource("proxy", "logs").Suffix(o.Path).URL().Path
		if strings.HasSuffix(o.Path, "/") {
			path += "/"
		}
		req := client.Get().RequestURI(path).SetHeader("Accept", "text/plain, */*").SetHeader("Accept-Encoding", "gzip")
		if o.Path == "journal" {
		...
		requests = append(requests, &logRequest{node: info.Name, req: req, raw: o.Raw || o.Path == "journal"})
		return nil
	})
	if err != nil {
	...
	if o.Unify {
		...
		for i := range requests {
			...
			req.skipPrefix = true
			pr, pw := io.Pipe()
			readers = append(readers, Reader{R: pr})
			go func() {
...
func (r mergeReader) WriteTo(out io.Writer) (int64, error) {
	...
	var errs []error
	for _, in := range r {
		buf := &buffer{r: bufio.NewReaderSize(in.R, bufSize), prefix: in.Prefix}
	...
	return n, errors.FilterOut(errors.NewAggregate(errs), func(err error) bool {
		return err == io.EOF
	})
}
