...
func TestPruneTask(t *testing.T) {
	...
	for _, orphans := range []bool{true, false} {
		for _, BuildPhaseOption := range BuildPhaseOptions {
			...
			filter := &andFilter{filterPredicates: []FilterPredicate{NewFilterBeforePredicate(keepYoungerThan)}}
			dataSet := NewDataSet(buildConfigs, filter.Filter(builds))
			resolver := NewPerBuildConfigResolver(dataSet, keepComplete, keepFailed)
			if orphans {
				resolver = &mergeResolver{resolvers: []Resolver{resolver, NewOrphanBuildResolver(dataSet, BuildPhaseFilter)}}
			}
			...
			options := PrunerOptions{KeepYoungerThan: keepYoungerThan, Orphans: orphans, KeepComplete: keepComplete, KeepFailed: keepFailed, BuildConfigs: buildConfigs, Builds: builds}
