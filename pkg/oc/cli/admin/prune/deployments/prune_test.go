...
func TestPruneTask(t *testing.T) {
	...
	for _, orphans := range []bool{true, false} {
		for _, deploymentStatusOption := range deploymentStatusOptions {
			...
			filter := &andFilter{filterPredicates: []FilterPredicate{FilterDeploymentsPredicate, FilterZeroReplicaSize, NewFilterBeforePredicate(keepYoungerThan)}}
			...
			if orphans {
				resolver = &mergeResolver{resolvers: []Resolver{resolver, NewOrphanDeploymentResolver(dataSet, deploymentStatusFilter)}}
			}
			...
			options := PrunerOptions{KeepYoungerThan: keepYoungerThan, Orphans: orphans, KeepComplete: keepComplete, KeepFailed: keepFailed, DeploymentConfigs: deploymentConfigs, Deployments: deployments}
