...
func NewCmdPruneAuth(f kcmdutil.Factory, name string, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name, Short: "Removes references to the specified roles, clusterroles, users, and groups.", Long: "Removes references to the specified roles, clusterroles, users, and groups.  Other types are ignored", Run: func(cmd *cobra.Command, args []string) {
...
func (o *PruneAuthOptions) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	o.Builder = f.NewBuilder().Unstructured().ContinueOnError().NamespaceParam(cmdNamespace).DefaultNamespace().FilenameParam(enforceNamespace, &o.FilenameOptions).LabelSelectorParam(o.Selector).SelectAllParam(o.All).ResourceTypeOrNameArgs(false, args...).RequireObject(false).Flatten()
	return nil
}
