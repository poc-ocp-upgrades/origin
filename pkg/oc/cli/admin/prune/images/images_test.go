...
func TestImagePruneNamespaced(t *testing.T) {
	...
	opts := &PruneImagesOptions{Namespace: "foo", AppsClient: &fakeappsv1client.FakeAppsV1{Fake: &(fakeappsclient.NewSimpleClientset().Fake)}, BuildClient: &fakebuildv1client.FakeBuildV1{Fake: &(fakebuildclient.NewSimpleClientset().Fake)}, ImageClient: imageFake, KubeClient: kFake, Out: ioutil.Discard, ErrOut: os.Stderr}
...
func TestImagePruneErrOnBadReference(t *testing.T) {
	...
	imageFake := &fakeimagev1client.FakeImageV1{Fake: &(fakeimageclient.NewSimpleClientset().Fake)}
	fakeDiscovery := &fakeVersionDiscovery{masterVersion: version.Get()}
	...
	opts := &PruneImagesOptions{AppsClient: &fakeappsv1client.FakeAppsV1{Fake: &(fakeappsclient.NewSimpleClientset().Fake)}, BuildClient: &fakebuildv1client.FakeBuildV1{Fake: &(fakebuildclient.NewSimpleClientset(&bcBad).Fake)}, ImageClient: imageFake, KubeClient: kFake, DiscoveryClient: fakeDiscovery, Timeout: time.Second, Out: ioutil.Discard, ErrOut: errBuf}
	verifyOutput := func(out string, expectClientVersionMismatch bool) {
		...
		expBadRefErrors := sets.NewString(`Pod[foo/pod1]: invalid docker image reference "invalid image reference": invalid reference format`, `BuildConfig[foo/bc1]: invalid ImageStreamImage reference "bar:invalid-digest": expected exactly one @ in the isimage name "bar:invalid-digest"`, `Deployment[foo/dep1]: invalid docker image reference "do not blame me": invalid reference format`)
...
func (f *fakeVersionDiscovery) RESTClient() restclient.Interface {
	...
	return &restfake.RESTClient{NegotiatedSerializer: kubernetesscheme.Codecs, Client: restfake.CreateHTTPClient(func(req *http.Request) (*http.Response, error) {
		...
		return &http.Response{StatusCode: http.StatusOK, Header: header, Body: objBody(&f.masterVersion)}, nil
	})}
}
