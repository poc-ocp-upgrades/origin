...
func NewCommandNewServiceAccountToken(name, fullname string, f cmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	newServiceAccountTokenCommand := &cobra.Command{Use: fmt.Sprintf(newServiceAccountTokenUsage, name), Short: newServiceAccountTokenShort, Long: newServiceAccountTokenLong, Example: fmt.Sprintf(newServiceAccountTokenExamples, fullname), Run: func(cmd *cobra.Command, args []string) {
...
func (o *ServiceAccountTokenOptions) Run() error {
	...
	tokenSecret := &corev1.Secret{ObjectMeta: metav1.ObjectMeta{GenerateName: osautil.GetTokenSecretNamePrefix(serviceAccount.Name), Namespace: serviceAccount.Namespace, Labels: o.Labels, Annotations: map[string]string{corev1.ServiceAccountNameKey: serviceAccount.Name}}, Type: corev1.SecretTypeServiceAccountToken, Data: map[string][]byte{}}
...
func waitForToken(token *corev1.Secret, serviceAccount *corev1.ServiceAccount, timeout time.Duration, client corev1client.SecretInterface) (*corev1.Secret, error) {
	...
	options := metav1.ListOptions{FieldSelector: fields.SelectorFromSet(fields.Set(map[string]string{"metadata.name": token.Name})).String(), Watch: true, ResourceVersion: token.ResourceVersion, TimeoutSeconds: &timeoutSeconds}
