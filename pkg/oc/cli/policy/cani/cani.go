...
func NewCanIOptions(streams genericclioptions.IOStreams) *CanIOptions {
	...
	return &CanIOptions{PrintFlags: genericclioptions.NewPrintFlags("").WithTypeSetter(scheme.Scheme), IOStreams: streams}
}
func NewCmdCanI(name, fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: name + " VERB RESOURCE [NAME]", Short: "Check whether an action is allowed", Long: "Check whether an action is allowed", Run: func(cmd *cobra.Command, args []string) {
...
func (o *CanIOptions) Run() error {
	...
	sar := &authorizationv1.SubjectAccessReview{Action: authorizationv1.Action{Namespace: o.Namespace, Verb: o.Verb, Group: o.Resource.Group, Resource: o.Resource.Resource, ResourceName: o.ResourceName}, User: o.User, GroupsSlice: o.Groups}
	if o.IgnoreScopes {
		sar.Scopes = []string{}
	}
	if len(o.Scopes) > 0 {
...
func (o *CanIOptions) listAllPermissions() error {
	...
	if len(o.User) == 0 && len(o.Groups) == 0 {
		...
		rulesReview := &authorizationv1.SubjectRulesReview{Spec: authorizationv1.SubjectRulesReviewSpec{User: o.User, Groups: o.Groups, Scopes: o.Scopes}}
	...
	for _, rule := range rulesReviewResult {
		fmt.Fprintf(w, "%v\t%v\t%v\t%v\t%v\n", rule.Verbs, rule.NonResourceURLsSlice, rule.ResourceNames, rule.APIGroups, rule.Resources)
	}
	w.Flush()
