...
func NewCmdRolloutLatest(fullName string, f kcmdutil.Factory, streams genericclioptions.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "latest DEPLOYMENTCONFIG", Short: "Start a new rollout for a deployment config with the latest state from its triggers", Long: rolloutLatestLong, Example: fmt.Sprintf(rolloutLatestExample, fullName), Run: func(cmd *cobra.Command, args []string) {
...
func (o *RolloutLatestOptions) Complete(f kcmdutil.Factory, cmd *cobra.Command, args []string) error {
	...
	if o.PrintFlags.OutputFormat != nil && *o.PrintFlags.OutputFormat == "revision" {
		...
		o.Printer, err = printers.NewTypeSetter(scheme.Scheme).WrapToPrinter(&revisionPrinter{}, nil)
		if err != nil {
			return err
		}
	} else {
...
func (o *RolloutLatestOptions) RunRolloutLatest() error {
	...
	dc := config
	if !o.DryRun {
		request := &appsv1.DeploymentRequest{Name: config.Name, Latest: !o.again, Force: true}
