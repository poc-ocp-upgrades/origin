...
func TestRollbackOptions_findTargetDeployment(t *testing.T) {
	...
	tests := []struct {
		...
		errorExpected	bool
	}{{name: "desired found", configVersion: 3, existing: []existingDeployment{{1, appsv1.DeploymentStatusComplete}, {2, appsv1.DeploymentStatusComplete}, {3, appsv1.DeploymentStatusComplete}}, desiredVersion: 1, expectedVersion: 1, errorExpected: false}, {name: "desired not found", configVersion: 3, existing: []existingDeployment{{2, appsv1.DeploymentStatusComplete}, {3, appsv1.DeploymentStatusComplete}}, desiredVersion: 1, errorExpected: true}, {name: "desired not supplied, target found", configVersion: 3, existing: []existingDeployment{{1, appsv1.DeploymentStatusComplete}, {2, appsv1.DeploymentStatusFailed}, {3, appsv1.DeploymentStatusComplete}}, desiredVersion: 0, expectedVersion: 1, errorExpected: false}, {name: "desired not supplied, target not found", configVersion: 3, existing: []existingDeployment{{1, appsv1.DeploymentStatusFailed}, {2, appsv1.DeploymentStatusFailed}, {3, appsv1.DeploymentStatusComplete}}, desiredVersion: 0, errorExpected: true}}
	for _, test := range tests {
		...
		opts := &RollbackOptions{kubeClient: fakekc}
