...
func TestStartBuildWebHook(t *testing.T) {
	...
	o := &StartBuildOptions{IOStreams: genericclioptions.NewTestIOStreamsDiscard(), ClientConfig: cfg.Client, FromWebhook: server.URL + "/webhook", Mapper: testrestmapper.TestOnlyStaticRESTMapper(scheme.Scheme)}
	if err := o.Run(); err != nil {
		t.Fatalf("unable to start hook: %v", err)
	}
	<-invoked
	o = &StartBuildOptions{IOStreams: genericclioptions.NewTestIOStreamsDiscard(), FromWebhook: server.URL + "/webhook", GitPostReceive: "unknownpath"}
	if err := o.Run(); err == nil {
...
2548 2548 refs/heads/stage`)
	...
	cfg := &FakeClientConfig{Err: testErr}
	o := &StartBuildOptions{IOStreams: genericclioptions.NewTestIOStreamsDiscard(), ClientConfig: cfg.Client, FromWebhook: server.URL + "/webhook", GitPostReceive: f.Name(), Mapper: testrestmapper.TestOnlyStaticRESTMapper(scheme.Scheme)}
	if err := o.Run(); err != nil {
...
func TestHttpBinary(t *testing.T) {
	...
	tests := []struct {
		...
		expectWarning		bool
	}{{description: "--from-file, filename in header", fromFile: true, urlPath: "/", contentDisposition: true}, {description: "--from-file, filename in URL", fromFile: true, urlPath: "/hi.txt"}, {description: "--from-file, no filename", fromFile: true, urlPath: "", expectedError: "unable to determine filename"}, {description: "--from-file, http error", fromFile: true, urlPath: "/", statusCode: 404, expectedError: "unable to download file"}, {description: "--from-file, network error", fromFile: true, urlPath: "/hi.txt", networkError: true, expectedError: "invalid port"}, {description: "--from-file, https with invalid certificate", fromFile: true, urlPath: "/hi.txt", tlsBadCert: true, expectedError: "certificate signed by unknown authority"}, {description: "--from-dir, filename in header", fromFile: false, contentDisposition: true, expectWarning: true}, {description: "--from-dir, filename in URL", fromFile: false, urlPath: "/hi.tar.gz", expectWarning: true}, {description: "--from-dir, no filename", fromFile: false, expectWarning: true}, {description: "--from-dir, http error", statusCode: 503, fromFile: false, expectedError: "unable to download file"}}
...
func TestStreamBuildLogs(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	cases := []logTestCase{{ExpectedLogMsg: "hello"}, {RequestErr: errors.New("simulated failure"), ExpectedErrMsg: "unable to stream the build logs"}, {RequestErr: &kerrors.StatusError{ErrStatus: metav1.Status{Reason: metav1.StatusReasonTimeout, Message: "timeout"}}, ExpectedErrMsg: "unable to stream the build logs"}, {IOErr: errors.New("failed to read"), ExpectedErrMsg: "unable to stream the build logs"}}
	for i, tc := range cases {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			out := &bytes.Buffer{}
			build := &buildv1.Build{ObjectMeta: metav1.ObjectMeta{Name: "test-build", Namespace: "test-namespace"}}
			fakeREST := &restfake.RESTClient{NegotiatedSerializer: legacyscheme.Codecs, GroupVersion: schema.GroupVersion{Group: "build.openshift.io", Version: "v1"}, Client: restfake.CreateHTTPClient(func(*http.Request) (*http.Response, error) {
				if tc.RequestErr != nil {
					return nil, tc.RequestErr
				}
				var body io.Reader
				if tc.IOErr != nil {
					body = &failReader{Err: tc.IOErr}
				} else {
			...
			o := &StartBuildOptions{IOStreams: ioStreams, BuildLogClient: buildclientmanual.NewBuildLogClient(fakeREST, build.Namespace)}
