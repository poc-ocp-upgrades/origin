...
func TestParallelIsRunnableWithSerialRunning(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	mixedBuilds := []buildv1.Build{addBuild("build-7", "sample-bc", buildv1.BuildPhaseRunning, buildv1.BuildRunPolicySerial), addBuild("build-8", "sample-bc", buildv1.BuildPhaseNew, buildv1.BuildRunPolicyParallel), addBuild("build-9", "sample-bc", buildv1.BuildPhaseNew, buildv1.BuildRunPolicyParallel)}
	client := newTestClient(mixedBuilds)
	policy := ParallelPolicy{BuildLister: client.Lister(), BuildUpdater: client}
	for _, build := range mixedBuilds {
		runnable, err := policy.IsRunnable(&build)
