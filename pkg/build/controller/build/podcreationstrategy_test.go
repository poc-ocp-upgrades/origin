...
func TestStrategyCreateBuildPod(t *testing.T) {
	...
	strategy := &typeBasedFactoryStrategy{dockerBuildStrategy: &testPodCreationStrategy{pod: dockerBuildPod}, sourceBuildStrategy: &testPodCreationStrategy{pod: sourceBuildPod}, customBuildStrategy: &testPodCreationStrategy{pod: customBuildPod}}
	strategyErr := fmt.Errorf("error")
	errorStrategy := &typeBasedFactoryStrategy{dockerBuildStrategy: &testPodCreationStrategy{err: strategyErr}, sourceBuildStrategy: &testPodCreationStrategy{err: strategyErr}, customBuildStrategy: &testPodCreationStrategy{err: strategyErr}}
	...
	tests := []struct {
		...
		expectedPod	*v1.Pod
		expectError	bool
	}{{strategy: strategy, build: dockerBuild, expectedPod: dockerBuildPod}, {strategy: strategy, build: sourceBuild, expectedPod: sourceBuildPod}, {strategy: strategy, build: customBuild, expectedPod: customBuildPod}, {strategy: strategy, build: pipelineBuild, expectError: true}, {strategy: strategy, build: &buildv1.Build{}, expectError: true}, {strategy: errorStrategy, build: dockerBuild, expectError: true}, {strategy: errorStrategy, build: sourceBuild, expectError: true}, {strategy: errorStrategy, build: customBuild, expectError: true}}
