...
func (bc *BuildController) defaults() builddefaults.BuildDefaults {
	...
	bc.configLock.Lock()
	defer bc.configLock.Unlock()
	copy := builddefaults.BuildDefaults{Config: bc.buildDefaults.Config.DeepCopy(), DefaultProxy: bc.buildDefaults.DefaultProxy.DeepCopy()}
	return copy
}
...
func (bc *BuildController) Run(workers int, stopCh <-chan struct{}) {
	...
	if !cache.WaitForCacheSync(stopCh, bc.buildStoreSynced, bc.podStoreSynced, bc.secretStoreSynced, bc.serviceAccountStoreSynced, bc.imageStreamStoreSynced, bc.openshiftConfigConfigMapStoreSynced, bc.controllerManagerConfigMapStoreSynced) {
...
func (bc *BuildController) createBuildCAConfigMapSpec(build *buildv1.Build, buildPod *corev1.Pod, caData map[string]string) *corev1.ConfigMap {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	cm := &corev1.ConfigMap{ObjectMeta: metav1.ObjectMeta{Name: buildapihelpers.GetBuildCAConfigMapName(build), OwnerReferences: []metav1.OwnerReference{makeBuildPodOwnerRef(buildPod)}}, Data: caData}
