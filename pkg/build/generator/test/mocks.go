...
var (
	...
	SampleDockerConfigs	= map[string][]byte{"hub": Encode(`{"https://index.docker.io/v1/":{"auth": "Zm9vOmJhcgo=", "email": ""}}`), "ipv4": Encode(`{"https://1.1.1.1:5000/v1/":{"auth": "Zm9vOmJhcgo=", "email": ""}}`), "host": Encode(`{"https://registry.host/v1/":{"auth": "Zm9vOmJhcgo=", "email": ""}}`)}
)
...
func MockBuilderServiceAccount(secrets []*corev1.Secret) corev1client.ServiceAccountsGetter {
	...
	for _, secret := range secrets {
		secretRefs = append(secretRefs, corev1.ObjectReference{Name: secret.Name, Kind: "Secret"})
		fakeObjects = append(fakeObjects, secret)
	}
	fakeObjects = append(fakeObjects, &corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Name: bootstrappolicy.BuilderServiceAccountName, Namespace: metav1.NamespaceDefault}, Secrets: secretRefs})
...
func MockSource() buildv1.BuildSource {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return buildv1.BuildSource{Git: &buildv1.GitBuildSource{URI: "http://test.repository/namespace/name", Ref: "test-tag"}}
}
...
func MockSourceStrategyForEnvs() buildv1.BuildStrategy {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return buildv1.BuildStrategy{SourceStrategy: &buildv1.SourceBuildStrategy{Env: []corev1.EnvVar{{Name: "FOO", Value: "VAR"}}, From: corev1.ObjectReference{Kind: "ImageStreamTag", Name: imageRepoName + ":" + tagName, Namespace: imageRepoNamespace}}}
}
...
func MockCustomStrategyForEnvs() buildv1.BuildStrategy {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return buildv1.BuildStrategy{CustomStrategy: &buildv1.CustomBuildStrategy{Env: []corev1.EnvVar{{Name: "FOO", Value: "VAR"}}, From: corev1.ObjectReference{Kind: "ImageStreamTag", Name: imageRepoName + ":" + tagName, Namespace: imageRepoNamespace}}}
}
...
func MockImageStream(repoName, dockerImageRepo string, tags map[string]string) *imagev1.ImageStream {
	...
	for tag, imageID := range tags {
		tagHistory = append(tagHistory, imagev1.NamedTagEventList{Tag: tag, Items: []imagev1.TagEvent{{Image: imageID, DockerImageReference: fmt.Sprintf("%s:%s", dockerImageRepo, imageID)}}})
	}
	return &imagev1.ImageStream{ObjectMeta: metav1.ObjectMeta{Name: repoName}, Status: imagev1.ImageStreamStatus{DockerImageRepository: dockerImageRepo, Tags: tagHistory}}
}
