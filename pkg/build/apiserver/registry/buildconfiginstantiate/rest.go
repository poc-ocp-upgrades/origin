...
type InstantiateREST struct{ generator *generator.BuildGenerator }
...
func (s *InstantiateREST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, options *metav1.CreateOptions) (runtime.Object, error) {
	...
	if request.TriggeredBy == nil {
		...
		request.TriggeredBy = append(buildTriggerCauses, buildapi.BuildTriggerCause{Message: buildapi.BuildTriggerCauseManualMsg})
	}
...
func NewBinaryStorage(generator *generator.BuildGenerator, buildClient buildtypedclient.BuildsGetter, inClientConfig *restclient.Config) *BinaryInstantiateREST {
	...
	return &BinaryInstantiateREST{Generator: generator, BuildClient: buildClient, ClientConfig: clientConfig, Timeout: 5 * time.Minute}
}
...
func (r *BinaryInstantiateREST) Connect(ctx context.Context, name string, options runtime.Object, responder rest.Responder) (http.Handler, error) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &binaryInstantiateHandler{r: r, responder: responder, ctx: ctx, name: name, options: options.(*buildapi.BinaryBuildRequestOptions)}, nil
}
...
func (h *binaryInstantiateHandler) handle(r io.Reader) (runtime.Object, error) {
	...
	if len(h.options.Commit) > 0 {
		request.Revision = &buildapi.SourceRevision{Git: &buildapi.GitSourceRevision{Committer: buildapi.SourceControlUser{Name: h.options.CommitterName, Email: h.options.CommitterEmail}, Author: buildapi.SourceControlUser{Name: h.options.AuthorName, Email: h.options.AuthorEmail}, Message: h.options.Message, Commit: h.options.Commit}}
	}
	request.Binary = &buildapi.BinaryBuildSource{AsFile: h.options.AsFile}
	...
	opts := &kapi.PodAttachOptions{Stdin: true, Container: buildstrategy.GitCloneContainer}
	...
	req := restClient.Post().Resource("pods").Name(buildPodName).Namespace(build.Namespace).SubResource("attach")
	req.VersionedParams(opts, legacyscheme.ParameterCodec)
	exec, err := remotecommand.NewSPDYExecutor(h.r.ClientConfig, "POST", req.URL())
	if err != nil {
		return nil, err
	}
	err = exec.Stream(remotecommand.StreamOptions{Stdin: r})
	if err != nil {
