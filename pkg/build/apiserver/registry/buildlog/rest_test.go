...
func TestRegistryResourceLocation(t *testing.T) {
	...
	expectedLocations := map[buildv1.BuildPhase]struct {
		...
		name		string
		container	string
	}{buildv1.BuildPhaseComplete: {namespace: "default", name: "running-build", container: ""}, buildv1.BuildPhaseFailed: {namespace: "default", name: "running-build", container: ""}, buildv1.BuildPhaseRunning: {namespace: "default", name: "running-build", container: ""}, buildv1.BuildPhaseNew: {}, buildv1.BuildPhasePending: {}, buildv1.BuildPhaseError: {}, buildv1.BuildPhaseCancelled: {}}
...
func TestWaitForBuild(t *testing.T) {
	...
	tests := []struct {
		...
		expectError	bool
	}{{name: "New -> Running", status: []buildv1.BuildPhase{buildv1.BuildPhaseNew, buildv1.BuildPhaseRunning}, expectError: false}, {name: "New -> Pending -> Complete", status: []buildv1.BuildPhase{buildv1.BuildPhaseNew, buildv1.BuildPhasePending, buildv1.BuildPhaseComplete}, expectError: false}, {name: "New -> Pending -> Failed", status: []buildv1.BuildPhase{buildv1.BuildPhaseNew, buildv1.BuildPhasePending, buildv1.BuildPhaseFailed}, expectError: false}, {name: "New -> Pending -> Cancelled", status: []buildv1.BuildPhase{buildv1.BuildPhaseNew, buildv1.BuildPhasePending, buildv1.BuildPhaseCancelled}, expectError: true}, {name: "New -> Pending -> Error", status: []buildv1.BuildPhase{buildv1.BuildPhaseNew, buildv1.BuildPhasePending, buildv1.BuildPhaseError}, expectError: true}, {name: "Pending -> Cancelled", status: []buildv1.BuildPhase{buildv1.BuildPhasePending, buildv1.BuildPhaseCancelled}, expectError: true}, {name: "Error", status: []buildv1.BuildPhase{buildv1.BuildPhaseError}, expectError: true}}
	for _, tt := range tests {
		...
		storage := REST{BuildClient: buildClient.BuildV1(), PodClient: newPodClient().CoreV1(), Timeout: defaultTimeout}
...
func TestWaitForBuildTimeout(t *testing.T) {
	...
	buildClient := buildfakeclient.NewSimpleClientset(build)
	ctx := apirequest.NewDefaultContext()
	storage := REST{BuildClient: buildClient.BuildV1(), PodClient: newPodClient().CoreV1(), Timeout: 100 * time.Millisecond}
	_, err := storage.Get(ctx, build.Name, &buildapi.BuildLogOptions{})
...
func TestPreviousBuildLogs(t *testing.T) {
	...
	storage := &REST{BuildClient: buildClient.BuildV1(), PodClient: anotherNewPodClient().CoreV1(), Timeout: defaultTimeout}
	actualPodNamespace := ""
	actualPodName := ""
	actualContainer := ""
	getSimplePodLogsFn := func(podNamespace, podName string, logOpts *kapi.PodLogOptions) (runtime.Object, error) {
