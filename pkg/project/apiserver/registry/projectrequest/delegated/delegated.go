...
func NewREST(message, templateNamespace, templateName string, projectClient projectclientinternal.ProjectsGetter, templateClient templateclient.Interface, sarClient authorizationclient.SubjectAccessReviewInterface, client dynamic.Interface, restMapper meta.RESTMapper, roleBindings rbacv1listers.RoleBindingLister) *REST {
	_logClusterCodePath()
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	return &REST{message: message, templateNamespace: templateNamespace, templateName: templateName, projectGetter: projectClient, templateClient: templateClient, sarClient: sarClient, client: client, restMapper: restMapper, roleBindings: roleBindings}
}
...
func (r *REST) Create(ctx context.Context, obj runtime.Object, createValidation rest.ValidateObjectFunc, options *metav1.CreateOptions) (runtime.Object, error) {
	...
	for i := range processedList.Items {
		...
		case schema.GroupKind{Group: "project.openshift.io", Kind: "Project"}:
			...
			continue
		case schema.GroupKind{Group: "authorization.openshift.io", Kind: "RoleBinding"}, schema.GroupKind{Group: "rbac.authorization.k8s.io", Kind: "RoleBinding"}:
	...
	for _, toCreate := range objectsToCreate {
		...
		if gvk := restMapping.GroupVersionKind; apierror.IsAlreadyExists(createErr) && gvk.Kind == roleBindingKind && roleBindingGroups.Has(gvk.Group) && defaultRoleBindingNames.Has(toCreate.GetName()) {
			continue
		}
...
func (r *REST) List(ctx context.Context, options *metainternal.ListOptions) (runtime.Object, error) {
	...
	accessReview := authorizationutil.AddUserToSAR(userInfo, &authorizationv1.SubjectAccessReview{Spec: authorizationv1.SubjectAccessReviewSpec{ResourceAttributes: &authorizationv1.ResourceAttributes{Verb: "create", Group: projectapi.GroupName, Resource: "projectrequests"}}})
	...
	if len(r.message) > 0 {
		...
		forbiddenError.ErrStatus.Details = &metav1.StatusDetails{Kind: "ProjectRequest", Causes: []metav1.StatusCause{{Message: r.message}}}
	} else {
