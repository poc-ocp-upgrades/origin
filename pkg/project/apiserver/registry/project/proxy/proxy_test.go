...
type mockLister struct{ namespaceList *corev1.NamespaceList }
...
func TestListProjects(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	namespaceList := corev1.NamespaceList{Items: []corev1.Namespace{{ObjectMeta: metav1.ObjectMeta{Name: "foo"}}}}
	mockClient := fake.NewSimpleClientset(&namespaceList)
	storage := REST{client: mockClient.CoreV1().Namespaces(), lister: &mockLister{&namespaceList}}
	user := &user.DefaultInfo{Name: "test-user", UID: "test-uid", Groups: []string{"test-groups"}}
...
func TestCreateProjectOK(t *testing.T) {
	...
	mockClient := &fake.Clientset{}
	storage := NewREST(mockClient.CoreV1().Namespaces(), &mockLister{}, nil, nil)
	_, err := storage.Create(apirequest.NewContext(), &projectapi.Project{ObjectMeta: metav1.ObjectMeta{Name: "foo"}}, rest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	if err != nil {
...
func TestDeleteProject(t *testing.T) {
	...
	defer _logClusterCodePath()
	mockClient := &fake.Clientset{}
	storage := REST{client: mockClient.CoreV1().Namespaces()}
