...
func fuzzInternalObject(t *testing.T, forVersion schema.GroupVersion, item runtime.Object, seed int64) runtime.Object {
	...
	f.Funcs(func(obj *configapi.MasterConfig, c fuzz.Continue) {
		...
		if obj.MasterClients.OpenShiftLoopbackClientConnectionOverrides == nil {
			obj.MasterClients.OpenShiftLoopbackClientConnectionOverrides = &configapi.ClientConnectionOverrides{AcceptContentTypes: "test/second", ContentType: "test/first"}
		}
		...
		if c.RandBool() {
			if len(obj.NetworkConfig.ClusterNetworks) == 0 {
				clusterNetwork := []configapi.ClusterNetworkEntry{{CIDR: "10.128.0.0/14", HostSubnetLength: 9}}
			...
			clusterNetwork := []configapi.ClusterNetworkEntry{{CIDR: obj.NetworkConfig.DeprecatedClusterNetworkCIDR, HostSubnetLength: obj.NetworkConfig.DeprecatedHostSubnetLength}}
			obj.NetworkConfig.ClusterNetworks = clusterNetwork
		}
		...
		if c.RandBool() {
			obj.AdmissionConfig.PluginConfig = map[string]*configapi.AdmissionPluginConfig{"abc": {Location: "test", Configuration: &configapi.LDAPSyncConfig{URL: "ldap://some:other@server:8080/test"}}}
		}
		...
		if obj.OAuthConfig != nil && c.RandBool() {
			obj.OAuthConfig.IdentityProviders = []configapi.IdentityProvider{{MappingMethod: "claim", Provider: &configapi.LDAPSyncConfig{URL: "ldap://some:other@server:8080/test"}}}
		}
		...
		if obj.MasterClientConnectionOverrides == nil {
			obj.MasterClientConnectionOverrides = &configapi.ClientConnectionOverrides{QPS: 1.0, Burst: 1, AcceptContentTypes: "test/other", ContentType: "test/third"}
		}
		...
		obj.AllowedRegistriesForImport = &configapi.AllowedRegistries{{DomainName: "docker.io"}, {DomainName: "gcr.io"}}
...
func TestSpecificRoundTrips(t *testing.T) {
	...
	}{{in: &configapi.MasterConfig{AdmissionConfig: configapi.AdmissionConfig{PluginConfig: map[string]*configapi.AdmissionPluginConfig{"test1": {Configuration: &configapi.LDAPSyncConfig{BindDN: "first"}}, "test2": {Configuration: &runtime.Unknown{Raw: []byte(`{"kind":"LDAPSyncConfig","apiVersion":"v1","bindDN":"second"}`)}}, "test3": {Configuration: &runtime.Unknown{Raw: []byte(`{"kind":"Unknown","apiVersion":"some/version"}`)}}, "test4": {Configuration: nil}}}}, to: legacyconfigv1.LegacySchemeGroupVersion, out: &legacyconfigv1.MasterConfig{TypeMeta: metav1.TypeMeta{Kind: "MasterConfig", APIVersion: "v1"}, AdmissionConfig: legacyconfigv1.AdmissionConfig{PluginConfig: map[string]*legacyconfigv1.AdmissionPluginConfig{"test1": {Configuration: runtime.RawExtension{Object: &legacyconfigv1.LDAPSyncConfig{BindDN: "first"}, Raw: []byte(`{"kind":"LDAPSyncConfig","apiVersion":"v1","url":"","bindDN":"first","bindPassword":"","insecure":false,"ca":"","groupUIDNameMapping":null}`)}}, "test2": {Configuration: runtime.RawExtension{Object: &legacyconfigv1.LDAPSyncConfig{BindDN: "second"}, Raw: []byte(`{"kind":"LDAPSyncConfig","apiVersion":"v1","bindDN":"second"}`)}}, "test3": {Configuration: runtime.RawExtension{Object: &runtime.Unknown{TypeMeta: runtime.TypeMeta{Kind: "Unknown", APIVersion: "some/version"}, ContentType: "application/json", Raw: []byte(`{"kind":"Unknown","apiVersion":"some/version"}`)}, Raw: []byte(`{"kind":"Unknown","apiVersion":"some/version"}`)}}, "test4": {}}}, VolumeConfig: legacyconfigv1.MasterVolumeConfig{DynamicProvisioningEnabled: &boolFalse}}, from: legacyconfigv1.LegacySchemeGroupVersion}}
...
func fuzzerFor(src rand.Source) *fuzz.Fuzzer {
	_logClusterCodePath()
	defer _logClusterCodePath()
	f := fuzzer.FuzzerFor(fuzzer.MergeFuzzerFuncs(kubeapitesting.FuzzerFuncs, Funcs), src, serializer.NewCodecFactory(configapi.Scheme))
	return f
}

var Funcs = func(codecs serializer.CodecFactory) []interface{} {
	return []interface{}{func(j *runtime.TypeMeta, c fuzz.Continue) {
		j.APIVersion = ""
		j.Kind = ""
	}, func(j *runtime.Object, c fuzz.Continue) {
		*j = &runtime.Unknown{TypeMeta: runtime.TypeMeta{APIVersion: "unknown.group/unknown", Kind: "Something"}, ContentType: "application/json", Raw: []byte(`{"apiVersion":"unknown.group/unknown","kind":"Something","someKey":"someValue"}`)}
