...
func TestValidate_ValidateEtcdStorageConfig(t *testing.T) {
	...
	for _, test := range tests {
		...
		config := config.EtcdStorageConfig{OpenShiftStorageVersion: test.openshiftStorageVersion, KubernetesStorageVersion: test.kubeStorageVersion}
...
func TestValidateAdmissionPluginConfig(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	locationOnly := configapi.AdmissionPluginConfig{Location: "/some/location"}
	configOnly := configapi.AdmissionPluginConfig{Configuration: &configapi.NodeConfig{}}
	locationAndConfig := configapi.AdmissionPluginConfig{Location: "/some/location", Configuration: &configapi.NodeConfig{}}
	bothEmpty := configapi.AdmissionPluginConfig{}
	tests := []struct {
		config		map[string]*configapi.AdmissionPluginConfig
		expectError	bool
		warningFields	[]string
	}{{config: map[string]*configapi.AdmissionPluginConfig{"one": &locationOnly, "two": &configOnly}}, {config: map[string]*configapi.AdmissionPluginConfig{"one": &locationOnly, "two": &locationAndConfig}, expectError: true}, {config: map[string]*configapi.AdmissionPluginConfig{"one": &configOnly, "two": &bothEmpty}, expectError: true}, {config: map[string]*configapi.AdmissionPluginConfig{"openshift.io/OriginResourceQuota": &configOnly, "two": &configOnly}, warningFields: []string{"[openshift.io/OriginResourceQuota]"}, expectError: false}}
...
func TestValidateMasterAuthConfig(t *testing.T) {
	...
	}{{testName: "No AuthConfig"}, {testName: "Valid RequestHeader", RequestHeader: &configapi.RequestHeaderAuthenticationOptions{ClientCA: "ClientCA", ClientCommonNames: []string{"ClientCommonNames", "2", "3"}, UsernameHeaders: []string{"Username", "Headers"}, GroupHeaders: []string{"Group", "Headers"}, ExtraHeaderPrefixes: []string{"Extra", "Header", "Prefixes"}}}, {testName: "Only One WebhookTokenAuthenticator", WebhookTokenAuthenticators: []configapi.WebhookTokenAuthenticator{{ConfigFile: testConfigFile.Name(), CacheTTL: "2m"}}}, {testName: "Unexisting config file in second WebhookTokenAuthenticator", WebhookTokenAuthenticators: []configapi.WebhookTokenAuthenticator{{ConfigFile: testConfigFile.Name(), CacheTTL: "2m"}, {ConfigFile: "Unexisting", CacheTTL: "2m"}}, expectedErrors: []string{"webhookTokenAuthenticators.ConfigFile: Invalid value: \"Unexisting\": could not read file: stat Unexisting: no such file or directory"}}, {testName: "Invalid and Empty CacheTTL WebhookTokenAuthenticator", WebhookTokenAuthenticators: []configapi.WebhookTokenAuthenticator{{ConfigFile: testConfigFile.Name(), CacheTTL: "-2m"}, {ConfigFile: testConfigFile.Name()}}, expectedErrors: []string{"webhookTokenAuthenticators.cacheTTL: Invalid value: \"-2m\": cannot be less than zero", "webhookTokenAuthenticators.cacheTTL: Required value"}}, {testName: "No OAuth Metadata file", OAuthMetadataFile: "NoFile", expectedErrors: []string{`oauthMetadataFile: Invalid value: "NoFile": Metadata validation failed: unable to read External OAuth Metadata file: open NoFile: no such file or directory`}}, {testName: "Bad Metadata file", OAuthMetadataFile: badMetadataFile.Name(), expectedErrors: []string{fmt.Sprintf(`oauthMetadataFile: Invalid value: %q: Metadata validation failed: unable to decode External OAuth Metadata file: invalid character 'b' looking for beginning of value`, badMetadataFile.Name())}}}
	for _, test := range testCases {
		config := configapi.MasterAuthConfig{RequestHeader: test.RequestHeader, WebhookTokenAuthenticators: test.WebhookTokenAuthenticators, OAuthMetadataFile: test.OAuthMetadataFile}
