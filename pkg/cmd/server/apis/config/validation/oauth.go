...
func ValidateOAuthConfig(config *configapi.OAuthConfig, fldPath *field.Path) common.ValidationResults {
	...
	if timeout := config.TokenConfig.AccessTokenInactivityTimeoutSeconds; timeout != nil {
		if *timeout != 0 && *timeout < oauthvalidation.MinimumInactivityTimeoutSeconds {
			validationResults.AddErrors(field.Invalid(fldPath.Child("tokenConfig", "accessTokenInactivityTimeoutSeconds"), *timeout, fmt.Sprintf("The minimum acceptable token timeout value is %d seconds", oauthvalidation.MinimumInactivityTimeoutSeconds)))
		}
	}
...
var validMappingMethods = sets.NewString(string(identitymapper.MappingMethodLookup), string(identitymapper.MappingMethodClaim), string(identitymapper.MappingMethodAdd), string(identitymapper.MappingMethodGenerate))
...
func ValidateRequestHeaderIdentityProvider(provider *configapi.RequestHeaderIdentityProvider, identityProvider configapi.IdentityProvider, fieldPath *field.Path) common.ValidationResults {
	...
	if len(provider.ChallengeURL) > 0 {
		...
		if len(urlErrs) == 0 && !strings.Contains(url.RawQuery, tokens.URLToken) && !strings.Contains(url.RawQuery, tokens.QueryToken) {
			validationResults.AddWarnings(field.Invalid(field.NewPath("provider", "challengeURL"), provider.ChallengeURL, fmt.Sprintf("query does not include %q or %q, redirect will not preserve original authorize parameters", tokens.URLToken, tokens.QueryToken)))
		}
	}
	if len(provider.LoginURL) > 0 {
		...
		if len(urlErrs) == 0 {
			if !strings.Contains(url.RawQuery, tokens.URLToken) && !strings.Contains(url.RawQuery, tokens.QueryToken) {
				validationResults.AddWarnings(field.Invalid(fieldPath.Child("provider", "loginURL"), provider.LoginURL, fmt.Sprintf("query does not include %q or %q, redirect will not preserve original authorize parameters", tokens.URLToken, tokens.QueryToken)))
			}
			if strings.HasSuffix(url.Path, "/") {
				validationResults.AddWarnings(field.Invalid(fieldPath.Child("provider", "loginURL"), provider.LoginURL, `path ends with "/", grant approval flows will not function correctly`))
			}
			if _, file := path.Split(url.Path); file != "authorize" {
				validationResults.AddWarnings(field.Invalid(fieldPath.Child("provider", "loginURL"), provider.LoginURL, `path does not end with "/authorize", grant approval flows will not function correctly`))
			}
		}
	}
...
func ValidateGitHubIdentityProvider(provider *configapi.GitHubIdentityProvider, challenge bool, mappingMethod string, fieldPath *field.Path) common.ValidationResults {
	...
	if caFile := provider.CA; len(caFile) != 0 {
		...
		validationResults.AddErrors(common.ValidateFile(caFile, caPath)...)
...
func ValidateSessionSecrets(config *configapi.SessionSecrets) field.ErrorList {
	...
	for i, secret := range config.Secrets {
		...
		case len(secret.Authentication) < 32:
			allErrs = append(allErrs, field.Invalid(idxPath.Child("authentication"), strings.Repeat("*", len(secret.Authentication)), "must be at least 32 characters long"))
		}
		...
		case 16, 24, 32:
		default:
			allErrs = append(allErrs, field.Invalid(idxPath.Child("encryption"), strings.Repeat("*", len(secret.Encryption)), "must be 16, 24, or 32 characters long"))
		}
	}
	return allErrs
}
