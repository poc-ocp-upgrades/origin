...
var testFilenames = []string{string([]byte{3}), string([]byte{4}), `white space`, `new
line`, `*`, `~`, `\`, `\\`, ` && touch and-escape`, ` || touch or-escape`, ` ; touch semi-escape`, ` " touch quote-escape`, ` ' touch apos-escape`, ` }"; touch brace-escape`, `env x='() { :;}; echo vulnerable'`, `$USER`, `...`, `.file`, `中文`, `κόσμε`, `Iñtërnâtiônàlizætiøn`}
...
func TestCheckEmpty(t *testing.T) {
	...
	tests := []struct {
		...
		filenames	[]string
		expectedError	bool
	}{{name: "EmptyDir", expectedError: false}, {name: "File", filenames: []string{"file"}, expectedError: true}, {name: "HiddenFile", filenames: []string{".file"}, expectedError: true}, {name: "TestFilenames", filenames: testFilenames, expectedError: true}}
