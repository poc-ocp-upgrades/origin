...
func RunIngressIPController(ctx *ControllerContext) (bool, error) {
	...
	ingressIPController := ingressip.NewIngressIPController(ctx.KubernetesInformers.Core().V1().Services().Informer(), ctx.ClientBuilder.ClientOrDie(bootstrappolicy.InfraServiceIngressIPControllerServiceAccountName), ipNet, resyncPeriod)
