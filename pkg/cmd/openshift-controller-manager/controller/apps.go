...
func RunDeployerController(ctx *ControllerContext) (bool, error) {
	...
	go deployercontroller.NewDeployerController(ctx.KubernetesInformers.Core().V1().ReplicationControllers(), ctx.KubernetesInformers.Core().V1().Pods(), kubeClient, bootstrappolicy.DeployerServiceAccountName, imageTemplate.ExpandOrDie("deployer"), nil).Run(5, ctx.Stop)
	return true, nil
}
func RunDeploymentConfigController(ctx *ControllerContext) (bool, error) {
	...
	go deployconfigcontroller.NewDeploymentConfigController(ctx.AppsInformers.Apps().V1().DeploymentConfigs(), ctx.KubernetesInformers.Core().V1().ReplicationControllers(), ctx.ClientBuilder.OpenshiftAppsClientOrDie(saName), kubeClient).Run(5, ctx.Stop)
	return true, nil
}
