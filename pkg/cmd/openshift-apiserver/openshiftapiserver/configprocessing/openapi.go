...
func DefaultOpenAPIConfig(oauthMetadata *oauthutil.OauthAuthorizationServerMetadata) *openapicommon.Config {
	...
	if oauthMetadata != nil {
		securityDefinitions["Oauth2Implicit"] = &spec.SecurityScheme{SecuritySchemeProps: spec.SecuritySchemeProps{Type: "oauth2", Flow: "implicit", AuthorizationURL: oauthMetadata.AuthorizationEndpoint, Scopes: scope.DescribeScopes(oauthMetadata.ScopesSupported)}}
		securityDefinitions["Oauth2AccessToken"] = &spec.SecurityScheme{SecuritySchemeProps: spec.SecuritySchemeProps{Type: "oauth2", Flow: "accessCode", AuthorizationURL: oauthMetadata.AuthorizationEndpoint, TokenURL: oauthMetadata.TokenEndpoint, Scopes: scope.DescribeScopes(oauthMetadata.ScopesSupported)}}
	}
	...
	return &openapicommon.Config{ProtocolList: []string{"https"}, GetDefinitions: openapigenerated.GetOpenAPIDefinitions, IgnorePrefixes: []string{"/swaggerapi", "/healthz", "/controllers", "/metrics", "/version/openshift", "/brokers"}, GetDefinitionName: defNamer.GetDefinitionName, GetOperationIDAndTags: func(r *restful.Route) (string, []string, error) {
		op := r.Operation
		path := r.Path
		...
		return apiserverendpointsopenapi.GetOperationIDAndTags(r)
	}, Info: &spec.Info{InfoProps: spec.InfoProps{Title: "OpenShift API (with Kubernetes)", Version: version.Get().String(), License: &spec.License{Name: "Apache 2.0 (ASL2.0)", URL: "http://www.apache.org/licenses/LICENSE-2.0"}, Description: heredoc.Doc(`
					...
					* a 'status' that represents how much of the desired state is reflected on
					...
					information.
				`)}}, DefaultResponse: &spec.Response{ResponseProps: spec.ResponseProps{Description: "Default Response."}}, SecurityDefinitions: &securityDefinitions}
}
