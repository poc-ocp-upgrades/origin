...
func ValidateRoleBindingRestriction(rbr *authorizationapi.RoleBindingRestriction) field.ErrorList {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	allErrs := validation.ValidateObjectMeta(&rbr.ObjectMeta, true, apimachineryvalidation.NameIsDNSSubdomain, field.NewPath("metadata"))
	allErrs = append(allErrs, ValidateRoleBindingRestrictionSpec(&rbr.Spec, field.NewPath("spec"))...)
	return allErrs
}
...
func ValidateRoleBindingRestrictionSpec(spec *authorizationapi.RoleBindingRestrictionSpec, fld *field.Path) field.ErrorList {
	...
	if spec.UserRestriction != nil {
		if spec.GroupRestriction != nil {
			allErrs = append(allErrs, field.Invalid(fld.Child("grouprestriction"), "both userrestriction and grouprestriction specified", invalidMsg))
		}
		if spec.ServiceAccountRestriction != nil {
			allErrs = append(allErrs, field.Invalid(fld.Child("serviceaccountrestriction"), "both userrestriction and serviceaccountrestriction specified", invalidMsg))
		}
	} else if spec.GroupRestriction != nil {
		if spec.ServiceAccountRestriction != nil {
			allErrs = append(allErrs, field.Invalid(fld.Child("serviceaccountrestriction"), "both grouprestriction and serviceaccountrestriction specified", invalidMsg))
		}
	} else if spec.ServiceAccountRestriction == nil {
		allErrs = append(allErrs, field.Required(fld.Child("userrestriction"), invalidMsg))
	}
	if spec.UserRestriction != nil {
...
func ValidateRoleBindingRestrictionUser(user *authorizationapi.UserRestriction, fld *field.Path) field.ErrorList {
	...
	for i, selector := range user.Selectors {
		allErrs = append(allErrs, unversionedvalidation.ValidateLabelSelector(&selector, fld.Child("selector").Index(i))...)
	}
	return allErrs
}
...
func ValidateRoleBindingRestrictionServiceAccount(sa *authorizationapi.ServiceAccountRestriction, fld *field.Path) field.ErrorList {
	...
	if !(len(sa.ServiceAccounts) > 0 || len(sa.Namespaces) > 0) {
		allErrs = append(allErrs, field.Required(fld.Child("serviceaccounts"), invalidMsg))
	}
	return allErrs
}
