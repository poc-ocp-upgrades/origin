...
func TestConversionErrors(t *testing.T) {
	...
	for _, test := range []struct {
		...
		expected	string
		f		func() error
	}{{name: "invalid origin role ref", expected: `invalid origin role binding rolebindingname: attempts to reference role in namespace "ns1" instead of current namespace "ns0"`, f: func() error {
		return Convert_authorization_RoleBinding_To_rbac_RoleBinding(&authorizationapi.RoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "rolebindingname", Namespace: "ns0"}, RoleRef: api.ObjectReference{Namespace: "ns1"}}, &rbac.RoleBinding{}, nil)
	}}, {name: "invalid origin subject kind", expected: `invalid kind for origin subject: "fancyuser"`, f: func() error {
		return Convert_authorization_ClusterRoleBinding_To_rbac_ClusterRoleBinding(&authorizationapi.ClusterRoleBinding{Subjects: []api.ObjectReference{{Kind: "fancyuser"}}}, &rbac.ClusterRoleBinding{}, nil)
	}}, {name: "invalid origin rol ref namespace", expected: `invalid origin cluster role binding clusterrolebindingname: attempts to reference role in namespace "fancyns" instead of cluster scope`, f: func() error {
		return Convert_authorization_ClusterRoleBinding_To_rbac_ClusterRoleBinding(&authorizationapi.ClusterRoleBinding{ObjectMeta: metav1.ObjectMeta{Name: "clusterrolebindingname"}, RoleRef: api.ObjectReference{Namespace: "fancyns"}}, &rbac.ClusterRoleBinding{}, nil)
	}}, {name: "invalid RBAC subject kind", expected: `invalid kind for rbac subject: "evenfancieruser"`, f: func() error {
		return Convert_rbac_ClusterRoleBinding_To_authorization_ClusterRoleBinding(&rbac.ClusterRoleBinding{Subjects: []rbac.Subject{{Kind: "evenfancieruser"}}}, &authorizationapi.ClusterRoleBinding{}, nil)
	}}, {name: "invalid RBAC rol ref kind", expected: `invalid kind "anewfancykind" for rbac role ref "fancyrolref"`, f: func() error {
		return Convert_rbac_ClusterRoleBinding_To_authorization_ClusterRoleBinding(&rbac.ClusterRoleBinding{RoleRef: rbac.RoleRef{Name: "fancyrolref", Kind: "anewfancykind"}}, &authorizationapi.ClusterRoleBinding{}, nil)
	}}} {
...
func TestResourceAndNonResourceRuleSplit(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	ocr := &authorizationapi.ClusterRole{Rules: []authorizationapi.PolicyRule{{Verbs: sets.NewString("get", "create"), APIGroups: []string{"v1", ""}, Resources: sets.NewString("pods", "nodes"), ResourceNames: sets.NewString("foo", "bar"), NonResourceURLs: sets.NewString("/api", "/health")}}}
	ocr2 := &authorizationapi.ClusterRole{}
	rcr := &rbac.ClusterRole{}
	if err := Convert_authorization_ClusterRole_To_rbac_ClusterRole(ocr, rcr, nil); err != nil {
		t.Fatal(err)
...
func TestAnnotationsConversion(t *testing.T) {
	...
	for _, boolval := range []string{"true", "false"} {
		ocr := &authorizationapi.ClusterRole{Rules: []authorizationapi.PolicyRule{}}
		...
		rcr := &rbac.ClusterRole{Rules: []rbac.PolicyRule{}}
...
var fuzzer = fuzz.New().NilChance(0).Funcs(func(*metav1.TypeMeta, fuzz.Continue) {
}, func(*runtime.Object, fuzz.Continue) {
	...
	sortAndDeduplicateRBACRulesFields(rr.Rules)
}, func(rcr *rbac.ClusterRole, c fuzz.Continue) {
	c.FuzzNoCustom(rcr)
	setRBACRuleType(rcr.Rules, c.RandBool())
	sortAndDeduplicateRBACRulesFields(rcr.Rules)
})
