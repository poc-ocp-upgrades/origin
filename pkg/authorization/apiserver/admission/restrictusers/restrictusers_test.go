...
func TestAdmission(t *testing.T) {
	...
	var (
		...
		userBob			= userapi.User{ObjectMeta: metav1.ObjectMeta{Name: "Bob"}, Groups: []string{"group"}}
		userBobSubj		= rbac.Subject{Kind: rbac.UserKind, Name: "Bob"}
		group			= userapi.Group{ObjectMeta: metav1.ObjectMeta{Name: "group", Labels: map[string]string{"baz": "quux"}}, Users: []string{userBobSubj.Name}}
		groupSubj		= rbac.Subject{Kind: rbac.GroupKind, Name: "group"}
		serviceaccount		= corev1.ServiceAccount{ObjectMeta: metav1.ObjectMeta{Namespace: "namespace", Name: "serviceaccount", Labels: map[string]string{"xyzzy": "thud"}}}
		serviceaccountSubj	= rbac.Subject{Kind: rbac.ServiceAccountKind, Namespace: "namespace", Name: "serviceaccount"}
	)
	testCases := []struct {
		name			string
		expectedErr		string
	...
	for _, tc := range testCases {
		...
		attributes := admission.NewAttributesRecord(tc.object, tc.oldObject, tc.kind, tc.namespace, tc.name, tc.resource, tc.subresource, admission.Create, false, &user.DefaultInfo{})
