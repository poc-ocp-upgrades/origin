...
func TestDeniedNamespace(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	test := &subjectAccessTest{authorizer: &testAuthorizer{allowed: kauthorizer.DecisionNoOpinion, err: "denied initial check", deniedNamespaces: sets.NewString("foo")}, reviewRequest: &authorizationapi.SubjectAccessReview{Action: authorizationapi.Action{Namespace: "foo", Verb: "get", Resource: "pods"}, User: "foo", Groups: sets.NewString()}, expectedError: "denied initial check"}
	test.runTest(t)
}
...
func TestNoErrors(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	test := &subjectAccessTest{authorizer: &testAuthorizer{allowed: kauthorizer.DecisionAllow, reason: "because good things"}, reviewRequest: &authorizationapi.SubjectAccessReview{Action: authorizationapi.Action{Verb: "delete", Resource: "deploymentConfigs"}, Groups: sets.NewString("not-master")}, expectedUserInfo: &user.DefaultInfo{Name: "", Groups: []string{"not-master"}, Extra: map[string][]string{}}}
	test.runTest(t)
}
...
func TestRegularWithScopes(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	test := &subjectAccessTest{authorizer: &testAuthorizer{allowed: kauthorizer.DecisionAllow, reason: "because good things"}, reviewRequest: &authorizationapi.SubjectAccessReview{Action: authorizationapi.Action{Verb: "delete", Resource: "deploymentConfigs"}, Groups: sets.NewString("not-master"), Scopes: []string{"scope-01"}}, expectedUserInfo: &user.DefaultInfo{Name: "", Groups: []string{"not-master"}, Extra: map[string][]string{authorizationapi.ScopesKey: {"scope-01"}}}, requestingUser: &user.DefaultInfo{Name: "", Groups: []string{"different"}, Extra: map[string][]string{authorizationapi.ScopesKey: {"scope-02"}}}}
	test.runTest(t)
}
...
func (r *subjectAccessTest) runTest(t *testing.T) {
	...
	expectedResponse := &authorizationapi.SubjectAccessReviewResponse{Namespace: r.reviewRequest.Action.Namespace, Allowed: r.authorizer.allowed == kauthorizer.DecisionAllow, Reason: r.authorizer.reason, EvaluationError: r.authorizer.err}
