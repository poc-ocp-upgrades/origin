...
func TestConflictingNamespace(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	authorizer := &testAuthorizer{allowed: kauthorizer.DecisionNoOpinion}
	reviewRequest := &authorizationapi.LocalSubjectAccessReview{Action: authorizationapi.Action{Namespace: "foo", Verb: "get", Resource: "pods"}, User: "foo", Groups: sets.NewString()}
...
func TestNoErrors(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	test := &subjectAccessTest{authorizer: &testAuthorizer{allowed: kauthorizer.DecisionAllow, reason: "because good things"}, reviewRequest: &authorizationapi.LocalSubjectAccessReview{Action: authorizationapi.Action{Namespace: "unittest", Verb: "delete", Resource: "deploymentConfigs"}, Groups: sets.NewString("not-master")}, expectedUserInfo: &user.DefaultInfo{Name: "", Groups: []string{"not-master"}, Extra: map[string][]string{}}}
	test.runTest(t)
}
...
func TestRegularWithScopes(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	test := &subjectAccessTest{authorizer: &testAuthorizer{allowed: kauthorizer.DecisionAllow, reason: "because good things"}, reviewRequest: &authorizationapi.LocalSubjectAccessReview{Action: authorizationapi.Action{Namespace: "unittest", Verb: "delete", Resource: "deploymentConfigs"}, Groups: sets.NewString("not-master"), Scopes: []string{"scope-01"}}, expectedUserInfo: &user.DefaultInfo{Name: "", Groups: []string{"not-master"}, Extra: map[string][]string{authorizationapi.ScopesKey: {"scope-01"}}}, requestingUser: &user.DefaultInfo{Name: "", Groups: []string{"different"}, Extra: map[string][]string{authorizationapi.ScopesKey: {"scope-02"}}}}
	test.runTest(t)
}
