...
func originFuzzer(t *testing.T, seed int64) *fuzz.Fuzzer {
	...
	f.Funcs(func(j *authorizationapi.RoleBinding, c fuzz.Continue) {
		...
		c.FuzzNoCustom(j)
		j.Objects = nil
		objs := []runtime.Object{&kapi.Pod{}, &apps.Deployment{}, &kapi.Service{}, &build.BuildConfig{}}
		for _, obj := range objs {
			...
			j.Objects = append(j.Objects, &runtime.Unknown{ContentType: "application/json", Raw: bytes.TrimRight(b, "\n")})
		}
		j.Objects = append(j.Objects, &runtime.Unknown{ContentType: "application/json", Raw: []byte(`{"kind":"Foo","apiVersion":"mygroup/v1","complex":{"a":true},"list":["item"],"bool":true,"int":1,"string":"hello"}`)})
		...
		*j.Weight = 100
		...
		case oapps.DeploymentStrategyTypeRolling:
			...
			policyTypes := []oapps.LifecycleHookFailurePolicy{oapps.LifecycleHookFailurePolicyRetry, oapps.LifecycleHookFailurePolicyAbort, oapps.LifecycleHookFailurePolicyIgnore}
			if c.RandBool() {
				params.Pre = &oapps.LifecycleHook{FailurePolicy: policyTypes[c.Rand.Intn(len(policyTypes))], ExecNewPod: &oapps.ExecNewPodHook{ContainerName: c.RandString()}}
			}
			if c.RandBool() {
				params.Post = &oapps.LifecycleHook{FailurePolicy: policyTypes[c.Rand.Intn(len(policyTypes))], ExecNewPod: &oapps.ExecNewPodHook{ContainerName: c.RandString()}}
			}
			if c.RandBool() {
		...
		*u64 = c.RandUint64() >> 8
		...
		volumeTypes := []securityapi.FSType{securityapi.FSTypeAWSElasticBlockStore, securityapi.FSTypeAzureFile, securityapi.FSTypeCephFS, securityapi.FSTypeCinder, securityapi.FSTypeDownwardAPI, securityapi.FSTypeEmptyDir, securityapi.FSTypeFC, securityapi.FSTypeFlexVolume, securityapi.FSTypeFlocker, securityapi.FSTypeGCEPersistentDisk, securityapi.FSTypeGitRepo, securityapi.FSTypeGlusterfs, securityapi.FSTypeISCSI, securityapi.FSTypeNFS, securityapi.FSTypePersistentVolumeClaim, securityapi.FSTypeRBD, securityapi.FSTypeSecret}
...
var dockerImageTypes = map[schema.GroupVersionKind]bool{image.SchemeGroupVersion.WithKind("DockerImage"): true}
