...
func TestGrant(t *testing.T) {
	...
	testCases := map[string]struct {
		...
		ExpectThen		string
	}{"display form": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: emptyAuthRegistry(), Path: "/grant?client_id=myclient&scope=myscope1%20myscope2&redirect_uri=/myredirect&then=/authorize", ExpectStatusCode: 200, ExpectContains: []string{`action="grant"`, `name="csrf" value="test"`, `name="client_id" value="myclient"`, `checked name="scope" value="myscope1"`, `checked name="scope" value="myscope2"`, `name="redirect_uri" value="/myredirect"`, `name="then" value="/authorize"`}}, "display form with existing scopes": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"newscope1", "newscope2", "existingscope1", "existingscope2"}), AuthRegistry: existingAuthRegistry([]string{"existingscope1", "existingscope2"}), Path: "/grant?client_id=myclient&scope=newscope1%20newscope2%20existingscope1%20existingscope2&redirect_uri=/myredirect&then=/authorize", ExpectStatusCode: 200, ExpectContains: []string{`action="grant"`, `name="csrf" value="test"`, `name="client_id" value="myclient"`, `checked name="scope" value="newscope1"`, `checked name="scope" value="newscope1"`, `type="hidden" name="scope" value="existingscope1"`, `type="hidden" name="scope" value="existingscope2"`, `name="redirect_uri" value="/myredirect"`, `name="then" value="/authorize"`}}, "Unauthenticated with redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: badAuth(nil), ClientRegistry: emptyClientRegistry(), AuthRegistry: emptyAuthRegistry(), Path: "/grant?then=/authorize", ExpectStatusCode: 302, ExpectRedirect: "/authorize"}, "Unauthenticated without redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: badAuth(nil), ClientRegistry: emptyClientRegistry(), AuthRegistry: emptyAuthRegistry(), Path: "/grant", ExpectStatusCode: 200, ExpectContains: []string{"reauthenticate"}}, "Auth error with redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: badAuth(errors.New("Auth error")), ClientRegistry: emptyClientRegistry(), AuthRegistry: emptyAuthRegistry(), Path: "/grant?then=/authorize", ExpectStatusCode: 302, ExpectRedirect: "/authorize"}, "Auth error without redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: badAuth(errors.New("Auth error")), ClientRegistry: emptyClientRegistry(), AuthRegistry: emptyAuthRegistry(), Path: "/grant", ExpectStatusCode: 200, ExpectContains: []string{"reauthenticate"}}, "error when POST fails CSRF": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: emptyAuthRegistry(), Path: "/grant", PostValues: url.Values{"client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"wrong"}}, ExpectStatusCode: 200, ExpectContains: []string{"CSRF"}}, "error when POST fails user check": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: emptyAuthRegistry(), Path: "/grant", PostValues: url.Values{"client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"wrong"}}, ExpectStatusCode: 200, ExpectContains: []string{"User did not match"}}, "error displaying form with invalid client": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: badClientRegistry(nil), AuthRegistry: emptyAuthRegistry(), Path: "/grant", ExpectStatusCode: 200, ExpectContains: []string{"find client"}}, "error submitting form with invalid client": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: badClientRegistry(nil), AuthRegistry: emptyAuthRegistry(), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 200, ExpectContains: []string{"find client"}}, "successful create grant with redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: emptyAuthRegistry(), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectCreatedAuthScopes: []string{"myscope1", "myscope2"}, ExpectRedirect: "/authorize?scope=myscope1+myscope2"}, "successful create grant without redirect": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: emptyAuthRegistry(), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 200, ExpectCreatedAuthScopes: []string{"myscope1", "myscope2"}, ExpectContains: []string{"granted", "no redirect"}}, "successful update grant with identical scopes": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: existingAuthRegistry([]string{"myscope2", "myscope1"}), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"myscope1", "myscope2"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectUpdatedAuthScopes: []string{"myscope1", "myscope2"}, ExpectRedirect: "/authorize?scope=myscope1+myscope2"}, "successful update grant with partial additional scopes": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"newscope1", "newscope2", "existingscope1", "existingscope2"}), AuthRegistry: existingAuthRegistry([]string{"existingscope2", "existingscope1"}), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"newscope1", "existingscope1"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize?scope=newscope1+newscope2+existingscope1"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectUpdatedAuthScopes: []string{"existingscope1", "existingscope2", "newscope1"}, ExpectRedirect: "/authorize?scope=newscope1+existingscope1"}, "successful update grant with additional scopes": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"newscope1", "existingscope1", "existingscope2"}), AuthRegistry: existingAuthRegistry([]string{"existingscope2", "existingscope1"}), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "scope": {"newscope1", "existingscope1"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectUpdatedAuthScopes: []string{"existingscope1", "existingscope2", "newscope1"}, ExpectRedirect: "/authorize?scope=newscope1+existingscope1"}, "successful reject grant via deny button": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: existingAuthRegistry([]string{"existingscope2", "existingscope1"}), Path: "/grant", PostValues: url.Values{"deny": {"true"}, "client_id": {"myclient"}, "scope": {"newscope1", "existingscope1"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectRedirect: "/authorize?error=access_denied"}, "successful reject grant via unchecking all requested scopes and approving": {CSRF: &csrf.FakeCSRF{Token: "test"}, Auth: goodAuth("username"), ClientRegistry: goodClientRegistry("myclient", []string{"myredirect"}, []string{"myscope1", "myscope2"}), AuthRegistry: existingAuthRegistry([]string{"existingscope2", "existingscope1"}), Path: "/grant", PostValues: url.Values{"approve": {"true"}, "client_id": {"myclient"}, "redirect_uri": {"/myredirect"}, "then": {"/authorize"}, "csrf": {"test"}, "user_name": {"username"}}, ExpectStatusCode: 302, ExpectRedirect: "/authorize?error=access_denied"}}
