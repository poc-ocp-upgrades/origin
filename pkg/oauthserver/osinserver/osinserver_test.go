...
import (
	...
	"golang.org/x/oauth2"
	"github.com/openshift/origin/pkg/oauthserver/osinserver/teststorage"
)
...
func TestClientCredentialFlow(t *testing.T) {
	...
	config := &oauth2.Config{ClientID: "test", ClientSecret: "secret", Scopes: []string{"a_scope"}, Endpoint: oauth2.Endpoint{AuthURL: server.URL + "/authorize", TokenURL: server.URL + "/token"}}
...
func TestAuthorizeStartFlow(t *testing.T) {
	...
	storage.Clients["test"] = &osin.DefaultClient{Id: "test", Secret: "secret", RedirectUri: assertServer.URL + "/assert"}
	oaclientConfig := &osincli.ClientConfig{ClientId: "test", ClientSecret: "secret", RedirectUrl: assertServer.URL + "/assert", AuthorizeUrl: server.URL + "/authorize", TokenUrl: server.URL + "/token"}
	...
	config := &oauth2.Config{ClientID: "test", ClientSecret: "", Scopes: []string{"a_scope"}, Endpoint: oauth2.Endpoint{AuthURL: server.URL + "/authorize", TokenURL: server.URL + "/token"}, RedirectURL: assertServer.URL + "/assert"}
