...
func TestGetEmptySessionSecretsFile(t *testing.T) {
	...
	defer os.Remove(tmpfile.Name())
	secrets := &config.SessionSecrets{Secrets: []config.SessionSecret{}}
...
func TestGetValidSessionSecretsFile(t *testing.T) {
	...
	defer os.Remove(tmpfile.Name())
	secrets := &config.SessionSecrets{Secrets: []config.SessionSecret{{Authentication: "a1", Encryption: "e1"}, {Authentication: "a2", Encryption: "e2"}}}
