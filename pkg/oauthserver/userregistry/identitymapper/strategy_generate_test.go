...
func TestStrategyGenerate(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	testcases := map[string]strategyTestCase{"no user": {MakeStrategy: NewStrategyGenerate, PreferredUsername: "bob", Identity: makeIdentity("", "idp", "bob", "", ""), ValidateActions: func(t *testing.T, actions []clienttesting.Action) {
		if len(actions) != 2 {
	...
	}, ExpectedUserName: "bob", ExpectedInitialize: true}, "existing user, no identities": {MakeStrategy: NewStrategyGenerate, PreferredUsername: "bob", Identity: makeIdentity("", "idp", "bob", "", ""), ExistingUsers: []runtime.Object{makeUser("bobUserUID", "bob")}, ValidateActions: func(t *testing.T, actions []clienttesting.Action) {
		if len(actions) != 3 {
			t.Fatal(spew.Sdump(actions))
		}
		if !actions[0].Matches("get", "users") || actions[0].(clienttesting.GetAction).GetName() != "bob" {
	...
	}, ExpectedUserName: "bob2", ExpectedInitialize: true}, "existing user, conflicting identity": {MakeStrategy: NewStrategyGenerate, PreferredUsername: "bob", Identity: makeIdentity("", "idp", "bob", "", ""), ExistingUsers: []runtime.Object{makeUser("bobUserUID", "bob", "otheridp:user")}, ValidateActions: func(t *testing.T, actions []clienttesting.Action) {
		if len(actions) != 3 {
			t.Fatal(spew.Sdump(actions))
		}
		if !actions[0].Matches("get", "users") || actions[0].(clienttesting.GetAction).GetName() != "bob" {
	...
	}, ExpectedUserName: "bob3", ExpectedInitialize: true}}
