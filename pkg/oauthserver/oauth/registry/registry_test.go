...
func TestRegistryAndServer(t *testing.T) {
	...
	validClient := &oauthv1.OAuthClient{ObjectMeta: metav1.ObjectMeta{Name: "test"}, Secret: "secret", RedirectURIs: []string{assertServer.URL + "/assert"}}
	restrictedClient := &oauthv1.OAuthClient{ObjectMeta: metav1.ObjectMeta{Name: "test"}, Secret: "secret", RedirectURIs: []string{assertServer.URL + "/assert"}, ScopeRestrictions: []oauthv1.ScopeRestriction{{ExactValues: []string{"user:info"}}}}
	...
	}{"needs auth": {Client: validClient, Check: func(h *testHandlers, _ *http.Request) {
	...
	}}, "needs grant": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user", UID: "1"}, Check: func(h *testHandlers, _ *http.Request) {
		if h.AuthNeed || !h.GrantNeed || h.AuthErr != nil || h.GrantErr != nil || h.HandleAuthorizeReq.Authorized {
			t.Errorf("expected request to need to grant access: %#v", h)
		}
	}}, "invalid scope": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user"}, Scope: "some-scope", Check: func(h *testHandlers, _ *http.Request) {
	...
	}}, "disallowed scope": {Client: restrictedClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user"}, Scope: "user:full", Check: func(h *testHandlers, _ *http.Request) {
	...
	}}, "has non covered grant": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user", UID: "1"}, ClientAuth: &oauthv1.OAuthClientAuthorization{ObjectMeta: metav1.ObjectMeta{Name: "user:test"}, UserName: "user", UserUID: "1", ClientName: "test", Scopes: []string{"user:info"}}, Scope: "user:info user:check-access", Check: func(h *testHandlers, req *http.Request) {
		if h.AuthNeed || !h.GrantNeed || h.AuthErr != nil || h.GrantErr != nil || h.HandleAuthorizeReq.Authorized {
			t.Errorf("expected request to need to grant access because of uncovered scopes: %#v", h)
		}
	}}, "has covered grant": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user", UID: "1"}, ClientAuth: &oauthv1.OAuthClientAuthorization{ObjectMeta: metav1.ObjectMeta{Name: "user:test"}, UserName: "user", UserUID: "1", ClientName: "test", Scopes: []string{"user:info", "user:check-access"}}, Scope: "user:info user:check-access", Check: func(h *testHandlers, req *http.Request) {
	...
	}}, "has auth and grant": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user", UID: "1"}, ClientAuth: &oauthv1.OAuthClientAuthorization{ObjectMeta: metav1.ObjectMeta{Name: "user:test"}, UserName: "user", UserUID: "1", ClientName: "test", Scopes: []string{"user:full"}}, Check: func(h *testHandlers, req *http.Request) {
		if h.AuthNeed || h.GrantNeed || h.AuthErr != nil || h.GrantErr != nil || !h.HandleAuthorizeReq.Authorized || h.HandleAuthorizeResp.ErrorId != "" {
			t.Errorf("unexpected flow: %#v, %#v, %#v", h, h.HandleAuthorizeReq, h.HandleAuthorizeResp)
			return
		}
	...
	}}, "has auth with no UID, mimics impersonation": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user"}, ClientAuth: &oauthv1.OAuthClientAuthorization{ObjectMeta: metav1.ObjectMeta{Name: "user:test"}, UserName: "user", UserUID: "2", ClientName: "test", Scopes: []string{"user:full"}}, Check: func(h *testHandlers, r *http.Request) {
	...
	}}, "has auth and grant with different UIDs": {Client: validClient, AuthSuccess: true, AuthUser: &user.DefaultInfo{Name: "user", UID: "1"}, ClientAuth: &oauthv1.OAuthClientAuthorization{ObjectMeta: metav1.ObjectMeta{Name: "user:test"}, UserName: "user", UserUID: "2", ClientName: "test", Scopes: []string{"user:full"}}, Check: func(h *testHandlers, _ *http.Request) {
		if h.AuthNeed || !h.GrantNeed || h.AuthErr != nil || h.GrantErr != nil || h.HandleAuthorizeReq.Authorized {
	...
	for _, testCase := range testCases {
		...
		h.AuthorizeHandler = osinserver.AuthorizeHandlers{handlers.NewAuthorizeAuthenticator(h, h, h), handlers.NewGrantCheck(NewClientAuthorizationGrantChecker(fakeOAuthClient.OauthV1().OAuthClientAuthorizations()), h, h)}
		server := osinserver.New(config, storage, h, osinserver.AccessHandlers{handlers.NewDenyAccessAuthenticator()}, h)
		mux := http.NewServeMux()
		server.Install(mux, "")
		s := httptest.NewServer(mux)
		oaclientConfig := &osincli.ClientConfig{ClientId: "test", ClientSecret: "secret", RedirectUrl: assertServer.URL + "/assert", AuthorizeUrl: s.URL + "/authorize", TokenUrl: s.URL + "/token", Scope: testCase.Scope}
