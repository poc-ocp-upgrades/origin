...
var (
	...
	condProgressing	= func() appsv1.DeploymentCondition {
		return appsv1.DeploymentCondition{Type: appsv1.DeploymentProgressing, Status: corev1.ConditionTrue, LastTransitionTime: now}
	}
	condProgressingDifferentTime	= func() appsv1.DeploymentCondition {
		return appsv1.DeploymentCondition{Type: appsv1.DeploymentProgressing, Status: corev1.ConditionTrue, LastTransitionTime: later}
	}
	condProgressingDifferentReason	= func() appsv1.DeploymentCondition {
		return appsv1.DeploymentCondition{Type: appsv1.DeploymentProgressing, Status: corev1.ConditionTrue, LastTransitionTime: later, Reason: NewReplicationControllerReason}
	}
	condNotProgressing	= func() appsv1.DeploymentCondition {
		return appsv1.DeploymentCondition{Type: appsv1.DeploymentProgressing, Status: corev1.ConditionFalse, LastUpdateTime: earlier, LastTransitionTime: earlier}
	}
	condAvailable	= func() appsv1.DeploymentCondition {
		return appsv1.DeploymentCondition{Type: appsv1.DeploymentAvailable, Status: corev1.ConditionTrue}
	}
)
...
func TestGetCondition(t *testing.T) {
	...
	tests := []struct {
		...
		expected	bool
	}{{name: "condition exists", status: exampleStatus(), condType: appsv1.DeploymentAvailable, expected: true}, {name: "condition does not exist", status: exampleStatus(), condType: appsv1.DeploymentReplicaFailure, expected: false}}
	for _, test := range tests {
...
func TestRemoveCondition(t *testing.T) {
	...
	tests := []struct {
		...
		condType	appsv1.DeploymentConditionType
		expectedStatus	*appsv1.DeploymentConfigStatus
	}{{name: "remove from empty status", status: &appsv1.DeploymentConfigStatus{}, condType: appsv1.DeploymentProgressing, expectedStatus: &appsv1.DeploymentConfigStatus{}}, {name: "simple remove", status: &appsv1.DeploymentConfigStatus{Conditions: []appsv1.DeploymentCondition{condProgressing()}}, condType: appsv1.DeploymentProgressing, expectedStatus: &appsv1.DeploymentConfigStatus{}}, {name: "doesn't remove anything", status: exampleStatus(), condType: appsv1.DeploymentReplicaFailure, expectedStatus: exampleStatus()}}
	for _, test := range tests {
...
func TestRolloutExceededTimeoutSeconds(t *testing.T) {
	...
	tests := []struct {
		...
		expectTimeout		bool
	}{{name: "recreate timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy.RecreateParams.TimeoutSeconds = &timeoutSeconds
		return config
	}(int64(10)), deploymentCreationTime: now.Add(-20 * time.Second), expectTimeout: true}, {name: "recreate no timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy.RecreateParams.TimeoutSeconds = &timeoutSeconds
		return config
	}(int64(0)), deploymentCreationTime: now.Add(-700 * time.Second), expectTimeout: false}, {name: "rolling timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy = appstest.OkRollingStrategy()
		config.Spec.Strategy.RollingParams.TimeoutSeconds = &timeoutSeconds
		return config
	}(int64(10)), deploymentCreationTime: now.Add(-20 * time.Second), expectTimeout: true}, {name: "rolling using default timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy = appstest.OkRollingStrategy()
		config.Spec.Strategy.RollingParams.TimeoutSeconds = nil
		return config
	}(0), deploymentCreationTime: now.Add(-20 * time.Second), expectTimeout: false}, {name: "recreate using default timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy.RecreateParams.TimeoutSeconds = nil
		return config
	}(0), deploymentCreationTime: now.Add(-20 * time.Second), expectTimeout: false}, {name: "custom using default timeout", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy = appstest.OkCustomStrategy()
		return config
	}(0), deploymentCreationTime: now.Add(-20 * time.Second), expectTimeout: false}, {name: "custom using default timeout timing out", config: func(timeoutSeconds int64) *appsv1.DeploymentConfig {
		config := appstest.OkDeploymentConfig(1)
		config.Spec.Strategy = appstest.OkCustomStrategy()
		return config
	}(0), deploymentCreationTime: now.Add(-700 * time.Second), expectTimeout: true}}
