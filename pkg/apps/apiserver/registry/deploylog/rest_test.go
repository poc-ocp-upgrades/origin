...
var (
	fakePodList = &corev1.PodList{Items: []corev1.Pod{{ObjectMeta: metav1.ObjectMeta{Name: "config-5-application-pod-1", Namespace: metav1.NamespaceDefault, CreationTimestamp: metav1.Date(2016, time.February, 1, 1, 0, 1, 0, time.UTC), Labels: testSelector}, Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "config-5-container-1"}}, NodeName: "some-host"}}, {ObjectMeta: metav1.ObjectMeta{Name: "config-5-application-pod-2", Namespace: metav1.NamespaceDefault, CreationTimestamp: metav1.Date(2016, time.February, 1, 1, 0, 3, 0, time.UTC), Labels: testSelector}, Spec: corev1.PodSpec{Containers: []corev1.Container{{Name: "config-5-container-2"}}, NodeName: "some-host"}}}}
)
...
func mockREST(version, desired int64, status appsv1.DeploymentStatus) *REST {
	...
	if desired > version {
		return &REST{dcClient: fakeDn.AppsV1(), timeout: defaultTimeout}
	}
	...
	if status == appsv1.DeploymentStatusComplete {
		...
		fakeDeployer := &kapi.Pod{ObjectMeta: metav1.ObjectMeta{Name: appsutil.DeployerPodNameForDeployment(obj.Name), Namespace: metav1.NamespaceDefault}, Spec: kapi.PodSpec{Containers: []kapi.Container{{Name: appsutil.DeployerPodNameForDeployment(obj.Name) + "-container"}}, NodeName: "some-host"}, Status: kapi.PodStatus{Phase: kapi.PodRunning}}
		fakePn.PrependReactor("get", "pods", func(action clientgotesting.Action) (handled bool, ret runtime.Object, err error) {
			return true, fakeDeployer, nil
		})
	}
	return &REST{dcClient: fakeDn.AppsV1(), rcClient: fakeRn.CoreV1(), podClient: fakePn.CoreV1(), timeout: defaultTimeout}
}
func TestRESTGet(t *testing.T) {
	...
	tests := []struct {
		...
		expectedName		string
		expectedErr		error
	}{{testName: "running deployment", rest: mockREST(1, 1, appsv1.DeploymentStatusRunning), name: "config", opts: &appsapi.DeploymentLogOptions{Follow: true, Version: intp(1)}, expectedNamespace: "default", expectedName: "config-1-deploy", expectedErr: nil}, {testName: "complete deployment", rest: mockREST(5, 5, appsv1.DeploymentStatusComplete), name: "config", opts: &appsapi.DeploymentLogOptions{Follow: true, Version: intp(5)}, expectedNamespace: "default", expectedName: "config-5-application-pod-1", expectedErr: nil}, {testName: "previous failed deployment", rest: mockREST(3, 2, appsv1.DeploymentStatusFailed), name: "config", opts: &appsapi.DeploymentLogOptions{Follow: false, Version: intp(2)}, expectedNamespace: "default", expectedName: "config-2-deploy", expectedErr: nil}, {testName: "previous deployment", rest: mockREST(3, 2, appsv1.DeploymentStatusFailed), name: "config", opts: &appsapi.DeploymentLogOptions{Follow: false, Previous: true}, expectedNamespace: "default", expectedName: "config-2-deploy", expectedErr: nil}, {testName: "non-existent previous deployment", rest: mockREST(1, 101, ""), name: "config", opts: &appsapi.DeploymentLogOptions{Follow: false, Previous: true}, expectedErr: errors.NewBadRequest("no previous deployment exists for deploymentConfig \"config\"")}}
