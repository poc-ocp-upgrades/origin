...
func TestGeneration(t *testing.T) {
	...
	from.Spec.Selector = map[string]string{"new1": "new2", "new2": "new2"}
	to := appstest.OkDeploymentConfig(1)
	rollbackSpecs := []*appsapi.DeploymentConfigRollbackSpec{}
	for i := 0; i < 15; i++ {
		spec := &appsapi.DeploymentConfigRollbackSpec{From: kapi.ObjectReference{Name: "deployment", Namespace: metav1.NamespaceDefault}, IncludeTriggers: i&(1<<0) > 0, IncludeTemplate: i&(1<<1) > 0, IncludeReplicationMeta: i&(1<<2) > 0, IncludeStrategy: i&(1<<3) > 0}
