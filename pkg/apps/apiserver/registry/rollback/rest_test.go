...
func TestCreateOk(t *testing.T) {
	...
	obj, err := NewREST(oc, kc).Create(apirequest.NewDefaultContext(), &appsapi.DeploymentConfigRollback{Name: "config", Spec: appsapi.DeploymentConfigRollbackSpec{Revision: 1}}, apiserverrest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	if err != nil {
...
func TestCreateRollbackToLatest(t *testing.T) {
	...
	_, err := NewREST(oc, &fake.Clientset{}).Create(apirequest.NewDefaultContext(), &appsapi.DeploymentConfigRollback{Name: "config", Spec: appsapi.DeploymentConfigRollbackSpec{Revision: 2}}, apiserverrest.ValidateAllObjectFunc, &metav1.CreateOptions{})
	if err == nil {
...
func TestCreateGeneratorError(t *testing.T) {
	...
	rest := REST{generator: &terribleGenerator{}, dn: oc.AppsV1(), rn: kc.CoreV1()}
	_, err := rest.Create(apirequest.NewDefaultContext(), &appsapi.DeploymentConfigRollback{Name: "config", Spec: appsapi.DeploymentConfigRollbackSpec{Revision: 1}}, apiserverrest.ValidateAllObjectFunc, &metav1.CreateOptions{})
