...
func deployerPod(deployment *corev1.ReplicationController, alternateName string, related bool) *corev1.Pod {
	...
	pod := &corev1.Pod{ObjectMeta: metav1.ObjectMeta{Name: deployerPodName, Namespace: deployment.Namespace, Labels: map[string]string{appsv1.DeployerPodForDeploymentLabel: deployment.Name}, Annotations: map[string]string{appsv1.DeploymentAnnotation: deployment.Name}}}
	if !related {
...
func TestMakeDeployerContainer(t *testing.T) {
	...
	}{{name: "simple var should be injected", environment: []corev1.EnvVar{{Name: "FOO", Value: "BAR"}}, expected: []corev1.EnvVar{{Name: "FOO", Value: "BAR"}}}, {name: "big vars should not be injected", environment: []corev1.EnvVar{{Name: "FOO", Value: randSeq(1001 * 128)}}, expected: []corev1.EnvVar{}}}
...
func TestHandle_deployerPodAlreadyExists(t *testing.T) {
	...
	}{{name: "pending", podPhase: corev1.PodPending, expected: appsv1.DeploymentStatusPending}, {name: "running", podPhase: corev1.PodRunning, expected: appsv1.DeploymentStatusRunning}, {name: "complete", podPhase: corev1.PodFailed, expected: appsv1.DeploymentStatusFailed}, {name: "failed", podPhase: corev1.PodSucceeded, expected: appsv1.DeploymentStatusComplete}}
	for _, test := range tests {
...
func TestHandle_noop(t *testing.T) {
	...
	}{{name: "pending", podPhase: corev1.PodPending, deploymentPhase: appsv1.DeploymentStatusPending}, {name: "running", podPhase: corev1.PodRunning, deploymentPhase: appsv1.DeploymentStatusRunning}, {name: "complete", podPhase: corev1.PodFailed, deploymentPhase: appsv1.DeploymentStatusFailed}}
	for _, test := range tests {
...
func TestHandle_cleanupPostNew(t *testing.T) {
	...
	tests := []struct {
		...
		podPhase	corev1.PodPhase
		expected	int
	}{{name: "pending", deploymentPhase: appsv1.DeploymentStatusPending, podPhase: corev1.PodPending, expected: len(hookPods) + 1}, {name: "running", deploymentPhase: appsv1.DeploymentStatusRunning, podPhase: corev1.PodRunning, expected: len(hookPods) + 1}, {name: "failed", deploymentPhase: appsv1.DeploymentStatusFailed, podPhase: corev1.PodFailed, expected: len(hookPods) + 1}, {name: "complete", deploymentPhase: appsv1.DeploymentStatusComplete, podPhase: corev1.PodSucceeded, expected: 0}}
	for _, test := range tests {
...
func TestHandle_deployerPodDisappeared(t *testing.T) {
	...
	tests := []struct {
		...
		willBeDropped	bool
		shouldRetry	bool
	}{{name: "pending - retry", phase: appsv1.DeploymentStatusPending, willBeDropped: false, shouldRetry: true}, {name: "pending - fail", phase: appsv1.DeploymentStatusPending, willBeDropped: true, shouldRetry: false}, {name: "running", phase: appsv1.DeploymentStatusRunning}}
	for _, test := range tests {
		var updatedDeployment *corev1.ReplicationController
		updateCalled := false
		client := &fake.Clientset{}
...
func TestHandle_transitionFromDeployer(t *testing.T) {
	...
	tests := []struct {
		...
		deploymentPhase	appsv1.DeploymentStatus
		expected	appsv1.DeploymentStatus
	}{{name: "New -> Pending", podPhase: corev1.PodPending, deploymentPhase: appsv1.DeploymentStatusNew, expected: appsv1.DeploymentStatusPending}, {name: "New -> Running", podPhase: corev1.PodRunning, deploymentPhase: appsv1.DeploymentStatusNew, expected: appsv1.DeploymentStatusRunning}, {name: "New -> Complete", podPhase: corev1.PodSucceeded, deploymentPhase: appsv1.DeploymentStatusNew, expected: appsv1.DeploymentStatusComplete}, {name: "New -> Failed", podPhase: corev1.PodFailed, deploymentPhase: appsv1.DeploymentStatusNew, expected: appsv1.DeploymentStatusFailed}, {name: "Pending -> Running", podPhase: corev1.PodRunning, deploymentPhase: appsv1.DeploymentStatusPending, expected: appsv1.DeploymentStatusRunning}, {name: "Pending -> Complete", podPhase: corev1.PodSucceeded, deploymentPhase: appsv1.DeploymentStatusPending, expected: appsv1.DeploymentStatusComplete}, {name: "Pending -> Failed", podPhase: corev1.PodFailed, deploymentPhase: appsv1.DeploymentStatusPending, expected: appsv1.DeploymentStatusFailed}, {name: "Running -> Complete", podPhase: corev1.PodSucceeded, deploymentPhase: appsv1.DeploymentStatusRunning, expected: appsv1.DeploymentStatusComplete}, {name: "Running -> Failed", podPhase: corev1.PodFailed, deploymentPhase: appsv1.DeploymentStatusRunning, expected: appsv1.DeploymentStatusFailed}}
	for _, test := range tests {
		var updatedDeployment *corev1.ReplicationController
		updateCalled := false
		client := &fake.Clientset{}
...
func TestMakeDeployerPod(t *testing.T) {
	...
	for i := 1; i <= 25; i++ {
		...
		f.Funcs(func(p *corev1.PodTemplateSpec, c fuzz.Continue) {
			c.FuzzNoCustom(p)
			...
			p.Spec.DNSConfig = &corev1.PodDNSConfig{Nameservers: []string{"8.8.8.8"}, Searches: []string{"svc.cluster.local", "cluster.local"}}
		...
		if !reflect.DeepEqual(inputPodTemplate.Spec, outputPodTemplate.Spec) {
			t.Fatalf("Deployer pod is missing fields:\n%s\n\n%s", diff.ObjectReflectDiff(inputPodTemplate.Spec, outputPodTemplate.Spec), diff.ObjectDiff(inputPodTemplate.Spec, outputPodTemplate.Spec))
		}
	}
}
