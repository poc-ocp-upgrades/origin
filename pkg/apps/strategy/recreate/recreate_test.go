...
func recreateParams(timeout int64, preFailurePolicy, midFailurePolicy, postFailurePolicy appsv1.LifecycleHookFailurePolicy) appsv1.DeploymentStrategy {
	...
	if len(midFailurePolicy) > 0 {
		mid = &appsv1.LifecycleHook{FailurePolicy: midFailurePolicy, ExecNewPod: &appsv1.ExecNewPodHook{}}
	}
	if len(postFailurePolicy) > 0 {
		post = &appsv1.LifecycleHook{FailurePolicy: postFailurePolicy, ExecNewPod: &appsv1.ExecNewPodHook{}}
	}
	return appsv1.DeploymentStrategy{Type: appsv1.DeploymentStrategyTypeRecreate, RecreateParams: &appsv1.RecreateDeploymentStrategyParams{TimeoutSeconds: &timeout, Pre: pre, Mid: mid, Post: post}}
}
...
func TestRecreate_initialDeployment(t *testing.T) {
	...
	strategy := &RecreateDeploymentStrategy{out: &bytes.Buffer{}, errOut: &bytes.Buffer{}, getUpdateAcceptor: getUpdateAcceptor, eventClient: fake.NewSimpleClientset().CoreV1()}
...
func TestRecreate_deploymentMidHookSuccess(t *testing.T) {
	...
	deployment, _ := appsutil.MakeDeployment(config)
	controllerClient := newFakeControllerClient(deployment)
	strategy := &RecreateDeploymentStrategy{out: &bytes.Buffer{}, errOut: &bytes.Buffer{}, rcClient: controllerClient, scaleClient: controllerClient.fakeScaleClient(), eventClient: fake.NewSimpleClientset().CoreV1(), getUpdateAcceptor: getUpdateAcceptor, hookExecutor: &hookExecutorImpl{executeFunc: func(hook *appsv1.LifecycleHook, deployment *corev1.ReplicationController, suffix, label string) error {
		return fmt.Errorf("hook execution failure")
	}}}
	strategy.podClient = &fakePodClient{deployerName: appsutil.DeployerPodNameForDeployment(deployment.Name)}
	err := strategy.Deploy(nil, deployment, 2)
...
func TestRecreate_acceptorSuccess(t *testing.T) {
	...
	acceptor := &testAcceptor{acceptFn: func(deployment *corev1.ReplicationController) error {
		acceptorCalled = true
		return nil
	}}
