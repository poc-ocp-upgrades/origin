...
func getDockerRegistryLocations(lister listers.ServiceLister, location serviceLocation, clusterDNSSuffix string) []string {
	...
	if hasClusterIP && len(service.Spec.Ports) > 0 {
		...
		ret := []string{net.JoinHostPort(service.Spec.ClusterIP, fmt.Sprintf("%d", svcPort)), net.JoinHostPort(fmt.Sprintf("%s.%s.svc", service.Name, service.Namespace), fmt.Sprintf("%d", svcPort))}
