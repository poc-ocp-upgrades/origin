...
var (
	registryService = &v1.Service{ObjectMeta: metav1.ObjectMeta{Name: registryName, Namespace: registryNamespace}, Spec: v1.ServiceSpec{ClusterIP: "172.16.123.123", Ports: []v1.ServicePort{{Port: 443}}}}
)
...
func controllerSetup(startingObjects []runtime.Object, t *testing.T, stopCh <-chan struct{}) (*fake.Clientset, *watch.FakeWatcher, *DockerRegistryServiceController, informers.SharedInformerFactory) {
	...
	kubeclient.PrependWatchReactor("services", func(action clientgotesting.Action) (handled bool, ret watch.Interface, err error) {
		return true, fakeWatch, nil
	})
	informerFactory := informers.NewSharedInformerFactory(kubeclient, controller.NoResyncPeriodFunc())
	controller := NewDockerRegistryServiceController(informerFactory.Core().V1().Secrets(), informerFactory.Core().V1().Services(), kubeclient, DockerRegistryServiceControllerOptions{Resync: 10 * time.Minute, DockercfgController: &DockercfgController{}, DockerURLsInitialized: make(chan struct{})})
	controller.initialSecretsCheckDone = true
	controller.secretsSynced = func() bool {
		return true
	}
...
func TestUpdateNewStyleSecretAndDNSSuffixAndAdditionalURLs(t *testing.T) {
	...
	newStyleDockercfgSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret-name", Namespace: registryNamespace, Annotations: map[string]string{ServiceAccountTokenValueAnnotation: "the-token", ServiceAccountTokenSecretNameKey: "sa-token-secret"}}, Type: v1.SecretTypeDockercfg, Data: map[string][]byte{v1.DockerConfigKey: []byte("{}")}}
	...
	expectedLocations := []string{"foo.bar.com", "172.16.123.123:443", "172.16.123.123", "docker-registry.default.svc:443", "docker-registry.default.svc", "docker-registry.default.svc.something.else:443", "docker-registry.default.svc.something.else"}
	for _, key := range expectedLocations {
		expectedDockercfgMap[key] = credentialprovider.DockerConfigEntry{Username: "serviceaccount", Password: newStyleDockercfgSecret.Annotations[ServiceAccountTokenValueAnnotation], Email: "serviceaccount@example.org"}
	}
	foundSecret := false
...
func TestUpdateOldStyleSecretWithKey(t *testing.T) {
	...
	for _, key := range []string{"somekey"} {
		existingDockercfgMap[key] = credentialprovider.DockerConfigEntry{Username: "serviceaccount", Password: "token-value", Email: "serviceaccount@example.org"}
	}
	dockercfgContent, err := json.Marshal(&existingDockercfgMap)
	if err != nil {
		t.Fatalf("unexpected err %v", err)
	}
	oldStyleDockercfgSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret-name", Namespace: registryNamespace, Annotations: map[string]string{ServiceAccountTokenSecretNameKey: "sa-token-secret"}}, Type: v1.SecretTypeDockercfg, Data: map[string][]byte{v1.DockerConfigKey: dockercfgContent}}
	...
	expectedDockercfgMap := credentialprovider.DockerConfig{}
	expectedLocations := []string{"172.16.123.123:443", "172.16.123.123", "docker-registry.default.svc:443", "docker-registry.default.svc"}
	for _, key := range expectedLocations {
		expectedDockercfgMap[key] = credentialprovider.DockerConfigEntry{Username: "serviceaccount", Password: "token-value", Email: "serviceaccount@example.org"}
	}
	foundSecret := false
	for _, action := range kubeclient.Actions() {
		switch {
...
func TestUpdateOldStyleSecretWithoutKey(t *testing.T) {
	...
	received := make(chan bool)
	updatedSecret := make(chan bool)
	oldStyleDockercfgSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret-name", Namespace: registryNamespace, Annotations: map[string]string{ServiceAccountTokenSecretNameKey: "sa-token-secret"}}, Type: v1.SecretTypeDockercfg, Data: map[string][]byte{v1.DockerConfigKey: []byte("{}")}}
	tokenSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "sa-token-secret", Namespace: registryNamespace, Annotations: map[string]string{ServiceAccountTokenSecretNameKey: "sa-token-secret"}}, Type: v1.SecretTypeServiceAccountToken, Data: map[string][]byte{v1.ServiceAccountTokenKey: []byte("the-sa-bearer-token")}}
	...
	expectedLocations := []string{"172.16.123.123:443", "172.16.123.123", "docker-registry.default.svc:443", "docker-registry.default.svc"}
	for _, key := range expectedLocations {
		expectedDockercfgMap[key] = credentialprovider.DockerConfigEntry{Username: "serviceaccount", Password: "the-sa-bearer-token", Email: "serviceaccount@example.org"}
	}
	foundSecret := false
	for _, action := range kubeclient.Actions() {
		switch {
...
func TestClearSecretAndRecreate(t *testing.T) {
	...
	oldStyleDockercfgSecret := &v1.Secret{ObjectMeta: metav1.ObjectMeta{Name: "secret-name", Namespace: registryNamespace, Annotations: map[string]string{ServiceAccountTokenValueAnnotation: "the-token", ServiceAccountTokenSecretNameKey: "sa-token-secret"}}, Type: v1.SecretTypeDockercfg, Data: map[string][]byte{v1.DockerConfigKey: dockercfgContent}}
	...
	expectedLocations := []string{"172.16.123.123:443", "172.16.123.123", "docker-registry.default.svc:443", "docker-registry.default.svc"}
	for _, key := range expectedLocations {
		expectedDockercfgMap[key] = credentialprovider.DockerConfigEntry{Username: "serviceaccount", Password: "the-token", Email: "serviceaccount@example.org"}
	}
	foundSecret := false
	for _, action := range kubeclient.Actions() {
		switch {
