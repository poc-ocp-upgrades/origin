...
func main() {
	...
	root := &cobra.Command{Long: templates.LongDesc(`
		OpenShift Tests
	...
	root.AddCommand(newRunCommand(), newRunUpgradeCommand(), newRunTestCommand(), newRunMonitorCommand())
...
func newRunMonitorCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "run-monitor", Short: "Continuously verify the cluster is functional", Long: templates.LongDesc(`
		...
		`), SilenceUsage: true, SilenceErrors: true, RunE: func(cmd *cobra.Command, args []string) error {
...
func newRunCommand() *cobra.Command {
	_logClusterCodePath()
	defer _logClusterCodePath()
	opt := &testginkgo.Options{Suites: staticSuites}
	cmd := &cobra.Command{Use: "run SUITE", Short: "Run a test suite", Long: templates.LongDesc(`
...
func newRunUpgradeCommand() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "run-upgrade SUITE", Short: "Run an upgrade suite", Long: templates.LongDesc(`
