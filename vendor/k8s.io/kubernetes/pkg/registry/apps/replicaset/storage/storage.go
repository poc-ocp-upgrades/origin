...
func NewStorage(optsGetter generic.RESTOptionsGetter) ReplicaSetStorage {
	...
	return ReplicaSetStorage{ReplicaSet: replicaSetRest, Status: replicaSetStatusRest, Scale: &ScaleREST{store: replicaSetRest.Store}}
}
...
func NewREST(optsGetter generic.RESTOptionsGetter) (*REST, *StatusREST) {
	...
	}, PredicateFunc: replicaset.MatchReplicaSet, DefaultQualifiedResource: apps.Resource("replicasets"), CreateStrategy: replicaset.Strategy, UpdateStrategy: replicaset.Strategy, DeleteStrategy: replicaset.Strategy, TableConvertor: printerstorage.TableConvertor{TablePrinter: printers.NewTablePrinter().With(printersinternal.AddHandlers)}}
...
func scaleFromReplicaSet(rs *apps.ReplicaSet) (*autoscaling.Scale, error) {
	...
	return &autoscaling.Scale{ObjectMeta: metav1.ObjectMeta{Name: rs.Name, Namespace: rs.Namespace, UID: rs.UID, ResourceVersion: rs.ResourceVersion, CreationTimestamp: rs.CreationTimestamp}, Spec: autoscaling.ScaleSpec{Replicas: rs.Spec.Replicas}, Status: autoscaling.ScaleStatus{Replicas: rs.Status.Replicas, Selector: selector.String()}}, nil
}
