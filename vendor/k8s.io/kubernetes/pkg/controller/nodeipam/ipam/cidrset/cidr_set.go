...
var (
 ErrCIDRRangeNoCIDRsRemaining = errors.New("CIDR allocation failed; there are no remaining CIDRs left to allocate in the accepted range")
 ErrCIDRSetSubNetTooBig       = errors.New("New CIDR set failed; the node CIDR size is too big")
)
...
func NewCIDRSet(clusterCIDR *net.IPNet, subNetMaskSize int) (*CidrSet, error) {
 ...
 return &CidrSet{clusterCIDR: clusterCIDR, clusterIP: clusterCIDR.IP, clusterMaskSize: clusterMaskSize, maxCIDRs: maxCIDRs, subNetMaskSize: subNetMaskSize}, nil
}
func (s *CidrSet) indexToCIDRBlock(index int) *net.IPNet {
 ...
 return &net.IPNet{IP: ip, Mask: net.CIDRMask(s.subNetMaskSize, mask)}
}
...
func (s *CidrSet) getBeginingAndEndIndices(cidr *net.IPNet) (begin, end int, err error) {
 ...
 if s.clusterMaskSize < maskSize {
  ...
  begin, err = s.getIndexForCIDR(&net.IPNet{IP: cidr.IP.Mask(subNetMask), Mask: subNetMask})
  if err != nil {
   return -1, -1, err
  }
  ip := make([]byte, ipSize)
  ...
  end, err = s.getIndexForCIDR(&net.IPNet{IP: net.IP(ip).Mask(subNetMask), Mask: subNetMask})
  if err != nil {
   return -1, -1, err
  }
 }
