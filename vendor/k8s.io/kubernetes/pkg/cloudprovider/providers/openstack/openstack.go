...
type MyDuration struct{ time.Duration }
...
func (cfg Config) toAuthOptions() gophercloud.AuthOptions {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	return gophercloud.AuthOptions{IdentityEndpoint: cfg.Global.AuthURL, Username: cfg.Global.Username, UserID: cfg.Global.UserID, Password: cfg.Global.Password, TenantID: cfg.Global.TenantID, TenantName: cfg.Global.TenantName, DomainID: cfg.Global.DomainID, DomainName: cfg.Global.DomainName, AllowReauth: true}
}
func (cfg Config) toAuth3Options() tokens3.AuthOptions {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	return tokens3.AuthOptions{IdentityEndpoint: cfg.Global.AuthURL, Username: cfg.Global.Username, UserID: cfg.Global.UserID, Password: cfg.Global.Password, DomainID: cfg.Global.DomainID, DomainName: cfg.Global.DomainName, AllowReauth: true}
}
func configFromEnv() (cfg Config, ok bool) {
	...
	ok = cfg.Global.AuthURL != "" && cfg.Global.Username != "" && cfg.Global.Password != "" && (cfg.Global.TenantID != "" || cfg.Global.TenantName != "" || cfg.Global.DomainID != "" || cfg.Global.DomainName != "" || cfg.Global.Region != "" || cfg.Global.UserID != "" || cfg.Global.TrustID != "")
...
func newOpenStack(cfg Config) (*OpenStack, error) {
	...
	if cfg.Global.TrustID != "" {
		...
		authOptsExt := trusts.AuthOptsExt{TrustID: cfg.Global.TrustID, AuthOptionsBuilder: &opts}
	...
	os := OpenStack{provider: provider, region: cfg.Global.Region, lbOpts: cfg.LoadBalancer, bsOpts: cfg.BlockStorage, routeOpts: cfg.Route, metadataOpts: cfg.Metadata}
...
func getServerByName(client *gophercloud.ServiceClient, name types.NodeName) (*servers.Server, error) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	opts := servers.ListOpts{Name: fmt.Sprintf("^%s$", regexp.QuoteMeta(mapNodeNameToServerName(name)))}
	pager := servers.List(client, opts)
...
func nodeAddresses(srv *servers.Server) ([]v1.NodeAddress, error) {
	...
	for network, addrList := range addresses {
		for _, props := range addrList {
			...
			v1helper.AddToNodeAddresses(&addrs, v1.NodeAddress{Type: addressType, Address: props.Addr})
		}
	}
	if srv.AccessIPv4 != "" {
		v1helper.AddToNodeAddresses(&addrs, v1.NodeAddress{Type: v1.NodeExternalIP, Address: srv.AccessIPv4})
	}
	if srv.AccessIPv6 != "" {
		v1helper.AddToNodeAddresses(&addrs, v1.NodeAddress{Type: v1.NodeExternalIP, Address: srv.AccessIPv6})
	}
	if srv.Metadata[TypeHostName] != "" {
		v1helper.AddToNodeAddresses(&addrs, v1.NodeAddress{Type: v1.NodeHostName, Address: srv.Metadata[TypeHostName]})
	}
	return addrs, nil
}
...
func (os *OpenStack) GetZoneByProviderID(ctx context.Context, providerID string) (cloudprovider.Zone, error) {
	...
	zone := cloudprovider.Zone{FailureDomain: srv.Metadata[availabilityZone], Region: os.region}
...
func (os *OpenStack) volumeService(forceVersion string) (volumeService, error) {
	...
	case "auto":
		...
		errTxt := "BlockStorage API version autodetection failed. " + "Please set it explicitly in cloud.conf in section [BlockStorage] with key `bs-version`"
...
func checkMetadataSearchOrder(order string) error {
	...
	for _, id := range elements {
		...
		case metadataID:
		default:
			return fmt.Errorf("invalid element %q found in section [Metadata] with key `search-order`."+"Supported elements include %q and %q", id, configDriveID, metadataID)
		}
	}
	return nil
}
