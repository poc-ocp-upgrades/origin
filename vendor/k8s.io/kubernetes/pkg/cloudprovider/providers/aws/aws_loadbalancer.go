...
func (c *Cloud) ensureLoadBalancerv2(namespacedName types.NamespacedName, loadBalancerName string, mappings []nlbPortMapping, instanceIDs, subnetIDs []string, internalELB bool, annotations map[string]string) (*elbv2.LoadBalancer, error) {
	...
	if loadBalancer == nil {
		createRequest := &elbv2.CreateLoadBalancerInput{Type: aws.String(elbv2.LoadBalancerTypeEnumNetwork), Name: aws.String(loadBalancerName)}
		if internalELB {
			createRequest.Scheme = aws.String("internal")
		}
		createRequest.SubnetMappings = createSubnetMappings(subnetIDs)
		for k, v := range tags {
			createRequest.Tags = append(createRequest.Tags, &elbv2.Tag{Key: aws.String(k), Value: aws.String(v)})
		}
		...
		addTagsInput := &elbv2.AddTagsInput{ResourceArns: []*string{}, Tags: []*elbv2.Tag{}}
		...
		{
			listenerDescriptions, err := c.elbv2.DescribeListeners(&elbv2.DescribeListenersInput{LoadBalancerArn: loadBalancer.LoadBalancerArn})
			if err != nil {
			...
			actualTargetGroups, err := c.elbv2.DescribeTargetGroups(&elbv2.DescribeTargetGroupsInput{LoadBalancerArn: loadBalancer.LoadBalancerArn})
			if err != nil {
			...
			for _, mapping := range mappings {
				...
				if listener, ok := actual[frontendPort]; ok {
					if targetGroup, ok := nodePortTargetGroup[nodePort]; !ok {
						...
						targetGroup, err = c.ensureTargetGroup(nil, mapping, instanceIDs, targetName, *loadBalancer.VpcId)
						if err != nil {
							return nil, err
						}
						_, err := c.elbv2.ModifyListener(&elbv2.ModifyListenerInput{ListenerArn: listener.ListenerArn, Port: aws.Int64(frontendPort), Protocol: aws.String("TCP"), DefaultActions: []*elbv2.Action{{TargetGroupArn: targetGroup.TargetGroupArn, Type: aws.String("forward")}}})
						if err != nil {
						...
						targetName := createTargetName(namespacedName, frontendPort, nodePort)
						_, err = c.ensureTargetGroup(targetGroup, mapping, instanceIDs, targetName, *loadBalancer.VpcId)
						if err != nil {
							return nil, err
						}
					}
		...
		if dirty {
			loadBalancers, err := c.elbv2.DescribeLoadBalancers(&elbv2.DescribeLoadBalancersInput{LoadBalancerArns: []*string{loadBalancer.LoadBalancerArn}})
			if err != nil {
...
func (c *Cloud) createListenerV2(loadBalancerArn *string, mapping nlbPortMapping, namespacedName types.NamespacedName, instanceIDs []string, vpcID string) (listener *elbv2.Listener, targetGroupArn *string, err error) {
	...
	target, err := c.ensureTargetGroup(nil, mapping, instanceIDs, targetName, vpcID)
	if err != nil {
		return nil, aws.String(""), err
	}
	createListernerInput := &elbv2.CreateListenerInput{LoadBalancerArn: loadBalancerArn, Port: aws.Int64(mapping.FrontendPort), Protocol: aws.String("TCP"), DefaultActions: []*elbv2.Action{{TargetGroupArn: target.TargetGroupArn, Type: aws.String(elbv2.ActionTypeEnumForward)}}}
...
func (c *Cloud) ensureTargetGroup(targetGroup *elbv2.TargetGroup, mapping nlbPortMapping, instances []string, name string, vpcID string) (*elbv2.TargetGroup, error) {
	...
	if targetGroup == nil {
		...
		registerInput := &elbv2.RegisterTargetsInput{TargetGroupArn: result.TargetGroups[0].TargetGroupArn, Targets: []*elbv2.TargetDescription{}}
		for _, instanceID := range instances {
			registerInput.Targets = append(registerInput.Targets, &elbv2.TargetDescription{Id: aws.String(string(instanceID)), Port: aws.Int64(mapping.TrafficPort)})
		}
	...
	}
	{
		...
		if len(additions) > 0 {
			registerInput := &elbv2.RegisterTargetsInput{TargetGroupArn: targetGroup.TargetGroupArn, Targets: []*elbv2.TargetDescription{}}
			for instanceID := range additions {
				registerInput.Targets = append(registerInput.Targets, &elbv2.TargetDescription{Id: aws.String(instanceID), Port: aws.Int64(mapping.TrafficPort)})
			}
			_, err := c.elbv2.RegisterTargets(registerInput)
			if err != nil {
		...
		if len(removals) > 0 {
			deregisterInput := &elbv2.DeregisterTargetsInput{TargetGroupArn: targetGroup.TargetGroupArn, Targets: []*elbv2.TargetDescription{}}
			for instanceID := range removals {
				deregisterInput.Targets = append(deregisterInput.Targets, &elbv2.TargetDescription{Id: aws.String(instanceID), Port: aws.Int64(mapping.TrafficPort)})
			}
	...
	if dirty {
		result, err := c.elbv2.DescribeTargetGroups(&elbv2.DescribeTargetGroupsInput{Names: []*string{aws.String(name)}})
		if err != nil {
...
func (c *Cloud) getVpcCidrBlocks() ([]string, error) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	vpcs, err := c.ec2.DescribeVpcs(&ec2.DescribeVpcsInput{VpcIds: []*string{aws.String(c.vpcID)}})
	if err != nil {
	...
	for instanceSecurityGroupID, portMap := range portChanges {
		...
		for port, add := range portMap {
			...
			if clientTraffic {
				...
				permission := &ec2.IpPermission{FromPort: aws.Int64(port), ToPort: aws.Int64(port), IpProtocol: aws.String("tcp")}
				ranges := []*ec2.IpRange{}
				for _, cidr := range clientCidrs {
					ranges = append(ranges, &ec2.IpRange{CidrIp: aws.String(cidr), Description: aws.String(clientRuleAnnotation)})
				}
				...
				ranges := []*ec2.IpRange{}
				for _, cidr := range clientCidrs {
					ranges = append(ranges, &ec2.IpRange{CidrIp: aws.String(cidr), Description: aws.String(healthRuleAnnotation)})
				}
				permission.IpRanges = ranges
				if add {
					adds = append(adds, permission)
		...
		if clientTraffic {
			...
			mtuPermission := &ec2.IpPermission{IpProtocol: aws.String("icmp"), FromPort: aws.Int64(3), ToPort: aws.Int64(4)}
			ranges := []*ec2.IpRange{}
			for _, cidr := range clientCidrs {
				ranges = append(ranges, &ec2.IpRange{CidrIp: aws.String(cidr), Description: aws.String(mtuRuleAnnotation)})
			}
...
func (c *Cloud) ensureLoadBalancer(namespacedName types.NamespacedName, loadBalancerName string, listeners []*elb.Listener, subnetIDs []string, securityGroupIDs []string, internalELB, proxyProtocol bool, loadBalancerAttributes *elb.LoadBalancerAttributes, annotations map[string]string) (*elb.LoadBalancerDescription, error) {
	...
	if loadBalancer == nil {
		...
		tags = c.tagging.buildTags(ResourceLifecycleOwned, tags)
		for k, v := range tags {
			createRequest.Tags = append(createRequest.Tags, &elb.Tag{Key: aws.String(k), Value: aws.String(v)})
		}
		klog.Infof("Creating load balancer for %v with name: %s", namespacedName, loadBalancerName)
...
func (c *Cloud) ensureLoadBalancerHealthCheck(loadBalancer *elb.LoadBalancerDescription, protocol string, port int32, path string, annotations map[string]string) error {
	...
	if aws.StringValue(expected.Target) == aws.StringValue(actual.Target) && aws.Int64Value(expected.HealthyThreshold) == aws.Int64Value(actual.HealthyThreshold) && aws.Int64Value(expected.UnhealthyThreshold) == aws.Int64Value(actual.UnhealthyThreshold) && aws.Int64Value(expected.Interval) == aws.Int64Value(actual.Interval) && aws.Int64Value(expected.Timeout) == aws.Int64Value(actual.Timeout) {
		return nil
	}
...
func (c *Cloud) ensureSSLNegotiationPolicy(loadBalancer *elb.LoadBalancerDescription, policyName string) error {
	...
	result, err := c.elb.DescribeLoadBalancerPolicies(&elb.DescribeLoadBalancerPoliciesInput{LoadBalancerName: loadBalancer.LoadBalancerName, PolicyNames: []*string{aws.String(fmt.Sprintf(SSLNegotiationPolicyNameFormat, policyName))}})
	if err != nil {
...
func (c *Cloud) setSSLNegotiationPolicy(loadBalancerName, sslPolicyName string, port int64) error {
	...
	request := &elb.SetLoadBalancerPoliciesOfListenerInput{LoadBalancerName: aws.String(loadBalancerName), LoadBalancerPort: aws.Int64(port), PolicyNames: []*string{aws.String(policyName)}}
...
func (c *Cloud) createProxyProtocolPolicy(loadBalancerName string) error {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	request := &elb.CreateLoadBalancerPolicyInput{LoadBalancerName: aws.String(loadBalancerName), PolicyName: aws.String(ProxyProtocolPolicyName), PolicyTypeName: aws.String("ProxyProtocolPolicyType"), PolicyAttributes: []*elb.PolicyAttribute{{AttributeName: aws.String("ProxyProtocol"), AttributeValue: aws.String("true")}}}
...
func (c *Cloud) setBackendPolicies(loadBalancerName string, instancePort int64, policies []*string) error {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	request := &elb.SetLoadBalancerPoliciesForBackendServerInput{InstancePort: aws.Int64(instancePort), LoadBalancerName: aws.String(loadBalancerName), PolicyNames: policies}
	if len(policies) > 0 {
...
func (c *Cloud) findInstancesForELB(nodes []*v1.Node) (map[awsInstanceID]*ec2.Instance, error) {
	...
	cacheCriteria := cacheCriteria{HasInstances: instanceIDs}
