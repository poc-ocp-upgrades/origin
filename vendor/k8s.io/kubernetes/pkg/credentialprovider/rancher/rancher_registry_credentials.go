...
type rancherProvider struct{ credGetter credentialsGetter }
type rancherCredentialsGetter struct{ client *client.RancherClient }
type rConfig struct{ Global configGlobal }
type credentialsGetter interface{ getCredentials() []registryCredential }
...
func init() {
 ...
 credentialprovider.RegisterCredentialProvider("rancher-registry-creds", &credentialprovider.CachingDockerConfigProvider{Provider: &rancherProvider{rancherGetter}, Lifetime: 30 * time.Second})
}
...
func (p *rancherProvider) Provide() credentialprovider.DockerConfig {
 ...
 for _, cred := range p.credGetter.getCredentials() {
  entry := credentialprovider.DockerConfigEntry{Username: cred.credential.PublicValue, Password: cred.credential.SecretValue, Email: cred.credential.Email}
...
func (g *rancherCredentialsGetter) getCredentials() []registryCredential {
 ...
 for _, cred := range credColl.Data {
  ...
  registryCred := registryCredential{credential: &cred, serverIP: registry.ServerAddress}
...
func getRancherClient() (*client.RancherClient, error) {
 ...
 conf := rConfig{Global: configGlobal{CattleURL: url, CattleAccessKey: accessKey, CattleSecretKey: secretKey}}
 return client.NewRancherClient(&client.ClientOpts{Url: conf.Global.CattleURL, AccessKey: conf.Global.CattleAccessKey, SecretKey: conf.Global.CattleSecretKey})
}
