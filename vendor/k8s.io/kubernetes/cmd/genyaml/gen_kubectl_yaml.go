...
func genFlagResult(flags *pflag.FlagSet) []cmdOption {
	...
	flags.VisitAll(func(flag *pflag.Flag) {
		if !(len(flag.ShorthandDeprecated) > 0) && len(flag.Shorthand) > 0 {
			opt := cmdOption{flag.Name, flag.Shorthand, flag.DefValue, forceMultiLine(flag.Usage)}
			result = append(result, opt)
		} else {
			opt := cmdOption{Name: flag.Name, DefaultValue: forceMultiLine(flag.DefValue), Usage: forceMultiLine(flag.Usage)}
			result = append(result, opt)
		}
	})
	return result
