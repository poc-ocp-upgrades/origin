...
var (
	placeholderToken = kubeadmapiv1beta1.BootstrapToken{Token: &kubeadmapiv1beta1.BootstrapTokenString{ID: "abcdef", Secret: "0123456789abcdef"}}
)
...
func NewCmdConfig(out io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "config", Short: "Manage configuration for a kubeadm cluster persisted in a ConfigMap in the cluster.", Long: fmt.Sprintf(dedent.Dedent(`
		...
		`), metav1.NamespaceSystem, constants.KubeadmConfigConfigMap), RunE: cmdutil.SubCmdRunE("config")}
...
func NewCmdConfigPrint(out io.Writer) *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	cmd := &cobra.Command{Use: "print", Short: "Print configuration", Long: "This command prints configurations for subcommands provided.", RunE: cmdutil.SubCmdRunE("print")}
...
func newCmdConfigPrintActionDefaults(out io.Writer, action string, configBytesProc func() ([]byte, error)) *cobra.Command {
	...
	cmd := &cobra.Command{Use: fmt.Sprintf("%s-defaults", action), Short: fmt.Sprintf("Print default %s configuration, that can be used for 'kubeadm %s'", action, action), Long: fmt.Sprintf(dedent.Dedent(`
		...
		`), action, action, placeholderToken), Run: func(cmd *cobra.Command, args []string) {
		runConfigPrintActionDefaults(out, componentConfigs, configBytesProc)
	}}
	cmd.Flags().StringSliceVar(&componentConfigs, "component-configs", componentConfigs, fmt.Sprintf("A comma-separated list for component config API objects to print the default values for. Available values: %v. If this flag is not set, no component configs will be printed.", getSupportedComponentConfigAPIObjects()))
	return cmd
}
...
func NewCmdConfigPrintDefault(out io.Writer) *cobra.Command {
	...
	defer _logClusterCodePath("Exited function: ")
	apiObjects := []string{}
	cmd := &cobra.Command{Use: "print-default", Aliases: []string{"print-defaults"}, Short: "Print the default values for a kubeadm configuration object.", Long: fmt.Sprintf(dedent.Dedent(`
			...
			Note that sensitive values like the Bootstrap Token fields are replaced with placeholder values like %q in order to pass validation but
			not perform the real computation for creating a token.
		`), placeholderToken), Deprecated: "Please, use `kubeadm config print` instead.", Run: func(cmd *cobra.Command, args []string) {
		if len(apiObjects) == 0 {
	...
	cmd.Flags().StringSliceVar(&apiObjects, "api-objects", apiObjects, fmt.Sprintf("A comma-separated list for API objects to print the default values for. Available values: %v. This flag unset means 'print all known objects'", getAllAPIObjectNames()))
	return cmd
}
...
func getDefaultedInitConfig() (*kubeadmapi.InitConfiguration, error) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	return configutil.ConfigFileAndDefaultsToInternalConfig("", &kubeadmapiv1beta1.InitConfiguration{LocalAPIEndpoint: kubeadmapiv1beta1.APIEndpoint{AdvertiseAddress: "1.2.3.4"}, ClusterConfiguration: kubeadmapiv1beta1.ClusterConfiguration{KubernetesVersion: fmt.Sprintf("v1.%d.0", constants.MinimumControlPlaneVersion.Minor()+1)}, BootstrapTokens: []kubeadmapiv1beta1.BootstrapToken{placeholderToken}})
}
...
func getDefaultNodeConfigBytes() ([]byte, error) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	internalcfg, err := configutil.JoinConfigFileAndDefaultsToInternalConfig("", &kubeadmapiv1beta1.JoinConfiguration{Discovery: kubeadmapiv1beta1.Discovery{BootstrapToken: &kubeadmapiv1beta1.BootstrapTokenDiscovery{Token: placeholderToken.Token.String(), APIServerEndpoint: "kube-apiserver:6443", UnsafeSkipCAVerification: true}}})
	if err != nil {
		return []byte{}, err
	}
	return configutil.MarshalKubeadmConfigObject(internalcfg)
...
func NewCmdConfigMigrate(out io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "migrate", Short: "Read an older version of the kubeadm configuration API types from a file, and output the similar config object for the newer version.", Long: fmt.Sprintf(dedent.Dedent(`
		...
		`), kubeadmapiv1alpha3.SchemeGroupVersion.String(), kubeadmapiv1beta1.SchemeGroupVersion.String(), kubeadmapiv1beta1.SchemeGroupVersion.String()), Run: func(cmd *cobra.Command, args []string) {
		if len(oldCfgPath) == 0 {
...
func NewCmdConfigUpload(out io.Writer, kubeConfigFile *string) *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	cmd := &cobra.Command{Use: "upload", Short: "Upload configuration about the current state, so that 'kubeadm upgrade' can later know how to configure the upgraded cluster.", RunE: cmdutil.SubCmdRunE("upload")}
...
func NewCmdConfigView(out io.Writer, kubeConfigFile *string) *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	return &cobra.Command{Use: "view", Short: "View the kubeadm configuration stored inside the cluster.", Long: fmt.Sprintf(dedent.Dedent(`
			Using this command, you can view the ConfigMap in the cluster where the configuration for kubeadm is located.

			The configuration is located in the %q namespace in the %q ConfigMap.
		`), metav1.NamespaceSystem, constants.KubeadmConfigConfigMap), Run: func(cmd *cobra.Command, args []string) {
...
func NewCmdConfigUploadFromFile(out io.Writer, kubeConfigFile *string) *cobra.Command {
	...
	defer _logClusterCodePath("Exited function: ")
	var cfgPath string
	cmd := &cobra.Command{Use: "from-file", Short: "Upload a configuration file to the in-cluster ConfigMap for kubeadm configuration.", Long: fmt.Sprintf(dedent.Dedent(`
...
func NewCmdConfigUploadFromFlags(out io.Writer, kubeConfigFile *string) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "from-flags", Short: "Create the in-cluster configuration file for the first time from using flags.", Long: fmt.Sprintf(dedent.Dedent(`
...
func NewCmdConfigImages(out io.Writer) *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	cmd := &cobra.Command{Use: "images", Short: "Interact with container images used by kubeadm.", RunE: cmdutil.SubCmdRunE("images")}
...
func NewCmdConfigImagesPull() *cobra.Command {
	...
	var err error
	cmd := &cobra.Command{Use: "pull", Short: "Pull images used by kubeadm.", Run: func(_ *cobra.Command, _ []string) {
...
func NewImagesPull(runtime utilruntime.ContainerRuntime, images []string) *ImagesPull {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	return &ImagesPull{runtime: runtime, images: images}
}
...
func NewCmdConfigImagesList(out io.Writer, mockK8sVersion *string) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "list", Short: "Print a list of images kubeadm will use. The configuration file is used in case any images or image repositories are customized.", Run: func(_ *cobra.Command, _ []string) {
		externalcfg.ClusterConfiguration.FeatureGates, err = features.NewFeatureGate(&features.InitFeatureGates, featureGatesString)
		kubeadmutil.CheckErr(err)
...
func NewImagesList(cfgPath string, cfg *kubeadmapiv1beta1.InitConfiguration) (*ImagesList, error) {
	...
	return &ImagesList{cfg: initcfg}, nil
}

type ImagesList struct{ cfg *kubeadmapi.InitConfiguration }
...
func AddImagesCommonConfigFlags(flagSet *flag.FlagSet, cfg *kubeadmapiv1beta1.InitConfiguration, cfgPath *string, featureGatesString *string) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	flagSet.StringVar(&cfg.ClusterConfiguration.KubernetesVersion, "kubernetes-version", cfg.ClusterConfiguration.KubernetesVersion, `Choose a specific Kubernetes version for the control plane.`)
	flagSet.StringVar(featureGatesString, "feature-gates", *featureGatesString, "A set of key=value pairs that describe feature gates for various features. "+"Options are:\n"+strings.Join(features.KnownFeatures(&features.InitFeatureGates), "\n"))
