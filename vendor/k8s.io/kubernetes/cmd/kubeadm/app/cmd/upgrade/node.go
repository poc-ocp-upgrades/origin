...
func NewCmdNode() *cobra.Command {
	...
	cmd := &cobra.Command{Use: "node", Short: "Upgrade commands for a node in the cluster. Currently only supports upgrading the configuration, not the kubelet itself.", RunE: cmdutil.SubCmdRunE("node")}
...
func NewCmdUpgradeNodeConfig() *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	flags := &nodeUpgradeFlags{kubeConfigPath: constants.GetKubeletKubeConfigPath(), kubeletVersionStr: "", dryRun: false}
	cmd := &cobra.Command{Use: "config", Short: "Downloads the kubelet configuration from the cluster ConfigMap kubelet-config-1.X, where X is the minor version of the kubelet.", Long: upgradeNodeConfigLongDesc, Example: upgradeNodeConfigExample, Run: func(cmd *cobra.Command, args []string) {
...
func NewCmdUpgradeControlPlane() *cobra.Command {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	flags := &controlplaneUpgradeFlags{kubeConfigPath: constants.GetKubeletKubeConfigPath(), advertiseAddress: "", etcdUpgrade: true, dryRun: false}
	cmd := &cobra.Command{Use: "experimental-control-plane", Short: "Upgrades the control plane instance deployed on this node. IMPORTANT. This command should be executed after executing `kubeadm upgrade apply` on another control plane instance", Long: upgradeNodeConfigLongDesc, Example: upgradeNodeConfigExample, Run: func(cmd *cobra.Command, args []string) {
		if flags.nodeName == "" {
...
func printFilesIfDryRunning(dryRun bool, kubeletDir string) error {
	...
	fileToPrint := dryrunutil.FileToPrint{RealPath: filepath.Join(kubeletDir, constants.KubeletConfigurationFileName), PrintPath: filepath.Join(constants.KubeletRunDirectory, constants.KubeletConfigurationFileName)}
