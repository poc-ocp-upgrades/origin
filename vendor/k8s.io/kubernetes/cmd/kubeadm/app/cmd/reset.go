...
func NewCmdReset(in io.Reader, out io.Writer) *cobra.Command {
	...
	cmd := &cobra.Command{Use: "reset", Short: "Run this to revert any changes made to this host by 'kubeadm init' or 'kubeadm join'.", Run: func(cmd *cobra.Command, args []string) {
	...
	cmd.PersistentFlags().StringVar(&certsDir, "cert-dir", kubeadmapiv1beta1.DefaultCertificatesDir, "The path to the directory where the certificates are stored. If specified, clean this directory.")
	cmd.PersistentFlags().StringVar(&criSocketPath, "cri-socket", kubeadmapiv1beta1.DefaultCRISocket, "The path to the CRI socket to use with crictl when cleaning up containers.")
	cmd.PersistentFlags().BoolVarP(&forceReset, "force", "f", false, "Reset the node without prompting for confirmation.")
	return cmd
}
...
func NewReset(in io.Reader, ignorePreflightErrors sets.String, forceReset bool, certsDir, criSocketPath string) (*Reset, error) {
	...
	return &Reset{certsDir: certsDir, criSocketPath: criSocketPath}, nil
}
...
func resetConfigDir(configPathDir, pkiPathDir string) {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	dirsToClean := []string{filepath.Join(configPathDir, kubeadmconstants.ManifestsSubDirName), pkiPathDir}
	...
	filesToClean := []string{filepath.Join(configPathDir, kubeadmconstants.AdminKubeConfigFileName), filepath.Join(configPathDir, kubeadmconstants.KubeletKubeConfigFileName), filepath.Join(configPathDir, kubeadmconstants.KubeletBootstrapKubeConfigFileName), filepath.Join(configPathDir, kubeadmconstants.ControllerManagerKubeConfigFileName), filepath.Join(configPathDir, kubeadmconstants.SchedulerKubeConfigFileName)}
