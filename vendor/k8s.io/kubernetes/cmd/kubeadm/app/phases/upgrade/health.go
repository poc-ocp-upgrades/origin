...
func CheckClusterHealth(client clientset.Interface, ignoreChecksErrors sets.String) error {
	...
	healthChecks := []preflight.Checker{&healthCheck{name: "APIServerHealth", client: client, f: apiServerHealthy}, &healthCheck{name: "MasterNodesReady", client: client, f: masterNodesReady}}
	healthChecks = append(healthChecks, &healthCheck{name: "StaticPodManifest", client: client, f: staticPodManifestHealth})
...
func masterNodesReady(client clientset.Interface) error {
	_logClusterCodePath("Entered function: ")
	defer _logClusterCodePath("Exited function: ")
	selector := labels.SelectorFromSet(labels.Set(map[string]string{constants.LabelNodeRoleMaster: ""}))
	masters, err := client.CoreV1().Nodes().List(metav1.ListOptions{LabelSelector: selector.String()})
	if err != nil {
...
func daemonSetHealth(dsStatus *apps.DaemonSetStatus) error {
	...
	if dsStatus.CurrentNumberScheduled != dsStatus.DesiredNumberScheduled {
		return errors.Errorf("current number of scheduled Pods ('%d') doesn't match the amount of desired Pods ('%d')", dsStatus.CurrentNumberScheduled, dsStatus.DesiredNumberScheduled)
	}
