...
func TestValidDependenciesArePinnedToOutput(t *testing.T) {
	...
	yamlfile := &glide.YamlFile{Package: "test", Imports: []*glide.YamlFileImport{{Package: "pkg/one", Version: "1"}, {Package: "pkg/two", Version: "2"}}}
	lockfile := &glide.LockFile{Hash: "test", Updated: time.Now(), Imports: []*glide.LockFileImport{{Name: "pkg/one", Version: "1"}, {Name: "pkg/two", Version: "2"}, {Name: "pkg/three", Version: "3"}, {Name: "pkg/four", Repo: "repo", Version: "4"}}}
	encodedYamlFile, err := yamlfile.Encode()
	if err != nil {
		t.Fatal(err)
	}
	opts := &PinImportsOpts{LockFile: lockfile, YamlFile: yamlfile, ExistingGlideYamlContent: encodedYamlFile, Out: out, ErrOut: errout}
	if err := opts.Validate(); err != nil {
		t.Fatal(err)
	}
	err = opts.Run()
...
func TestFlagsToOptionsReturnsOptions(t *testing.T) {
	...
	}{{name: "error on empty LockFile", expectErr: true, expectErrMessage: "contained an empty file"}, {name: "error on empty YamlFile", expectErr: true, lockFileData: []byte(lockFileData), expectErrMessage: "contained an empty file"}, {name: "error on invalid data format", lockFileData: []byte(lockFileData), yamlFileData: []byte("invalid"), expectErr: true, expectErrMessage: "cannot unmarshal"}, {name: "success creating options with decoded yaml bytes", lockFileData: []byte(lockFileData), yamlFileData: []byte(yamlFileData)}}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			flags := PinImportsFlags{LockFileName: lockfile.Name(), YamlFileName: yamlfile.Name()}
...
func TestPinImportsAppendsNewImports(t *testing.T) {
	...
	opts := PinImportsOpts{LockFile: lockfile, YamlFile: yamlfile, Out: out, ErrOut: errout}
	err := opts.Run()
	if err != nil {
		t.Fatalf("%v", err)
	}
