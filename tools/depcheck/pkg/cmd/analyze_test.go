...
func TestGraphAnalyzisCalculatesYoursMineOurs(t *testing.T) {
	_logClusterCodePath()
	defer _logClusterCodePath()
	opts := &AnalyzeOptions{GraphOptions: newGraphOptions(), Dependencies: []string{"foo.com/bar/baz/vendor/vendor.com/one"}}
	...
	expectedYours := []string{"vendor.com/two", "vendor.com/three"}
	expectedMine := []string{"vendor.com/one", "vendor.com/mine"}
	expectedOurs := []string{"vendor.com/ours"}
	expectedMissing := []string{"vendor.com/transitive_ours"}
