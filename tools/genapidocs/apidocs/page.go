...
func (page Page) OutputPath() string {
	...
	dir := parts[0]
	if gvk.Group == "extensions" && gvk.Version == "v1" && gvk.Kind == "ReplicationController" {
		dir = "api"
	}
	if gvk.Group != "" {
		dir += "-" + gvk.Group
...
func BuildPages(s *spec.Swagger) (Pages, error) {
	...
	for pathName, path := range s.Paths.Paths {
		if strings.HasPrefix(pathName, "/api/v1/proxy/") {
			continue
		}
		for opName, op := range Operations(path) {
			o := operation{s: s, Path: path, PathName: pathName, Operation: op, OpName: opName}
			var err error
			o.gvk, err = o.GVK()
			if err != nil {
				return nil, err
...
func (pages Pages) Write(root string) error {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	t, err := template.New("page.adoc").Funcs(template.FuncMap{"FriendlyTypeName": FriendlyTypeName, "EscapeMediaTypes": EscapeMediaTypes}).ParseGlob("tools/genapidocs/apidocs/templates/*")
	if err != nil {
		return err
	}
