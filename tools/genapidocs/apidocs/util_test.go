...
func TestFriendlyTypeName(t *testing.T) {
	...
	_logClusterCodePath()
	defer _logClusterCodePath()
	s := FriendlyTypeName(&spec.Schema{SchemaProps: spec.SchemaProps{Ref: spec.Ref{Ref: jsonreference.MustCreateRef("")}, Type: spec.StringOrArray{"string"}}})
	if s != "string" {
		t.Error(s)
	}
	s = FriendlyTypeName(&spec.Schema{SchemaProps: spec.SchemaProps{Ref: spec.Ref{Ref: jsonreference.MustCreateRef("#/definitions/baz.bar.foo")}}})
	if s != "bar.foo" {
		t.Error(s)
	}
}
...
func TestOperations(t *testing.T) {
	...
	s := Operations(spec.PathItem{PathItemProps: spec.PathItemProps{Get: &get, Put: &put, Post: &post, Delete: &delete, Options: &options, Head: &head, Patch: &patch}})
	if !reflect.DeepEqual(s, map[string]*spec.Operation{"Get": &get, "Put": &put, "Post": &post, "Delete": &delete, "Options": &options, "Head": &head, "Patch": &patch}) {
		t.Error(s)
	}
}
...
func TestPluralise(t *testing.T) {
	...
	tests := []struct {
		singular	string
		plural		string
	}{{singular: "APIVersions", plural: "APIVersions"}, {singular: "Endpoints", plural: "Endpoints"}, {singular: "SecurityContextConstraints", plural: "SecurityContextConstraints"}, {singular: "ComponentStatus", plural: "ComponentStatuses"}, {singular: "Policy", plural: "Policies"}, {singular: "Pod", plural: "Pods"}}
